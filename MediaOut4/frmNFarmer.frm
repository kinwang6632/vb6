...
Begin VB.Form frmNFarmer 
   Appearance      =   0  '平面
   ClientHeight    =   6810
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11190
   Icon            =   "frmNFarmer.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   ScaleHeight     =   6810
...
'    strUpdTime = Format(RightNow, "EE/MM/DD HH:MM:SS")
    ...
    '*********************************************************************************************************************************************************
        ...
        '************************************************************************
        '#5683 挑選出RealStopDate 與 PayKind By Kin 2010/08/05
        If strWhere <> "" Then strWhere = Mid(strWhere, 5)
        If strMedia = 0 Then
            '#6441 單張帳單合計總額<=0是否產生 By Kin 2013/05/13
            strSQL = "SELECT " & GetUseIndexStr("SO033", "ShouldDate") & _
                    " A.BillNo,Sum(A.ShouldAmt) ShouldAmt,A.AccountNo,A.CustId, " & _
                    " A.MediaBillNo,Max(Nvl(A.PayKind,0)) PayKind, " & _
...
'                End If
           ...
           End If
           If strMedia = 1 Then
              strBillNo = "000" & GetFieldValue(rsTmp, "BillNo") & ""
           ...
           Else
              strBillNo = "000" & GetBillNo_New(GetFieldValue(rsTmp, "BillNo") & "")
           ...
           With rsDefTmp
                ...
                '扣帳金額 15-22
                .Fields("TransStatus") = 9
                '轉帳狀態 23-23
                .Fields("ResvDate") = Replace(gdaTransfer.GetOriginalValue, "/", "") & ""
                '轉帳預定日期 24-29
                .Fields("TransDate") = ""
                '完成轉帳日期 30-35
                .Fields("Memo") = ""
...
lNextRcd:
        ...
        If blnUpdUcCode And 1 = 0 Then
            ...
            If Not GetRS(rsUpdUCCode, strUPDUCCode, gcnGi, adUseClient, adOpenKeyset, adLockPessimistic) Then Exit Function
            Do While Not rsUpdUCCode.EOF
                '#5683 如果收視截止日小於畫面條件,不要UPD By Kin 2010/08/05
                '#5843 如果設定不檢核,則強迫出帳 By Kin 2010/11/24
                If (IsPayKindOK(rsUpdUCCode, gdaTransfer.GetValue & "", giSingle, Val(strMedia))) _
                    Or (Not blnChkPayKind) Then
                    rsUpdUCCode("UPDEN") = objStorePara.uUpdEn
  ...
  On Error GoTo ChkErr
    ...
    With rsDefTmp
         ...
         For lngloop = 0 To .RecordCount - 1
            WriteLineData GetString(varData(0, lngloop), 14) & GetString(varData(1, lngloop), 8, GIRIGHT, True) & _
                GetString(varData(2, lngloop), 1) & GetString(varData(3, lngloop), 6) & _
                GetString(varData(4, lngloop), 6) & GetString(varData(5, lngloop), 3) & _
                GetString(varData(6, lngloop), 8) & GetString(varData(7, lngloop), 14), 0
             DoEvents
         Next
    End With
    subWriteLine = True
...
Private Sub DefineRs()
    ...
    With rsDefTmp
        ...
        .Fields.Append ("Amt"), adBSTR, 8, adFldIsNullable         '轉帳金額
        .Fields.Append ("TransStatus"), adBSTR, 1, adFldIsNullable '轉帳狀態
        .Fields.Append ("ResvDate"), adBSTR, 6, adFldIsNullable    '轉帳預定日期
        .Fields.Append ("TransDate"), adBSTR, 8, adFldIsNullable   '完成轉帳日期
        .Fields.Append ("Memo"), adBSTR, 3, adFldIsNullable        '摘要
        .Fields.Append ("CorpId"), adBSTR, 8, adFldIsNullable      '委託單位
        .Fields.Append ("BillNo"), adBSTR, 14, adFldIsNullable     '000+單據編號
        .Open
    End With
    Exit Sub
ChkErr:
...
Private Sub RcdToScr()
    ...
    Dim rsBank As New ADODB.Recordset
        ...
        strSQL = "Select CorpId From " & strGetOwner & "CD018 Where CompCode =" & strCompCode & " And CodeNo = '" & strBankId & "'"
