...
Begin VB.Form frmChuNan 
   ClientHeight    =   6810
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   11190
   Icon            =   "frmChuNan.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   ScaleHeight     =   6810
...
'              IIf(Len(strWhere) = 0, "", " And ") & strChoose
    '#6441 單張帳單合計總額<=0是否產生 By Kin 2013/05/13
    '7179
    strUPDUCCode = GetUpdateSQL(Val(strMedia), StrTableName, _
                            strWhere, strChoose, blnHavingOutZero, IIf(isCrossCustCombine, strSQL, ""))
    '**************************************************************************************************
    ...
    Else
        blnNodata = False
        strOldBillNo = "-1"
        '#5683 如果RealStopDate<畫面條件,則不能出帳 By Kin 2010/08/04
...
'           lngShouldAmt = Val(GetRsValue("Select Sum(ShouldAmt) From " & strGetOwner & "So033 Where BillNo = '" & rsTmp("BillNo") & "' And CancelFlag = 0 And UCCode Is Not Null ", gcnGi) & "")
           ...
           With rsDefTmp
                ...
                '扣帳帳號 1-12
                .Fields("TransDate") = gdaTransfer.GetValue
                '轉帳日期 13-19
                .Fields("Type") = "4"
                '存提別 21-21
                '***************************************************************
                '#3968 原本固定填83,改填畫面上的企業識別碼 By Kin 2008/07/10
                '.Fields("Memo") = "83"
                .Fields("Memo") = objStorePara.uBID
                '摘要 22-23
                '***************************************************************
                .Fields("Amt") = lngShouldAmt
                '扣帳金額 24-36(後2位為小數)
                .Fields("BillNo") = strBillNo
                '所屬月份+客戶號碼(舊版單編) 37-49
                .Fields("Note") = "0"
                '保留欄位 50-50
...
lNextRcd:
        ...
        If blnUpdUcCode And 1 = 0 Then
            ...
            If Not GetRS(rsUpdUCCode, strUPDUCCode, gcnGi, adUseClient, adOpenKeyset, adLockPessimistic) Then Exit Function
            Do While Not rsUpdUCCode.EOF
                '#5683 如果RealStopDate<小於畫面條則不UPD By Kin 2010/08/04
                '#5843 如果設定不檢核,則強迫出帳 By Kin 2010/11/24
                If (IsPayKindOK(rsUpdUCCode, gdaTransfer.GetValue, giSingle, Val(strMedia))) _
                    Or (Not blnChkPayKind) Then
                    rsUpdUCCode("UPDEN") = objStorePara.uUpdEn
  ...
  On Error GoTo ChkErr
    ...
    With rsDefTmp
         ...
         For lngloop = 0 To .RecordCount - 1
            WriteLineData GetString(varData(0, lngloop), 12) & GetString(varData(1, lngloop), 8) & _
                GetString(varData(2, lngloop), 1) & GetString(varData(3, lngloop), 2) & _
                GetString(varData(4, lngloop), 11, GIRIGHT, True) & "00" & GetString(varData(5, lngloop), 13) & _
                GetString(varData(6, lngloop), 1), 0
             DoEvents
         Next
    End With
    subWriteLine = True
...
Private Sub DefineRs()
    ...
    With rsDefTmp
        ...
        .Fields.Append ("TransDate"), adBSTR, 8, adFldIsNullable   '轉帳日期
        .Fields.Append ("Type"), adBSTR, 1, adFldIsNullable        '存提別
        .Fields.Append ("Memo"), adBSTR, 2, adFldIsNullable        '摘要
        .Fields.Append ("Amt"), adBSTR, 13, adFldIsNullable        '交易金額
        .Fields.Append ("BillNo"), adBSTR, 13, adFldIsNullable     '單據編號
        .Fields.Append ("Note"), adBSTR, 1, adFldIsNullable        '保留欄位
        .Open
    End With
    Exit Sub
ChkErr:
...
'        strSQL = "Select InvoiceId From SO041 Where CompCode =" & strCompCode & " And ServiceType = '" & strServiceType & "'"
