...
'    strUpdTime = Format(RightNow, "EE/MM/DD HH:MM:SS")
    ...
    Else
        ...
        strSQL = "SELECT " & GetUseIndexStr("SO033", "ShouldDate") & " A.MediaBillNo BillNo,Sum(A.ShouldAmt) ShouldAmt," & _
                " A.AccountNo,Max(NVL(A.PayKind,0)) PAYKIND, " & IIf(isCrossCustCombine, "A.CustId,", "1 CustId,") & _
                strMinRealStopDate & _
                 " From " & strGetOwner & "SO033 A " & StrTableName & _
                 " Where " & strWhere & _
                 IIf(Len(strWhere) = 0, "", " And ") & strChoose & strHavingOutZero & _
                 " Group By A.AccountNo,A.MediaBillNo,A.PayKind " & IIf(isCrossCustCombine, ",A.CustId", "")
         strSQL = "Select * From ( " & strSQL & " ) "
