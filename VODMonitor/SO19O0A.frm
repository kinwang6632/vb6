...
Begin VB.Form frmSO19O0A 
   BorderStyle     =   1  '單線固定
   Caption         =   "VOD監控中心 [SO19O0A]"
   ClientHeight    =   7560
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   11835
   ...
   Begin VB.Frame fraData 
      ...
      Begin VB.ComboBox cboType 
         Height          =   315
         ItemData        =   "SO19O0A.frx":0000
         Left            =   6930
         List            =   "SO19O0A.frx":0013
         Style           =   2  '單純下拉式
         TabIndex        =   29
         Top             =   150
         Width           =   2865
...
'    End If
    strWhere = " Where Exists(Select * From " & GetOwner & "SO555 Where CM003.EmpNo=SO555.OperatorId)"
    
    SetgiMulti gimExecEntry, "EmpNo", "EmpName", "CM003", "員工代號", "員工姓名", strWhere
    
    SetgiMultiAddItem gimCommand, "A1,A2,A3,A4,A6,A7,A8,A9,B11,B12,E1,C4", _
                                "啟動訂購權,關閉訂購權,暫停訂購權,恢復訂購權,維修換拆(整組換)," & _
                                "配對,維修換拆 STB Swap,維修換拆 Smart Card Swap,訂購SVOD,取消SVOD," & _
                                "重設訂購密碼,設定變更區域"
...
Private Function OpenData(ByVal blnClear As Boolean) As Boolean
  On Error GoTo ChkErr
    ...
    StrTableName = strCMOwner & "SO555 A"
    strField = "A.RowId,A.*"
...
Private Function subChoose() As String
  On Error GoTo ChkErr
    ...
    If gilCompCode.GetCodeNo <> Empty Then Call subAnd("A.ServiceProviderID=" & gilCompCode.GetCodeNo)
    'If gilCompCode.GetCodeNo <> Empty Then Call subAnd("A.CompCode=" & gilCompCode.GetCodeNo)
    If gdtUpdTime1.GetValue <> Empty Then
        Call subAnd("A.ExchTime>=To_Date(" & gdtUpdTime1.GetValue & ",'YYYY/MM/DD HH24:MI:SS')")
'        Select Case intCMType
'            Case 2
...
'        End Select
    ...
    If gdtUpdTime2.GetValue <> Empty Then
        subAnd ("A.ExchTime<=To_Date(" & gdtUpdTime1.GetValue & ",'YYYY/MM/DD HH24:MI:SS')")
'        Select Case intCMType
'            Case 2
...
'        End Select
    ...
    If gimCommand.GetQueryCode <> Empty Then
        Call subAnd("A.CMD In(" & gimCommand.GetQueryCode & ")")
'        If intCMType = 2 Then
...
'            End If
        ...
        Case 2  '錯誤命令重送(一定要有錯誤的才處理)
            Call subAnd("A.CmdStatus<>'W' And A.CmdStatus<>'S'  And A.CmdStatus<>'P'")
'            If intCMType = 2 Then
...
'        End If
    ElseIf optHandleYes.Value Then  '已處理
        Call subAnd("A.CMDSTATUS IN('S','E','T')")
'        If intCMType = 2 Then
