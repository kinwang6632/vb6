...
Private Sub cmdSTB_Click()
  On Error GoTo ChkErr
    ...
    With frmSO1131E
        ...
        .uEditMode = giEditModeEdit
        '#6395 拆除設備要改成能選取 By Kin 2012/12/18
        .uPRCanClick = True
        .uRefNo = 3
...
Private Sub subPrint(ByVal aEXETYPE As Integer)
  On Error GoTo ChkErr
    ...
    Else
        ...
        Call PrintRpt2(GetPrinterName(5), "SO32E2A.RPT", , "預計結算明細報表 [SO32E2A]", strPrintSQL, strChooseString, , True, , , strFormula)
        SendSQL , , True
    End If
    Me.Enabled = True
    Screen.MousePointer = vbDefault
...
'                " GROUP BY CUSTID,VODACCOUNTID,FACISNO,SUM3"
    ...
    '#6388 SQL語法換掉 By Kin 2013/01/02
'    strCrtSQL = "SELECT DISTINCT CUSTID,VODACCOUNTID,FACISNO, " & _
'            " NOPAYCREDIT MINUSCREDIT,PREPAY ADDCREDIT,MUSTPAYCREDIT TOTAL " & _
...
'               " AND NVL(B.STOPFLAG,0)=0 AND A.SEQNO=B.FACISEQNO "
    ...
    If strViewXLSName <> "" And txtXLSFile.Text <> "" Then
        strExcelWhere = " AND A.FACISNO IN ( SELECT FACISNO FROM " & strViewXLSName & ") " & _
                " AND A.VODACCOUNTID IN ( SELECT VODACCOUNTID FROM " & strViewXLSName & " ) "
    End If
    ...
    End If
    '如果不是EXCEL的資料,不要執行結算 By Kin 2010/08/06
    '#6101 如果勾選強迫出帳則強制出帳 By Kin 2011/08/09
...
Private Function subChoose() As Boolean
  On Error GoTo ChkErr
    ...
    If strViewXLSName <> Empty And txtXLSFile.Text <> "" Then
        Call subAnd2(strChoose, " A.CUSTID IN( SELECT CUSTID From " & strViewXLSName & ")")
    End If
'    If gdaEndDate.GetValue <> "" Then
...
'    End If
    ...
    '#6395 拆機也要能算出,把 A.PRDATE IS NULL 條件取消 By Kin 2012/12/18
...
Private Function GetSQL(ByVal strWhere As String)
  On Error GoTo ChkErr
    ...
    aExcelWhere2 = Empty
     If strViewXLSName <> "" And txtXLSFile.Text <> "" Then
        aExcelWhere1 = " ( A.FACISNO IN ( SELECT FACISNO FROM " & strViewXLSName & ") " & _
                " AND A.VODACCOUNTID IN ( SELECT VODACCOUNTID FROM " & strViewXLSName & " ) " & _
                " AND (NVL(A.MUSTPAYCREDIT,0) - NVL(A.UNPAY,0)-NVL(A.OverCredit,0) ) < " & IIf(blnforceBill, "-1", " nvl(B.PARA35,0)") & ")"
                
        aExcelWhere2 = " ( A.FACISNO NOT IN ( SELECT FACISNO FROM " & strViewXLSName & ") " & _
                " AND A.VODACCOUNTID NOT IN ( SELECT VODACCOUNTID FROM " & strViewXLSName & " ) "
                
    End If
    aSQL1 = "SELECT RANK() OVER (PARTITION BY B.VODACCOUNTID ORDER BY B.SEQNO ) RankX," & _
                " A.NOPAYCREDIT,A.PREPAY,A.PRESENT,B.VODACCOUNTID,B.SERVICETYPE,B.COMPCODE,B.CUSTID,B.SEQNO,B.FACICODE,B.FACISNO " & _
                " FROM " & GetOwner & "SO004G A," & GetOwner & "SO004 B " & _
                " WHERE A.VODACCOUNTID = B.VODACCOUNTID AND A.NOPAYCREDIT > A.PREPAY + A.PRESENT " & _
    ...
    aSQL1 = "SELECT (A.NOPAYCREDIT - A.PREPAY + A.PRESENT ) MustPayCredit, " & _
            ...
            " AND A.CUSTID = C.CUSTID AND A.SEQNO = C.FACISEQNO " & _
            " AND C.UCCODE NOT IN (SELECT CODENO FROM " & GetOwner & "CD013 WHERE REFNO IN (3,7,8) OR PAYOK = 1) " & _
            " AND (C.CITEMCODE IN (SELECT CODENO FROM " & GetOwner & "CD019 WHERE REFNO =21) OR C.SALEPOINTCODE IS NOT NULL)) UNPAY," & _
            " (SELECT Min(incurredDate) FROM (" & _
            "  SELECT VODACCOUNTID,INCURREDDATE,SEQNO,VALUE FROM " & GetOwner & "SO033VOD D " & _
            " WHERE A.VODACCOUNTID = D.VODACCOUNTID " & _
            " MINUS " & _
            " SELECT F.VODACCOUNTID,F.INCURREDDATE,F.SEQNO,VALUE " & _
            " FROM " & GetOwner & "SO182SO033VOD E ," & GetOwner & "SO033VOD F " & _
            " WHERE E.CREDITSEQNO = F.SEQNO AND A.VODACCOUNTID = F.VODACCOUNTID) " & _
            " B WHERE A.VODACCOUNTID = B.VODACCOUNTID AND VALUE >0 ) MINUSEDATE," & _
    ...
    aSQL1 = "SELECT A.*,0 FLAG,B.Para35,B.Para36 FROM ( " & aSQL1 & " A WHERE A.RANKX=1) A,SO043 B " & _
            " WHERE A.SERVICETYPE = B.SERVICETYPE AND (NVL(A.MUSTPAYCREDIT,0) - NVL(A.UNPAY,0) - NVL(A.OVERCREDIT,0) ) > 0 " & _
            " AND ((nvl(A.MUSTPAYCREDIT,0) - nvl(A.UNPAY,0)-nvl(A.OverCredit,0) ) >=" & IIf(blnforceBill, "1", " nvl(B.PARA35,0) ") & _
            " OR TO_DATE(TO_CHAR(A.MINUSEDATE,'yyyymm') || '01','yyyymmdd')+B.PARA36 <=To_Date(To_Char(Sysdate,'yyyymm') ||'01','yyyymmdd') " & _
            IIf(aExcelWhere1 = Empty, "", " OR " & aExcelWhere1) & " )" & _
            " AND " & strWhere

    aSQL2 = "SELECT A.*,1 FLAG,B.Para35,B.Para36 FROM ( " & aSQL2 & " A WHERE A.RANKX=1) A,SO043 B " & _
            " WHERE A.SERVICETYPE = B.SERVICETYPE AND (NVL(A.MUSTPAYCREDIT,0) - NVL(A.UNPAY,0) - NVL(A.OVERCREDIT,0) ) > 0 " & _
            " AND ((nvl(A.MUSTPAYCREDIT,0) - nvl(A.UNPAY,0)-nvl(A.OverCredit,0) ) < " & IIf(blnforceBill, "-1", " nvl(B.PARA35,0) ") & _
            " AND TO_DATE(TO_CHAR(A.MINUSEDATE,'yyyymm') || '01','yyyymmdd')+B.PARA36 > To_Date(To_Char(Sysdate,'yyyymm') ||'01','yyyymmdd') " & _
...
Private Function CreateBill(ByRef aHaveData As Boolean) As Boolean
  On Error GoTo ChkErr
    ...
    If Not GetRS(rs182, "SELECT * FROM (" & strSQLExe & ") Order By VodAccountId,EXETYPE  ", gcnGi, _
                adUseClient, adOpenKeyset, adLockOptimistic) Then Exit Function

    '#6388 語法換掉 By Kin 2013/01/02
...
'        .Fields("ShouldAmt").Value = Abs(Val(rsSO182.Fields("MINUSTotal") & "") - Val(rsSO182.Fields("ADDTOTAL") & "") - Val(rsSO182.Fields("SUM3") & ""))
        ...
        '#6388 語法變更,欄位也變更 By Kin 2013/01/02
'        .Fields("OldAmt").Value = rsSO182("MUSTPAYCREDIT")
'        .Fields("ShouldAmt").Value = rsSO182("MUSTPAYCREDIT")
