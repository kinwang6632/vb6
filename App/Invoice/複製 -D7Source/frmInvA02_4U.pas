...
type
  ...
  TfrmInvA02_4 = class(TForm)
    ...
    cxLabel31: TcxLabel;
    DetailGridViewColumn1: TcxGridDBBandedColumn;
...
uses cbUtilis, Uotheru, frmMainU, dtmMainU, dtmMainHU, dtmMainJU, dtmSOU,
     ...
     frmSearchCustU, frmInvA02_6U,frmInvA02_7U, StdConvs;
...
//    chkEInv.Checked := False;
  ...
  begin
    ...
    txtGiveUnit.Enabled := ( FInvCreateType in [icLocale] );
  end
    ...
    try
      ...
      begin
        ...
        begin
          ...
          dtmMain.adoComm.Open;
          if dtmMain.adoComm.Recordset.RecordCount > 0 then
          begin
            aAccountNo := dtmMain.adoComm.FieldByName( 'ACCOUNTNO' ).AsString;
            aCustId := dtmMain.adoComm.FieldByName( 'CUSTID' ).AsString;
          end;
        end;
      end;
      dtmMain.adoComm.Close;
      ...
      begin
        ...
        aFaciSNO := dtmMain.adoComm.FieldByName( 'FACISNO' ).AsString;
        if dtmMain.adoComm.FieldByName( 'FACISEQNO' ).AsString <> '' then
        begin
          aSQL := Format( 'SELECT SmartCardNo FROM %s.SO004 WHERE          ' +
            'SEQNO = ''%s''',
            [dtmMain.getMisDbOwner,
            dtmMain.adoComm.FieldByName( 'FACISEQNO').AsString] );
          dtmMain.adoComm.Close;
          dtmMain.adoComm.SQL.Text := aSQL;
          dtmMain.adoComm.Open;
          if dtmMain.adoComm.Recordset.RecordCount > 0 then
          begin
            if dtmMain.adoComm.FieldByName( 'SMARTCARDNO' ).AsString <> '' then
            begin
              aSmartCardNo := dtmMain.adoComm.FieldByName( 'SMARTCARDNO' ).AsString;
            end;
          end;
        end;
...
function TfrmInvA02_4.SyncInv008ADataSet(const aMode: TDMLMode;
  ...
  begin
    ...
    {}
    if not cdsInv008A.Locate('ITEMIDREF;BILLID;BILLIDITEMNO',VarArrayOf(
