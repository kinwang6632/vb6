object frmInvB06: TfrmInvB06
  Left = 250
  Top = 195
  ActiveControl = mskSPECIALPRIZE1
  BorderStyle = bsDialog
  Caption = '[B06] '#20013#29518#30332#31080#34399#30908#36664#20837
  ClientHeight = 300
  ClientWidth = 666
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 666
    Height = 300
    Align = alClient
    TabOrder = 0
    object Panel2: TPanel
      Left = 1
      Top = 1
      Width = 664
      Height = 38
      Align = alTop
      TabOrder = 0
      object btnAdd: TcxButton
        Left = 6
        Top = 4
        Width = 85
        Height = 27
        Caption = #26032#22686
        TabOrder = 0
        OnClick = btnAddClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
      end
      object btnEdit: TcxButton
        Left = 100
        Top = 4
        Width = 85
        Height = 27
        Caption = #20462#25913
        TabOrder = 1
        OnClick = btnEditClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
      end
      object btnCancel: TcxButton
        Left = 190
        Top = 4
        Width = 85
        Height = 27
        Cancel = True
        Caption = #21462#28040
        TabOrder = 2
        OnClick = btnCancelClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
      end
      object btnSave: TcxButton
        Left = 280
        Top = 4
        Width = 85
        Height = 27
        Caption = #23384#27284
        TabOrder = 3
        OnClick = btnSaveClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
      end
      object btnQuit: TcxButton
        Left = 371
        Top = 4
        Width = 85
        Height = 27
        Cancel = True
        Caption = #32080#26463
        TabOrder = 4
        OnClick = btnQuitClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        NumGlyphs = 2
      end
    end
    object Panel3: TPanel
      Left = 1
      Top = 39
      Width = 664
      Height = 216
      Align = alTop
      TabOrder = 1
      object Bevel1: TBevel
        Left = 0
        Top = 34
        Width = 673
        Height = 5
        Shape = bsBottomLine
      end
      object Bevel2: TBevel
        Left = -6
        Top = 123
        Width = 679
        Height = 5
        Shape = bsBottomLine
      end
      object Bevel3: TBevel
        Left = -2
        Top = 171
        Width = 677
        Height = 5
        Shape = bsBottomLine
      end
      object Bevel4: TBevel
        Left = -17
        Top = 73
        Width = 679
        Height = 5
        Shape = bsBottomLine
      end
      object cxLabel1: TcxLabel
        Left = 12
        Top = 10
        Caption = #30332#31080#20351#29992#24180
        ParentFont = False
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxLabelNotNullStyle
        Style.IsFontAssigned = True
      end
      object mskInvUseYear: TcxMaskEdit
        Left = 80
        Top = 10
        TabOrder = 1
        Width = 57
      end
      object mskSPECIALPRIZE1: TcxDBMaskEdit
        Left = 101
        Top = 95
        DataBinding.DataField = 'SPECIALPRIZE1'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.IncrementalSearch = False
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 8
        OnExit = mskSPECIALPRIZE1Exit
        Width = 100
      end
      object cboInvUseMonth: TcxComboBox
        Left = 140
        Top = 10
        Properties.Items.Strings = (
          '01'
          '03'
          '05'
          '07'
          '09'
          '11')
        Properties.OnEditValueChanged = cboInvUseMonthPropertiesEditValueChanged
        TabOrder = 2
        Text = '01'
        Width = 37
      end
      object cxLabel2: TcxLabel
        Left = 180
        Top = 10
        Caption = #26376
        ParentFont = False
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxLabelNotNullStyle
        Style.IsFontAssigned = True
      end
      object cxLabel3: TcxLabel
        Left = 214
        Top = 10
        Caption = #22519#34892#23565#29518#26085
        ParentFont = False
        Style.StyleController = dtmMain.cxEditStyle
        Style.TextColor = clBlue
      end
      object cxDBLabel1: TcxDBLabel
        Left = 284
        Top = 10
        DataBinding.DataField = 'EXECUTEDATE'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Style.StyleController = dtmMain.cxLabelStyle
        Height = 19
        Width = 161
      end
      object cxLabel4: TcxLabel
        Left = 12
        Top = 95
        Caption = #29305#29518#34399#30908
        ParentFont = False
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -13
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.TextColor = clBlue
        Style.TextStyle = [fsBold]
        Style.IsFontAssigned = True
      end
      object mskSPECIALPRIZE2: TcxDBMaskEdit
        Left = 214
        Top = 95
        DataBinding.DataField = 'SPECIALPRIZE2'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 10
        OnExit = mskSPECIALPRIZE1Exit
        Width = 100
      end
      object mskSPECIALPRIZE3: TcxDBMaskEdit
        Left = 326
        Top = 95
        DataBinding.DataField = 'SPECIALPRIZE3'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 12
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object cxLabel5: TcxLabel
        Left = 12
        Top = 139
        Caption = #38957#29518#34399#30908
        ParentFont = False
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -13
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.TextColor = clGreen
        Style.TextStyle = [fsBold]
        Style.IsFontAssigned = True
      end
      object mskFIRSTPRIZE1: TcxDBMaskEdit
        Left = 101
        Top = 139
        DataBinding.DataField = 'FIRSTPRIZE1'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskFIRSTPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 17
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskFIRSTPRIZE2: TcxDBMaskEdit
        Left = 214
        Top = 139
        DataBinding.DataField = 'FIRSTPRIZE2'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskFIRSTPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 18
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskFIRSTPRIZE3: TcxDBMaskEdit
        Left = 326
        Top = 139
        DataBinding.DataField = 'FIRSTPRIZE3'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskFIRSTPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 19
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object cxLabel6: TcxLabel
        Left = 12
        Top = 184
        Caption = #21152#38283#34399#30908
        ParentFont = False
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -13
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.TextColor = clPurple
        Style.TextStyle = [fsBold]
        Style.IsFontAssigned = True
      end
      object mskEXTRAPRIZE1: TcxDBMaskEdit
        Left = 101
        Top = 184
        DataBinding.DataField = 'EXTRAPRIZE1'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '\d?\d?\d?\d?\d?\d?\d?\d?'
        Properties.MaxLength = 0
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 22
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskEXTRAPRIZE2: TcxDBMaskEdit
        Left = 214
        Top = 184
        DataBinding.DataField = 'EXTRAPRIZE2'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '\d?\d?\d?\d?\d?\d?\d?\d?'
        Properties.MaxLength = 0
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 23
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskSPECIALPRIZE4: TcxDBMaskEdit
        Left = 436
        Top = 95
        DataBinding.DataField = 'SPECIALPRIZE4'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 14
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskSPECIALPRIZE5: TcxDBMaskEdit
        Left = 546
        Top = 95
        DataBinding.DataField = 'SPECIALPRIZE5'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 16
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskFIRSTPRIZE4: TcxDBMaskEdit
        Left = 436
        Top = 139
        DataBinding.DataField = 'FIRSTPRIZE4'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskFIRSTPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 20
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskFIRSTPRIZE5: TcxDBMaskEdit
        Left = 546
        Top = 139
        DataBinding.DataField = 'FIRSTPRIZE5'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskFIRSTPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 21
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskEXTRAPRIZE3: TcxDBMaskEdit
        Left = 326
        Top = 184
        DataBinding.DataField = 'EXTRAPRIZE3'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '\d?\d?\d?\d?\d?\d?\d?\d?'
        Properties.MaxLength = 0
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 24
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskEXTRAPRIZE4: TcxDBMaskEdit
        Left = 436
        Top = 184
        DataBinding.DataField = 'EXTRAPRIZE4'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '\d?\d?\d?\d?\d?\d?\d?\d?'
        Properties.MaxLength = 0
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 25
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object mskEXTRAPRIZE5: TcxDBMaskEdit
        Left = 546
        Top = 184
        DataBinding.DataField = 'EXTRAPRIZE5'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '\d?\d?\d?\d?\d?\d?\d?\d?'
        Properties.MaxLength = 0
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 26
        OnExit = mskSPECIALPRIZE1Exit
        Width = 101
      end
      object cxLabel9: TcxLabel
        Left = 4
        Top = 51
        Caption = #29305#21029#29518#34399#30908
        ParentFont = False
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -13
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.TextColor = clRed
        Style.TextStyle = [fsBold]
        Style.IsFontAssigned = True
      end
      object MSKUNUSUALPRIZE1: TcxDBMaskEdit
        Left = 101
        Top = 48
        DataBinding.DataField = 'UNUSUALPRIZE1'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.IncrementalSearch = False
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 3
        OnExit = mskSPECIALPRIZE1Exit
        Width = 100
      end
      object MSKUNUSUALPRIZE2: TcxDBMaskEdit
        Left = 213
        Top = 48
        DataBinding.DataField = 'UNUSUALPRIZE2'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.IncrementalSearch = False
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 4
        OnExit = mskSPECIALPRIZE1Exit
        Width = 100
      end
      object MSKUNUSUALPRIZE3: TcxDBMaskEdit
        Left = 326
        Top = 48
        DataBinding.DataField = 'UNUSUALPRIZE3'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.IncrementalSearch = False
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 5
        OnExit = mskSPECIALPRIZE1Exit
        Width = 100
      end
      object MSKUNUSUALPRIZE4: TcxDBMaskEdit
        Left = 438
        Top = 48
        DataBinding.DataField = 'UNUSUALPRIZE4'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.IncrementalSearch = False
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 6
        OnExit = mskSPECIALPRIZE1Exit
        Width = 100
      end
      object MSKUNUSUALPRIZE5: TcxDBMaskEdit
        Left = 548
        Top = 48
        DataBinding.DataField = 'UNUSUALPRIZE5'
        DataBinding.DataSource = dsMaster
        ParentFont = False
        Properties.CharCase = ecUpperCase
        Properties.IncrementalSearch = False
        Properties.MaskKind = emkRegExprEx
        Properties.EditMask = '[\d][\d][\d][\d][\d][\d][\d][\d]'
        Properties.MaxLength = 0
        Properties.OnValidate = mskSPECIALPRIZE1PropertiesValidate
        Style.Font.Charset = ANSI_CHARSET
        Style.Font.Color = clWindowText
        Style.Font.Height = -12
        Style.Font.Name = 'Tahoma'
        Style.Font.Style = []
        Style.StyleController = dtmMain.cxEditStyle
        Style.IsFontAssigned = True
        TabOrder = 7
        OnExit = mskSPECIALPRIZE1Exit
        Width = 100
      end
    end
    object lblVerifyEn: TcxDBLabel
      Left = 102
      Top = 269
      DataBinding.DataField = 'VerifyEn'
      DataBinding.DataSource = dsMaster
      ParentColor = False
      ParentFont = False
      Properties.Alignment.Horz = taLeftJustify
      Properties.Alignment.Vert = taVCenter
      Style.StyleController = dtmMain.cxLabelStyle
      Height = 24
      Width = 100
    end
    object cxLabel7: TcxLabel
      Left = 13
      Top = 271
      Caption = #23529#26680#20154#21729
      ParentFont = False
      Style.Font.Charset = ANSI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.TextColor = clBlue
      Style.TextStyle = [fsBold]
      Style.IsFontAssigned = True
    end
    object cxLabel8: TcxLabel
      Left = 214
      Top = 271
      Caption = #23529#26680#26178#38291
      ParentFont = False
      Style.Font.Charset = ANSI_CHARSET
      Style.Font.Color = clWindowText
      Style.Font.Height = -13
      Style.Font.Name = 'Tahoma'
      Style.Font.Style = []
      Style.TextColor = clBlue
      Style.TextStyle = [fsBold]
      Style.IsFontAssigned = True
    end
    object lblVerifyTime: TcxDBLabel
      Left = 289
      Top = 269
      DataBinding.DataField = 'VerifyTime'
      DataBinding.DataSource = dsMaster
      ParentColor = False
      ParentFont = False
      Properties.Alignment.Horz = taLeftJustify
      Properties.Alignment.Vert = taVCenter
      Style.StyleController = dtmMain.cxLabelStyle
      Height = 24
      Width = 142
    end
  end
  object cdsMaster: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspEinv'
    AfterScroll = cdsMasterAfterScroll
    Left = 522
    Top = 260
  end
  object dspEinv: TDataSetProvider
    DataSet = dtmMainH.adoEinv
    Options = [poAllowCommandText]
    Left = 558
    Top = 260
  end
  object dsMaster: TDataSource
    DataSet = cdsMaster
    Left = 594
    Top = 260
  end
end
