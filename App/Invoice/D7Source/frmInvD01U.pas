...
type
  TfrmInvD01 = class(TForm)
    ...
    lbl3: TLabel;
    lbl4: TLabel;
    medMainBusinessId: TMaskEdit;
    lbl5: TLabel;
    medeFormat: TMaskEdit;
    lbl6: TLabel;
    medEformat2: TMaskEdit;
    lbl7: TLabel;
    medInvoiceType: TMaskEdit;
    lbl8: TLabel;
    medAESKey: TMaskEdit;
    lbl9: TLabel;
    medupdlimtDate: TMaskEdit;
...
begin
    ...
    begin
      ...
      FindField( 'AESKey' ).Visible := False;
      FindField( 'UPDLIMTDATE' ).Visible := False;
...
begin
  ...
  begin
    ...
    begin
      ...
      {#6791 增加總公司統一編號 By Kin 2014/05/28}
      medMainBusinessId.Text := FieldByName('MainBusinessId').AsString;
      ...
      //7152 ADD updlimtdate field by kin 2016/02/02
      medupdlimtDate.Text := FieldByName('UpdlimtDate').AsString;
      ...
      {#6966 增加eFormat,eFormat2欄位 By Kin 2014/12/18}
      medeFormat.Text := ( FieldByName( 'EFORMAT' ).AsString );
      medEformat2.Text := ( FieldByName( 'EFORMAT2' ).AsString );
      if ( FieldByName( 'InvoiceType' ).AsString <> EmptyStr ) then
        medInvoiceType.Text := FieldByName( 'InvoiceType' ).AsString
      else
        medInvoiceType.Text := EmptyStr;
      {#7076 增加AESKey By Kin 2015/08/24}
      if FieldByName( 'AESKey' ).AsString <> EmptyStr then
        medAESKey.Text := FieldByName( 'AESKey' ).AsString
      else
        medAESKey.Text := EmptyStr;
    end else
    begin
      ...
      medeFormat.Text := EmptyStr;
      {#7001 增加InvoiceType By Kin 2015/04/02}
      medInvoiceType.Text := EmptyStr;
      {#7076 增加AESKey By Kin 2015/08/24}
      medAESKey.Text := EmptyStr;
    end;
...
var
  aCompId, aCompSName, aCompName, aBusinessId,aMainBusinessId, aTaxId, aManager,
  aTel, aCompAddr, aDecaCount, aPayCount, aTaxRate, aTaxDivName,
  aTaxNum1, aTaxNum2, aNotiType, aTaxGetDate, aIsSelfCreated,
  aSQL, aServiceTypeStr,aMisOwner,aSysId,aA_CarrierType,aEFormat,aEFormat2, aInvoiceType : String;
...
begin
  ...
  begin
    ...
    aMainBusinessId := Trim(medMainBusinessId.Text);
    ...
    aUpdLimtDate := Trim( medupdlimtDate.Text );
    ...
    {#6966 增加eFormat、eFormat2 欄位 By Kin 2014/12/18}
    aEFormat := Trim( medeFormat.Text );
    aEFormat2 := Trim( medEformat2.Text );
    {#7001 增加InvoiceType 欄位 By Kin 2015/04/02}
    aInvoiceType := Trim(medInvoiceType.Text);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          {#6791 增加總公司統一編號 By Kin 2014/05/28}
          FieldByName('MainBusinessId').AsString := aMainBusinessId;
          ...
          {#7152 Add UpdlimtDate field By Kin 2015/02/02}
          FieldByName('UpdlimtDate').AsString := aUpdLimtDate;
          ...
          {#6966 增加EFORMAT、EFORMAT2 欄位 By Kin 2014/12/18}
          FieldByName( 'EFORMAT' ).AsString := aEFormat;
          FieldByName( 'EFORMAT2' ).AsString := aEFormat2;
          {#7001 增加InvoiceType 欄位 By Kin 2015/04/02}
          FieldByName( 'InvoiceType' ).AsString := aInvoiceType;
          {#7076 增加AESKey By Kin 2015/08/24}
          FieldByName( 'AESKey' ).AsString := aAESKey;
      ...
      begin
        ...
        {#7076 增加AESKey By Kin 2015/08/24}
        dtmMain.sG_AESKey :=
          ( dsrInv001.DataSet.FieldByName( 'AESKey' ).AsString );
        dtmMain.sG_UpdLimtDate :=
          ( dsrInv001.DataSet.FieldByName( 'UpdlimtDate' ).AsString );
      end;
    end;
    dtmMainJ.getAllInv001Data;
    medCompID.Enabled := True;
