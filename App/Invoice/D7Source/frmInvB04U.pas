...
type
  TfrmInvB04 = class(TForm)
    ...
    cxLabel2: TcxLabel;
    cxLabel3: TcxLabel;
    lblSelectCount: TcxLabel;
    lblDiscountAmt: TcxLabel;
    ...
    procedure gvSO034CustomDrawCell(Sender: TcxCustomGridTableView;
      ...
      var ADone: Boolean);
    procedure gvSO034SELECTEDPropertiesEditValueChanged(Sender: TObject);
  private
    ...
    function CanAllowance(aAppYearMonth :String;aInvDate :TDateTime) : Boolean;
...
uses cbUtilis, frmMainU, dtmMainU, dtmMainJU, dtmSOU, dtmReportModule,
  frmMultiSelectU, VarConv,DateUtils;
...
begin
  //設定權限
  {#6852 權限讀錯，將B3改成B4 By Kin 2014/08/19}
  dtmMain.getCompetence( 'B04', aQuery, aInsert, aDelete, aUpdate, aExecute, aVerify );
...
begin
  ...
  begin
    ...
    //#6001 自動帶入申報年月 By Kin 2011/07/13
...
begin
  ...
  {#6436 SO034增加DBLink與DbOwner By Kin 2013/03/04}
  if ( rdoMemberY.Checked ) then aMemberFlag := '1';
  Result := Format(
    ...
    '   from %s.so034  a, %s.so001 b, %s.cd019      c        ' +
...
begin
  ...
  try
    ...
    try
      ...
      FSelectCount :=0;
      FDiscountAmt :=0;
      lblSelectCount.Caption := '0';
      lblDiscountAmt.Caption := '0';
    end;
  finally
...
begin
  ...
  aChkSO034.Close;
  {#6436 SO034增加DBLink By Kin 2013/03/04}
...
begin
  ...
  try
    ...
    {#6476 判斷是否有超折(原本已修正，但程式碼又被蓋掉) By Kin 2014/01/22}
    if ( Abs( cdsSO034.FieldByName( 'REALAMT' ).AsInteger ) >
         Abs( cdsSO034.FieldByName( 'INVAMT' ).AsInteger ) ) then
    begin
      raise Exception.Create( '折讓金額 > 原發票開立金額。' );
    end;

    {#7340 make sure the realamt is negative number by Kin 2016/11/08}
    if ( cdsSO034.FieldByName('REALAMT').AsInteger > 0 ) then
    begin
      raise Exception.Create( '退費折讓金額不可>0。' );
    end;

    if (Abs(cdsSO034.FieldByName( 'REALAMT' ).AsInteger ) =
      Abs( cdsSO034.FieldByName( 'INVAMT' ).AsInteger ) ) then
    begin
      if dtmMain.sG_UpdLimtDate <> EmptyStr then
        aUpdLimtDate := dtmMain.sG_UpdLimtDate;
    ...
    if not VarIsNull( cdsSO034.FieldByName( 'INVDATE' ).Value ) then
    begin
      {#7152 測試不OK 請不用特別判斷申報年月輸入值 By Kin 2016/09/06}
      {
      if StrToDate(txtYearMonth.Text + '/'+ IntToStr( DaysInMonth(StrToDate(txtYearMonth.Text+'/01')))) <
          cdsSO034.FieldByName( 'INVDATE' ).AsDateTime then
      begin
        raise Exception.Create( '申報年月 < 發票日期。');
      end; }
    end;
    {#7152 }
    if  ( dtmMain.sG_UpdLimtDate <> EmptyStr ) then
    begin
      if ( StrToInt( dtmMain.sG_UpdLimtDate ) > 0 ) then
      begin
        if not CanAllowance( txtYearMonth.Text ,cdsSO034.FieldByName( 'INVDATE' ).AsDateTime ) then
        begin
          if (Abs(cdsSO034.FieldByName( 'REALAMT' ).AsInteger ) =
                Abs( cdsSO034.FieldByName( 'INVAMT' ).AsInteger ) ) then
          begin
            raise Exception.Create( '原發票金額與折讓金額相等不可產生折讓，須作廢發票！');
          end;
        end;
      end;
    ...
    {}
    aChkDouble.Close;
    //#6001 如果UPLOADFlag = 0 而且 PAPERDATE = 畫面的日期 代表重複,直接upd By Kin 2011/07/13
    ...
    begin
      ...
      {#6423 如果TaxAmount小於1則四捨五入取0 By Kin 2013/02/08}
      if ( aWriter.FieldByName('TaxAmount').AsInteger < 1 ) then
        aWriter.FieldByName('TaxAmount').AsInteger := 0;
    ...
    try
      ...
      begin
        ...
        begin
          if aSelected then
          begin
            if not cdsSO034.FieldByName( 'SELECTED' ).AsBoolean then
            begin
              FSelectCount := FSelectCount + 1;
              FDiscountAmt := FDiscountAmt + abs( cdsSO034.FieldByName( 'RealAmt' ).AsInteger);
            end;
          end else
          begin
            if cdsSO034.FieldByName( 'SELECTED' ).AsBoolean then
            begin
              FSelectCount := FSelectCount - 1;
              FDiscountAmt := FDiscountAmt - abs( cdsSO034.FieldByName( 'RealAmt' ).AsInteger);
            end;
      ...
      lblSelectCount.Caption := IntToStr( FSelectCount );
      lblDiscountAmt.Caption := FloatToStr( FDiscountAmt );
...
procedure TfrmInvB04.gvSO034SELECTEDPropertiesEditValueChanged(
  Sender: TObject);
begin
//  ShowMessage( inttostr(gvSO034SELECTED.EditValue));
{
  if  TcxCheckBox(gvSO034SELECTED).Checked then
    ShowMessage( 'yes' );
}

  if gvSO034.Controller.EditingItem = gvSO034SELECTED then
  begin
    try
      if TcxCheckBox(Sender).Checked then
      begin
        FSelectCount := FSelectCount + 1;
        FDiscountAmt := FDiscountAmt + Abs(StrToFloat( VarToStr( gvSO034REALAMT.EditValue )));
      end else
      begin
        FDiscountAmt := FDiscountAmt - Abs(StrToFloat( VarToStr( gvSO034REALAMT.EditValue )));
        FSelectCount := FSelectCount - 1;
...
{判斷發票是否在申報期間內}
function TfrmInvB04.CanAllowance(aAppYearMonth: String;
  aInvDate: TDateTime): Boolean;
  var aAppDate,aNowDate : TDateTime;
  var aInvMonth,aNowMonth,aInvYear,aNowYear,aInvDay,aNowDay : Word;
  var aCheckDate : TDateTime;
begin
  Result := True;
  aAppDate := StrToDate(aAppYearMonth + '/'+  dtmMain.sG_UpdLimtDate);
  aNowDate := FsysDate;
  DecodeDate(aInvDate,aInvYear,aInvMonth,aInvDay);
  DecodeDate(aNowDate,aNowYear,aNowMonth,aNowDay);
  {發票日期單數月加2個月再以參數日為申報日期 例2016/01/02 申報日期為 2016/03/參數(15)}
  case aInvMonth of
    1,3,5,7,9,11:
    begin
      aInvDate := IncMonth( aInvDate , 2 );
      aInvDate := EncodeDate(YearOf(aInvDate),MonthOf(aInvDate),StrToInt(dtmMain.sG_UpdLimtDate) );
    end;
  else
    begin
       aInvDate := IncMonth( aInvDate ,1 );
       aInvDate := EncodeDate(YearOf(aInvDate),MonthOf(aInvDate),StrToInt(dtmMain.sG_UpdLimtDate) );
    end;
  end;
  {判斷當前日期是否在申報期間內,如果為單數月又超過參數日，則加2個月才是申報日 例 2016/03/16 ,則申報月為2016/5/15}
  case aNowMonth of
    1,3,5,7,9,11:
    begin
      if aNowDay > StrToInt( dtmMain.sG_UpdLimtDate ) then
      begin
        aNowDate := IncMonth( FsysDate , 2 );
      end;
      aNowDate := EncodeDate(YearOf(aNowDate),MonthOf(aNowDate),StrToInt(dtmMain.sG_UpdLimtDate) );
    end;
  else
    {
    begin
       aNowDate := IncMonth( aNowDate ,1 );
       aNowDate := EncodeDate(YearOf(aNowDate),MonthOf(aNowDate),StrToInt(dtmMain.sG_UpdLimtDate) );
    end;
    }
  end;
  if aNowDate > aInvDate then
    Result := True
  else
    Result := False;
  if ( aAppDate > aInvDate ) and ( not Result ) then
  begin
    Result := True;
  end;
