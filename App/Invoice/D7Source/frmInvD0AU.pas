...
uses
  ...
  cbAppClass, DB, ADODB, dxSkinsCore, dxSkinBlack, dxSkinBlue,
  dxSkinCaramel, dxSkinCoffee, dxSkinsDefaultPainters, cxTextEdit,
  cxControls, cxContainer, cxEdit, cxDBEdit;
...
type
  TfrmInvD0A = class(TForm)
    ...
    LoadTimer: TTimer;
    lbl1: TLabel;
    txtBUSINESSID: TcxTextEdit;
    lbl2: TLabel;
    txtLoveNum: TcxTextEdit;
  ...
  case AMode of
    ...
    dmSave:
      begin
        ...
        begin
          adoInv041.FieldByName( 'Businessid' ).AsString := txtBUSINESSID.EditingText;
        end;
        adoInv041.FieldByName( 'LoveNum' ).AsString := txtLoveNum.Text;
        adoInv041.Post;
...
begin
  ...
  adoInv041.FieldByName( 'Businessid' ).DisplayLabel := '受贈單位統一編號';
  adoInv041.FieldByName( 'LoveNum' ).DisplayLabel := '愛心碼';
...
begin
  ...
  begin
    ...
    {#6371 增加統一編號 By Kin 2013/02/18}
    txtBUSINESSID.Text := adoInv041.FieldByName( 'Businessid' ).AsString;
    txtLoveNum.Text := adoInv041.FieldByName( 'LoveNum' ).AsString;
  end;
...
procedure TfrmInvD0A.txtBUSINESSIDExit(Sender: TObject);
begin
  if ( txtBUSINESSID.EditingText <> '' ) then
  begin
