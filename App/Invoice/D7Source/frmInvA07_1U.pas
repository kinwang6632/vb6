...
begin
  ...
  {#5922 增加發票種類與上傳註記 By Kin 2011/02/16}
  FSqlBody := Format(
    ...
    '        A.OBSOLETEREASON,                                  ' +
    '        A.SOURCE,                                          ' +
    '        DECODE(NVL(C.INVOICEKIND,0),0,''電子計算機發票'',  ' +
    '               ''電子發票'') INVOICEKIND,                  ' +
    '        DECODE(NVL(A.UPLOADFLAG,0),0,''否'',               ' +
    '               ''是'') UPLOADFLAG,                         ' +
...
begin
  ...
  cdsMaster.AfterScroll := nil;
  {#5922 增加上傳註記、發票種類 By Kin 2011/02/16}
  try
...
begin
  {#6262 增加清除發票資料 By Kin 2012/07/06}
  Screen.Cursor := crSQLWait;
  try
    ...
    begin
      ...
      begin
        EmptyInvoiceToSO;
        MessageDlg('清除完畢！',mtInformation,[mbOK],0);
      end else
      begin
        WarningMsg(Format('發票號碼 = %s 發票未完全折讓，無法清除[客服系統]收費資料上的發票相關資訊！',
...
begin
  ...
  try
    ...
    try
      ...
      try
        {#6206 將排序拿掉,不然Group會亂掉 By Kin 2012/01/19}
