...
type
  ...
  public
    ...
    sG_StarComBine : Boolean;
    sG_StarAllUpload : Boolean;
    sG_EInvDb : String;
    sG_EInvUser: String;
    sG_EInvPassword : String;
    sG_UseCOM : Boolean;
    sG_UploadType : Integer;
    sG_AESKey : String;
    sG_UpdLimtDate : String;
    ...
    function GetMisDBSid: String;
    function GetMisDBUserId: String;
    function GetMisDBPassword: String;
...
begin
  ...
  try
   ...
   FMisDbSid := aobj.MisDbSid;
   FMisDbUserId := aobj.MisDbOwner;
   FMisDbPassword := aobj.MisDbPassword;
   Result := True;
  except
    on E: Exception do
    begin
...
begin
  ...
  adoComm.SQL.Text :=
    ...
    '        NVL( STARTCMTVMAIL, 0 ) STARTCMTVMAIL,   ' +
    '        NVL( AutoNotify, 0 ) AutoNotify,         ' +
    '        NVL( STARTALLUPLOAD, 0 ) STARTALLUPLOAD, ' +
    '        NVL( UPLOADTYPE,0 ) UPLOADTYPE,          ' +
    '        NVL( STARTCOMBINE,0) STARTCOMBINE,       ' +
    '        AESKEY,UpdlimtDate                       ' +
    '   FROM INV001         ' +
    '  WHERE IDENTIFYID1 =  ' + QuotedStr( IDENTIFYID1 ) +
    '    AND IDENTIFYID2 =  ' + QuotedStr( IDENTIFYID2 ) +
  ...
  begin
    ...
    aObj.StarAllUpload := ( dtmMain.adoComm.FieldByName( 'STARTALLUPLOAD' ).AsInteger = 1 );
    {#6371 增加UploadType By Kin 2013/02/18}
    aObj.UploadType := (dtmMain.adoComm.FieldByName( 'UPLOADTYPE' ).AsInteger);
    aObj.StarComBine := ( dtmMain.adoComm.FieldByName( 'STARTCOMBINE' ).AsInteger = 1 );
    {#7076 增加AESKey By Kin 2015/08/24}
    aObj.AESKey := ( dtmMain.adoComm.FieldByName( 'AESKEY' ).AsString );
    aObj.UpdLimtDate := ( dtmMain.adoComm.FieldByName( 'UpdlimtDate' ).AsString );
    aList.Add( aObj );
    adoComm.Next;
  end;
  adoComm.Close;
...
function TdtmMain.GetMisDBSid: String;
begin
  Result := FMisDbSid;
end;

function TdtmMain.GetMisDBUserId: String;
begin
  Result := FMisDbUserId;
end;

function TdtmMain.GetMisDBPassword: String;
begin
  Result := FMisDbPassword;
end;
