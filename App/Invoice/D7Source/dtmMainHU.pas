...
type
  ...
  public
    ...
    function getInv014Amt(const aInvId, aCompId: string ) : Double;
...
function TdtmMainH.getInv014Amt(const aInvId,aCompId: string): Double;
 var aSQL : string;
begin
  adoInv014.Close;
  try
    aSQL := Format( 'SELECT SUM(INVAMOUNT) FROM  INV014             ' +
              ' WHERE IDENTIFYID1 = ''%s''                          ' +
              ' AND IDENTIFYID2 = %s                                ' +
              ' AND INVID = ''%s''                                  ' +
              ' AND ISOBSOLETE = ''N''                              ' +
    ...
    adoInv014.SQL.Text := aSQL;
    adoInv014.Open;
    if adoInv014.RecordCount = 0 then
      Result := 0
    else
      Result := adoInv014.Fields[0].AsFloat;
  finally
    adoInv014.Close;
  end;
