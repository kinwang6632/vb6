...
begin
  ...
  try
    ...
    {#6764 抬頭增加顯示登錄資訊 By Kin 2014/05/02}
    {#6764 測試不OK,增加顯示公司別代碼 By Kin 2014/07/14}
    Self.Caption := Format('%s <<公司代碼 : %s, %s -- %s ( %s ) >>',
...
begin
  ...
  try
    ...
    {#6764 測試不OK,要同步Form.Caption By Kin 2014/07/14}
    Self.Caption := Format('%s <<公司代碼 : %s, %s -- %s ( %s ) >>',
            [FFormCaption,dtmMain.getCompID, stcCompany.Text,
            dtmMain.getLoginUser,dtmMain.getLoginUserName]);
    stcCompany.OnChange := stcCompanyChange;
...
begin
  ...
  {#6370 判斷是否啟動StartAllUpload By Kin 2012/11/23}
  if dtmMain.sG_StarAllUpload then
    aParam[12] := '1'
  else
    aParam[12] := '0';

  {增加MisDBSid、MisDBUserId、MisDBPassword、MisOwner For Miggie By Kin 2013/01/18}
  aParam[13] := 'X';
  aParam[14] := 'X';
  aParam[15] := 'X';
  aParam[16] := 'X';
...
begin
  ...
  {#6396 增加UploadTime,因為從發票系統直接進入B08,不需要傳UploadTime 所以傳X By Kin 2012/12/18}
  aParam[16] := 'X';
