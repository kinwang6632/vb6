...
type
  ...
  TdtmMainJ = class(TDataModule)
    ...
    cdsSelectInv016SaleAmount: TIntegerField;
    cdsSelectInv016CustId: TIntegerField;
    cdsSelectInv016TaxAmount: TIntegerField;
    cdsSelectInv016InvAmount: TIntegerField;
    adoQrySelectInv017BILLIDITEMNO: TIntegerField;
    adoQrySelectInv017QUANTITY: TIntegerField;
    adoQrySelectInv017UNITPRICE: TCurrencyField;
    adoQrySelectInv017TAXAMOUNT: TCurrencyField;
    adoQrySelectInv017TOTALAMOUNT: TCurrencyField;
    adoInv045Code: TADOQuery;
    cdsChargeDetailExcelFixFlag: TStringField;
    procedure DataModuleCreate(Sender: TObject);
    procedure DataModuleDestroy(Sender: TObject);
  private
    { Private declarations }
    function getQrcodeFieldName( aFilePathName : String ): String;
  public
    { Public declarations }
    ...
    procedure getPrintInvoiceTempData(sI_SInvID,sI_EInvID,
             ...
             sI_Where,sI_GiftUnit : String;aUseAllComp,aPrintCarrier,aFilterINV043 :Boolean;
             sI_OriginalType : String );
    ...
    function getChargeData( sI_CompID, sI_UserID, sI_SInvDate, sI_EInvDate, sI_SChargeDate,
       ...
       aRptSource,aFixFlag,aUploadFlag : Integer): Boolean;
    ...
    procedure listLastInvDate(sI_CompID, sI_YearMonth:String; sI_BusinessId, sI_InvoiceKind: Integer );//列出最後已建立發票日期(畫面)
    ...
    function getAllInv045Data  : Integer;
    ...
    function assignInvoiceWithSF(nI_TotalUnit:Integer; fI_TotalSaleAmount, fI_TotalTaxAmount, fI_TotalInvAmount:double;
             ...
             nI_Order : Integer;bI_InvDateEqualChargeDate : Boolean; sI_ShowFaci,sI_StarCMTVmail,sI_FilterBusinessId,sI_InvoiceKind : Integer;
...
uses
  ...
  UdateTimeu, MaskUtils, dtmSOU, StrUtils;
...

{
      ...
      begin
        ...
        begin
          ...
          //#5945 未印要增加發票種類 By Kin 2011/03/16
          //#5945 更改規格，如果是匯入發票不要出Log,除非真的包含在匯入發票 By Kin 2011/04/15
          ...
          begin
            ...
            begin
              //尋找是否有符合匯入發票的資料 By Kin 2011/04/12
...
{ ---------------------------------------------------------------------------- }
//#5922 增加aUseGive 是使用勾選捐贈 By Kin 2011/02/10
//#5922 增加aRptSource 統計來源,如果是0 Detail要使用INV008A By Kin 2011/02/10
function TdtmMainJ.getChargeData(sI_CompID, sI_UserID, sI_SInvDate,
  ...
  aUseGive:Integer;aRptSource,aFixFlag,aUploadFlag : Integer): Boolean;
var
    ...
    sL_NvlInv028 : String;
    sL_ConditionFixFlag,sL_ConditionUploadFlag: String;
begin
  ...
  sL_CurrDateTime := TUstr.replaceStr( TUstr.replaceStr( TUstr.replaceStr(
    ...
    //發票種類 #5668 增加發票種類 By Kin 2010/06/28
    case aInvKind of
    ...
    //#5922 增加捐贈註記 By Kin 2011/02/10
    case aUseGive of
      0 :
      begin
        sL_ExcelTitle := sL_ExcelTitle + '@捐贈註記:全部';
        sL_ConditionIsUseGive := '捐贈註記:全部';
      end;
      1 :
      begin
        sL_ExcelTitle := sL_ExcelTitle + '@捐贈註記:捐贈';
        sL_ConditionIsUseGive :=  '捐贈註記:捐贈';
      end;
    else
      begin
        sL_ExcelTitle := sL_ExcelTitle + '@捐贈註記:非捐贈';
        sL_ConditionIsUseGive :=  '捐贈註記:非捐贈';
      end;
    end;
    {#6629 增加字軌屬性 By Kin 2013/11/04}
    case aFixFlag of
      1:
      begin
        sL_Where := sL_Where + ' AND NVL(A.FIXFLAG,0) = 1';
        sL_ExcelTitle := sL_ExcelTitle + '@字軌屬性:電子發票專用字軌';
        sL_ConditionFixFlag := '字軌屬性:電子發票專用字軌';
      end;
      2:
      begin
        sL_Where := sL_Where + ' AND NVL(A.FIXFLAG,0) = 0';
        sL_ExcelTitle := sL_ExcelTitle + '@字軌屬性:其它';
        sL_ConditionFixFlag := '字軌屬性:其它';
      end;
    else
      begin
        sL_ExcelTitle := sL_ExcelTitle + '@字軌屬性:全部';
        sL_ConditionFixFlag := '字軌屬性:全部';
      end;
    end;
    {#6629 增加上傳註記 By Kin 2013/11/04}
    case aUploadFlag of
      1:
      begin
        sL_Where := sL_Where + ' AND NVL(A.UPLOADFLAG,0) = 1';
        sL_ExcelTitle := sL_ExcelTitle + '@上傳註記:已上傳';
        sL_ConditionUploadFlag := '上傳註記:已上傳';
      end;
      2:
      begin
        sL_Where := sL_Where + ' AND NVL(A.UPLOADFLAG,0) = 0';
        sL_ExcelTitle := sL_ExcelTitle + '@上傳註記:未上傳';
        sL_ConditionUploadFlag := '上傳註記:未上傳';
      end;
    else
      begin
        sL_ExcelTitle := sL_ExcelTitle + '@上傳註記:全部';
        sL_ConditionUploadFlag := '上傳註記:全部';
      end;
    end;
    { 報表的條件, 顯示在報表頁頭的條件 }
    ...
    aText3 := Format( '%s,%s,%s,%s,%s',
        ...
        sL_ConditionTaxType, sL_ConditionToCreate,
               sL_ConditionIsUseGive] );
     aText4 := format('%s,%s',[ sL_ConditionFixFlag,
          sL_ConditionUploadFlag ]);
    {
    aText4 := format('%s,%s,%s',[ sL_ConditionChargeItem,
          sL_ConditionFixFlag,sL_ConditionUploadFlag ]);
    }
    {}
...
//******************************************************************************
    ...
    begin
      //#5922 增加捐贈註記,所以要多串INV028 By Kin 2011/02/10
      if aUseGive = 0 then
      begin
        sL_MasterSQL :=
...
//          rptInvC01_1.Free;
    ...
    begin
      ...
      //取出所有發票明細
      //#5922 增加 上傳註記,發票種類,捐贈註記,受贈單位 By Kin 2011/02/10
      //#6018 同一個收費項目+BillNo會導致出來只有一筆，現在加入B.SEQ避免掉(不照需求寫的將Distinct拿掉,因為串INV028會有問題) By Kin 2011/05/17
      sL_DetailSQL :=
        ...
        '        DECODE(NVL(A.UPLOADFLAG,0),0,''未上傳'',''已上傳'' ) UPLOADFLAG,          ' +
        '        DECODE(NVL(INVOICEKIND,0),0,''電子計算機發票'',''電子發票'') INVOICEKIND, ' +
        '        A.GIVEUNITDESC,DECODE(NVL(C.REFNO,0),1,''捐贈'',''非捐贈'') ISGIVE,       ' +
        '        DECODE(NVL(A.FIXFLAG,0),0,''其它'',''電子發票專用字軌'') FIXFLAG          ' +
        '   FROM INV007 A, ' + sL_DetailTable + ' B, INV028 C                              ' +
      ...
      begin
        ...
        FindField('UPLOADFLAG').DisplayLabel :='上傳註記';
        FindField('INVOICEKIND').DisplayLabel := '發票開立種類';
        FindField('ISGIVE').DisplayLabel := '捐贈註記';
        FindField('GIVEUNITDESC').DisplayLabel := '受贈單位';
        FindField('FIXFLAG').DisplayLabel := '字軌屬性';
        ...
        FindField( 'FIXFLAG').DisplayWidth := 30;
...
//            rptInvC01_2.Free;
          ...
          begin
            ...
            begin
              ...
              cdsChargeDetailExcel.FindField('FIXFLAG').AsString := FieldByName('FIXFLAG').AsString;
            ...
            cdsChargeDetailExcel.FindField('INVOICEKIND').DisplayLabel := '發票開立種類';
            cdsChargeDetailExcel.FindField('ISGIVE').DisplayLabel := '捐贈註記';
            cdsChargeDetailExcel.FindField('GIVEUNITDESC').DisplayLabel := '受贈單位';
            cdsChargeDetailExcel.FindField('FIXFLAG').DisplayLabel := '字軌屬性';
            if bI_PrintMemo1 then
            ...
            cdsChargeDetailExcel.FindField('UPLOADFLAG').DisplayLabel := '上傳註記';
            ...
            cdsChargeDetailExcel.FindField('FIXFLAG').DisplayWidth :=40;
            if not bI_PrintMemo1 then
...
begin
  ...
  {#6600 增加防偽隨機碼欄位與過濾 CarrierId1 IS NULL的資料 By Kin 2013/10/02}
  {#6629 把CarriderId1條件拿掉 By Kin 2013/10/23}
  {#6629 增加字軌屬性 By Kin 2013/11/01}
  {#6764 增加DepositMK判斷,N與NULL 才可列印 By Kin 2014/05/02}
  {#7128 Adding PRIZETYPE field into inv023 By Kin 2015/12/14}
  aSQL := Format(
    ' INSERT INTO INV023 (                                        ' +
    ...
    '    INVUSEDESC,GIVEUNITID,GIVEUNITDESC,RANDOMNUM,FIXFLAG,    ' +
    '    CREATEINVDATE,PRIZETYPE                              )   ' +
    ' SELECT                                                      ' +
    ...
    '    A.GIVEUNITID,A.GIVEUNITDESC,A.RANDOMNUM,A.FIXFLAG,       ' +
    '    A.CREATEINVDATE,A.PRIZETYPE                              ' +
    '  FROM INV007 A, %s                                          ' +
    ...
    {#6629 增加判斷是否列印客戶載具資料 By Kin 2013/10/28}
    if ( not aPrintCarrier ) then
    begin
      //aSQL :=  aSQL + ' AND A.CARRIERID1 IS NULL   ';
      aSQL := aSQL + ' AND (A.CARRIERID1 IS NULL             ' +
              ' OR A.CARRIERTYPE IN ( SELECT CODENO FROM INV044 WHERE NVL(REFNO,0)= 0)  ' +
              ' OR A.CARRIERTYPE NOT IN (SELECT CODENO FROM INV044)  )';
    end else
    begin
      //aSQL :=  aSQL + ' AND A.CARRIERID1 IS NOT NULL   ';
      aSQL := aSQL + ' AND (  A.CARRIERID1 IS NOT NULL       '  +
                ' AND A.CARRIERTYPE IN ( SELECT CODENO FROM INV044 WHERE NVL(REFNO,0)>=1) )';
    end;
   //營業人
  ...
  {#7128 Checking Whether invoice is original By Kin 2015/12/02 }
  if sI_OriginalType = '0' then
    aSQL := ( aSQL + ' AND A.CANMODIFY = ''Y''' )
  else if sI_OriginalType = '1' then
    aSQL := ( aSQL + ' AND A.CANMODIFY = ''N''' );
...
begin
  { 若為設定捐贈發票 }
  { #5922 AIsInvUse設成Integer 0,代表全部,1代表捐贈 2.代表非捐贈 By Kin 2011/02/10}
  if ( AIsInvUse = 1 ) then   // 過濾只剩捐贈的資料
  begin
    adoQryCommon2.Close;
    adoQryCommon2.SQL.Text := Format(
...
var
  ...
  nL_InvNumber,nL_Inv023Count,nl_OriginalNum,nl_CopyNum : Integer;
  ...
  sL_GIVEUNITDESC,sL_RandomNum,sL_RandomNum2: string;
  sL_SQLInv001 : string;
  aQRCodeFileName : String;
  ...
  aRANDOMNUM : String;
  aInvDate : TDateTime;
  sL_CreateInvDate,sL_EFormat : String;
  tt_InvDate, tt_CompId, tt_CompName, tt_BusinessId, tt_CompAddr : String;
  tt_Result : String;
begin

  {#4370 多增加Memo1與Memo2 By Kin 2009/02/19}
  {#6001 增加受贈單位名稱 By Kin 2011/07/13}
  {#6600 增加防偽隨機碼 By Kin 2013/10/02}
  {#6629 增加字軌屬性 By Kin 2013/11/01}
  {#6966 增加抓取Inv001.EFormat 欄位 By Kin 2014/12/18}
  {#7076 There have to be produced QRCodeData By Kin 2015/09/04}
  {#7144 To Add CREATEINVDATE Field By Kin 2015/11/17 }
  {#7128}
  sL_SQL := Format(
    ' SELECT A.INVDATE,             ' +
    '        A.INVID,               ' +
    ...
    '        A.RANDOMNUM,           ' +
    '        A.CREATEINVDATE,       ' +
    '        NVL(A.CANMODIFY,''N'') CANMODIFY,                                     ' +
    '        NVL(A.FIXFLAG,0) FIXFLAG,                                             ' +
    ' (SELECT COMPSNAME FROM INV001 WHERE COMPID = A.COMPID ) AS COMPSNAME,        ' +
    ' (SELECT EFORMAT FROM INV001 WHERE COMPID = A.COMPID ) AS EFORMAT,            ' +
  ...
  {#6966 增加判斷EFormat 欄位 By Kin 2014/12/18}
  sL_SQLInv001 := Format( 'SELECT EFORMAT                 ' +
                          ' FROM INV001 A                 ' +
                          '  WHERE A.IDENTIFYID1 = ''%s'' ' +
                          '    AND A.IDENTIFYID2 = ''%s'' ' +
                          '    AND A.COMPID = ''%s''      ',
  ...
  aQRCodeFileName := getQrcodeFieldName( aFilePathName );
  ...
  begin
    ...
    begin
      try
      {#7076 It is Writing QrCode Text on File for each invoice Number By Kin 2015/09/07}
      if  ( sL_InvID <> FieldByName('INVID').AsString ) and ( sL_InvID <> '' ) then
      begin
        {-------------------------------------------------------------}
        {#7128 Adding fields value into textfile By Kin 2015/12/14}
        tt_CompId := FieldByName( 'COMPID' ).AsString;
        tt_CompName := FieldByName( 'COMPNAME' ).AsString;
        tt_BusinessId := FieldByName( 'COMPBUSINESSID' ).AsString;
        tt_CompAddr := FieldByName( 'COMPADDR' ).AsString;
        tt_Result :='TT' + adjString(12,tt_InvDate,False,False) +
                        adjString(6,tt_CompId,False,False) +
                        adjString(50,tt_CompName,False,False) +
                        adjString(8,tt_BusinessId,False,False ) +
                        adjString(60,tt_CompAddr,False,False);
        {-------------------------------------------------------------}
        aQRCode1 := '';
        aQRCode2 := '';
        aCode39Data := '';
        PrepareQRCodeData(sL_InvID, dtmMain.sG_AESKey,aQRCode1,aQRCode2);
        sL_TxtRecord := adjString(6,#32,True,False) + sL_InvID;
        aQRCode1 := 'Q1' + aQRCode1;
        aQRCode2 := 'Q2' + aQRCode2;
        aCode39Data := dtmMainJ.GetCode39Data( aInvDate ,
                           sL_InvID,
                           aRANDOMNUM);

        aCode39Data := 'LL' + aCode39Data;
        L_TxtList.Add( sL_TxtRecord + tt_Result );
        L_TxtList.Add( sL_TxtRecord + aCode39Data );
        L_TxtList.Add(sL_TxtRecord + aQRCode1 );
        L_TxtList.Add(sL_TxtRecord + aQRCode2 );
      end;
      aInvDate := FieldByName('INVDATE').AsDateTime;
      sL_InvDate := getYearMonthDay7(FieldByName('INVDATE').AsDateTime,0);
      {#7144 To Add CreateInvDate of item to 36 By Kin 2015/11/17 }

      sL_CreateInvDate := Rpad ( FormatDateTime('yyyy-mm-dd',FieldByName('INVDATE').AsDateTime) + #32 +
                            FormatDateTime('hh:mm:ss',FieldByName('CREATEINVDATE').AsDateTime),20, #32);
      {#7128 }
      tt_InvDate := GetTwInvDate( FieldByName( 'INVDATE' ).AsDateTime );
      ...
      {#5604 如果 BUSINESSID=空值 項次7(單價)要改抓INV008.TotalAmount/Inv008.Quantity(小數點取兩位)
         項次9(稅額)都填入0　By Kin 2010/03/25 }
      {#5764 判斷非營業人是否列印0稅率,如果否,直接抓unitprice By Kin 2010/09/10}
      ...
      begin
        ...
        //#5604 原本是填10個0,現在改為10個空白 By Kin 2010/04/01
        //#5764 判斷非營業人是否列印0稅率 ,如果否則跑 else那段 By Kin 2010/09/10
        if aPrintZeroTax then
      ...
      {5604 項次8(銷售額)要改抓INV008.TotalAmount By Kin 2010/03/25}
      {5764 判斷非營業人參數是否列印0稅率,如果否則跑else那段 By Kin 2010/09/10 }
...
//        sL_SaleAmount := adjString(10, FloatToStr(dL_SaleAmount),true,true);
      ...
      {#4370 增加Memo1與Memo2 By Kin 2009/02/19}
//      sL_Memo1 :=adjString( 60,FieldByName('Memo1').AsString,False,False );
//      sL_Memo2 :=adjString( 60,FieldByName('Memo2').AsString,False,False );
        sL_Memo1 := Rpad( LeftBStr(FieldByName('Memo1').asString,60), 60, #32) ;
        sL_Memo2 := Rpad( LeftBStr(FieldByName('Memo2').asString,60), 60, #32) ;
      ...
      if sL_Date<>  '' then
        ...
        sL_ShowDate :=adjString(12,'',False,False); { #4421 如果是空白要補足20空白 By Kin 2009/03/11 }
      ...
      {}
      { #4421 一定要補足20 Bytes By Kin 2009/03/11 }
      ...
      {#5604  項次22要填 Inv007.MainSaleAmount 、
        項次23主發票稅額要填成0 By Kin 2010/03/25 }
      {#5764 判斷非營人列印0稅率,如果否則跑 else 那段 By Kin 2010/09/10}
      {#7701 Cancel the condition of aPrintZeroTax By Kin 2017/02/13 }
      if (FieldByName('BUSINESSID').AsString = '')  then
      begin
        aMainSaleAmount := Lpad( FieldByName( 'MAININVAMOUNT' ).AsString, 10, '0' );
        //#5604 改成填滿10個空白 By Kin 2010/04/01
        aMainTaxAmount := adjString( 10,'',True,False );
        {#7144 To add eformat value }
        sL_EFormat := Rpad('',7,#32);
      end else
      begin
        aMainSaleAmount := Lpad( FieldByName( 'MAINSALEAMOUNT' ).AsString, 10, '0' );
        aMainTaxAmount := Lpad( FieldByName( 'MAINTAXAMOUNT' ).AsString, 10, '0' );
        {#7144 To add eformat value }
        sL_EFormat := Rpad('格式 ' + FieldByName( 'EFORMAT' ).AsString ,7,#32);
      end;
      ...
      { 帳號/卡號 }
      {#7076 Use Creditcard4d data first By Kin 2015/10/23}
      if FieldByName( 'Creditcard4d' ).AsString = '' then
      begin
        aAccountNo := dtmMainJ.GetAccountNo( sL_InvID, sL_Seq, EmptyStr, EmptyStr );
      end else
      begin
        aAccountNo := FieldByName( 'Creditcard4d' ).AsString;
      end;
...
//      if Trim( aAccountNo ) <> EmptyStr then aAccountNo := ( '帳號:' + aAccountNo );
      {#7076 It prints 4 length numbers   By Kin 2015/10/29}
      if Trim( aAccountNo ) <> EmptyStr then
      begin
        aAccountNo := RightStr(aAccountNo,4);
      end;
      ...
      {#6600 增加防偽隨機碼 By Kin 2013/10/03}
      {#6629 判斷是否為營業人,印出隨機碼或21 By Kin 2013/11/01}
      sL_RandomNum := EmptyStr;
      if FieldByName( 'FixFlag' ).AsInteger = 1 then
      begin
        if (FieldByName('BUSINESSID').AsString = '') then
        begin
          sL_RandomNum := '電子發票隨機碼:' + adjString(4,FieldByName('RandomNum').AsString,false,false) + #32;
        end else
        begin
          if FieldByName( 'EFORMAT' ).AsString = '' then
            sL_RandomNum := '電子發票格式代號:21' + #32
          else
            sL_RandomNum := '電子發票格式代號:' + adjString(2,FieldByName('EFORMAT').AsString,false,false) + #32;

        end;
      end;
      aRANDOMNUM := FieldByName('RandomNum').AsString;
      {#7144}
      sL_RandomNum2 := Rpad( '隨機碼 ' + FieldByName('RandomNum').AsString , 11, #32);
      {#7076 Add CanModify Field Data into file By Kin 2015/10/23}
      sL_CanModify := FieldByName( 'CANMODIFY' ).AsString;
      //#7076 Add Inv001.CompSName Field Data By Kin 2015/10/23
      //#7144 COMPSNAME field length change to be 30 By Kin 2015/11/17
      sl_CompSName := Rpad( LeftBStr(FieldByName('COMPSNAME').asString,30), 30, #32) ;
      ...
      //#4348 項次1與18變成空白,將項次1與18移至28與29並以民國100年方式顯示 By Kin 2009/02/04
      //#4370 增加項次30與31(Memo1,Memo2) By Kin 2009/02/19
      //#6001 增加贈送單位名稱 By Kin 2011/07/13
      //#6600 增加防偽隨機碼 By Kin 2013/10/02
      //#7076 Add CanModify Field Data By Kin 2015/10/23
      //#7076 Add Inv001.CompSName Field Data By Kin 2015/10/23
      //#7144 To Add
      sL_TxtRecord := adjString(6,#32,True,False) + sL_InvID + sL_CheckNo + sL_BusinessID +
        ...
        sL_ShowDate + sL_Memo1 + sL_Memo2 + sL_GIVEUNITDESC + sL_RandomNum +
        sL_CanModify + sl_CompSName + sL_CreateInvDate + sL_RandomNum2 + sL_EFormat;
    ...
    {#7076 Finally must be recorded in the last invoice By Kin 2015/09/07}
    if (sL_InvID <> '' )  then
    begin
      adoQryTrans2Txt.Last;
     {-------------------------------------------------------------}
      {#7128 Adding fields value into textfile By Kin 2015/12/14}
      tt_CompId := FieldByName( 'COMPID' ).AsString;
      tt_CompName := FieldByName( 'COMPNAME' ).AsString;
      ...
      aQRCode1 := 'Q1' + aQRCode1;
      aQRCode2 := 'Q2' + aQRCode2;
      aCode39Data := dtmMainJ.GetCode39Data(FieldByName('INVDATE').AsDateTime,
                            FieldByName('INVID').AsString,
                            FieldByName('RANDOMNUM').AsString);
      aCode39Data := 'LL' + aCode39Data;
      {#7128}
      L_TxtList.Add( sL_TxtRecord + tt_Result );
      L_TxtList.Add( sL_TxtRecord + aCode39Data );
...
//    L_TxtQRCodeList.SaveToFile( aQRCodeFileName );
    L_TxtList.Free;
//    L_TxtQRCodeList.Free;
  ...
  //LOG檔的部份
    ...
    {----------------------------------------------------------------------------}
    {#7128 Count how many originals and copys are in the result By Kin 2015/12/02}
    {Original}
    sL_SQL := Format(
      ' SELECT COUNT(1) COUNT FROM INV023 WHERE IDENTIFYID1 = ''%s''        ' +
      '   AND IDENTIFYID2 = ''%s'' AND COMPID = ''%s'' AND OWNER = ''%s''   ' +
      ...
      '   AND CANMODIFY = ''%s''                                            ',
      [IDENTIFYID1, IDENTIFYID2, dtmMain.getCompID,
       dtmMain.getLoginUserName, GetIfPrintCheckValue,'Y'] );
    if ( aHowToCreate = 1 ) then //表選了不含發票現場開立
    begin
      sL_SQL := ( sL_SQL + ' AND HOWTOCREATE <> ''3'' ' );
    end;
    nl_OriginalNum := StrToInt( OpenSQL( sL_SQL ) );
    {Copy}
    sL_SQL := Format(
      ' SELECT COUNT(1) COUNT FROM INV023 WHERE IDENTIFYID1 = ''%s''        ' +
      '   AND IDENTIFYID2 = ''%s'' AND COMPID = ''%s'' AND OWNER = ''%s''   ' +
      ...
      '   AND CANMODIFY = ''%s''                                            ',
      [IDENTIFYID1, IDENTIFYID2, dtmMain.getCompID,
       dtmMain.getLoginUserName, GetIfPrintCheckValue,'N'] );
    if ( aHowToCreate = 1 ) then //表選了不含發票現場開立
    begin
      sL_SQL := ( sL_SQL + ' AND HOWTOCREATE <> ''3'' ' );
    end;
    nl_CopyNum := StrToInt( OpenSQL( sL_SQL ) );
    {---------------------------------------------------------------------------}
    {#7128 Adding amount of original and copy recording By Kin 2015/12/14}
    L_LogList.Add( '發票張數:' + IntToStr( nL_Inv023Count ) );
    L_LogList.Add( '正本張數:' + IntToStr( nl_OriginalNum ) );
    L_LogList.Add( '副本張數:' + IntToStr( nl_CopyNum ) );
...
begin
  { #5668 要把INVOICEKIND = 1 改成 0 By Kin 2010/06/29  }
  { #5764 增加記錄列印人員 By Kin 2010/09/13}
  aSql:=
    ' UPDATE INV007                                  ' +
    '    SET CANMODIFY = ''N'',                      ' +
...
begin
    ...
    else if sI_QueryType = QUERY_TYPE_INVAMOUNT then
    begin
      sL_Where := sL_Where + ' AND INVAMOUNT >= ' + sI_CustIDOrBillIDCondition + ' AND INVAMOUNT <= ' + sI_CustIDOrBillIDCondition2;
    end
...
procedure TdtmMainJ.listLastInvDate(sI_CompID, sI_YearMonth: String; sI_BusinessId,sI_InvoiceKind : Integer);
var
    sL_SQL,sL_Where : String ;
begin
    Inv099DataSet.Close ;
    ...
    begin
      Close;
      SQL.Clear ;
      {#6370 增加判斷是否為營業人,Where條件增加Upload By Kin 2012/11/22}
      sL_Where := EmptyStr;
      case sI_BusinessId of
        1 : sL_Where := ' AND NVL(A.UPLOADFLAG,0) = 0 ' ;
        2 : sL_Where := ' AND NVL(A.UPLOADFLAG,0) = 1 ' ;
      end;
      {#6370 將營業人的條件取消 By Kin 2012/12/05}
      sL_Where := EmptyStr;
      {#6370 增加判斷是發票種類 By Kin 2012/12/05}
      case sI_InvoiceKind of
        0: sL_Where := sL_Where + ' AND NVL(A.UPLOADFLAG,0) = 0 ';
        1: sL_Where := sL_Where + ' AND NVL(A.UPLOADFLAG,0) = 1 ';
      end;

      if sI_YearMonth='' then
        sL_SQL := 'SELECT A.IDENTIFYID1,A.IDENTIFYID2,A.COMPID,A.STARTNUM,A.PREFIX, A.LASTINVDATE,A.YEARMONTH,A.CURNUM,A.MEMO, ' +
                  'DECODE(NVL(A.UPLOADFLAG,0),0,''否'',''是'') UPLOADFLAG                                                      ' +
                  'FROM INV099 A, INV001 B'+
                  ' WHERE A.COMPID = :COMPID AND A.COMPID=B.COMPID AND INVFORMAT=1 AND USEFUL=''Y''' + sL_Where + ' ORDER BY A.YEARMONTH DESC '
      else
        sL_SQL := 'SELECT A.IDENTIFYID1,A.IDENTIFYID2,A.COMPID,A.STARTNUM,A.PREFIX, A.LASTINVDATE,A.YEARMONTH,A.CURNUM,A.MEMO,  ' +
                  'DECODE(NVL(A.UPLOADFLAG,0),0,''否'',''是'') UPLOADFLAG                                                       ' +
                  'FROM INV099 A, INV001 B'+
                ' WHERE A.COMPID = :COMPID AND A.COMPID=B.COMPID AND INVFORMAT=1 AND USEFUL=''Y''' +
                ' AND YEARMONTH=' + '''' + sI_YearMonth + '''' + sL_Where + ' ORDER BY A.YEARMONTH DESC ';
...
begin
    ...
    Case bI_Select of
       ...
       3:
          ...
          sL_Where := sL_Where + ' and Exists (Select * From inv019 where inv002.compid = inv019.compid ' +
                                  ' and inv002.custid = inv019.custid                                   ' +
                                  ' and inv019.businessid = ' + QuotedStr( sI_Sno ) + ')                ';
...
begin
  ...
  aConnection.KeepConnection := False;
  aConnection.LoginPrompt := False;
  aConnection.ConnectionString := dtmMain.InvConnection.ConnectionString;
  aConnection.Connected := True;
//  adoQryCommon.Connection := aConnection;

  spAssignInvID.Free;


  spAssignInvID := TADOStoredProc.Create( self );
  spAssignInvID.Connection := dtmMain.InvConnection;
  }

  if not dtmMain.InvConnection.Connected then
    dtmMain.InvConnection.Connected := True;
  adoQryCommon.Close;
  adoQryCommon.SQL.CommaText :=  format('delete inv031 where UptEn = ''%s'' ',
                                                   [sI_UptEn]);
  adoQryCommon.ExecSQL;
  adoQryCommon.Close;
  ...
  try
    ...
    //#6080 增加StarCMTVMail 參數 By Kin 2011/0823
    spAssignInvID.Parameters.ParamByName('P_STARCMTVMAIL').Value := sI_StarCMTVmail;
    {#6370 增加判斷是否要過濾營業別 By Kin 2012/11/23}
    spAssignInvID.Parameters.ParamByName('p_FilterBusinessId' ).Value := sI_FilterBusinessId;
    {#6370 增加判斷發票種類 By Kin 2012/12/05}
    spAssignInvID.Parameters.ParamByName( 'p_FilterInvoiceKind' ).Value := sI_InvoiceKind;
  except
    on E: Exception do
    begin
  ...
  try
    ...
    begin
      ...
      {----------------------------------------------------------------------------}
      {#5945 增加電子發票與非電子發票的統計 By Kin 2011/03/15}
    ...
    spAssignInvID.Connection.Close;
    {
    aConnection.KeepConnection := False;
    aConnection.Close;
    ...
    {將Connection關掉再重開,否則再開立一次會出現錯誤 By Kin 2013/12/13}
//    dtmMain.InvConnection.Close;
...
//  cdsQryInsertInv017.FieldByName('TAXRATE').AsInteger := StrToInt(sI_MasterTaxRate);
  //遇到小數點程式會出錯,現在改為AsString By Kin 2011/03/01
...
begin
  ...
  aSQL := ' SELECT IDENTIFYID1,                                               ' +
            ...
            '        UPLOADFLAG,                                              ' +
            '        USEKIND,                                                 ' +
            '   Decode(NVL(UPLOADFLAG,0),0,''否'',''是'') UPLOADFLAG2,        ' +
            '   Decode(NVL(USEKIND,0),0,''整批開立'',''櫃台開立'') USEKIND2   ' +
            '   FROM INV099               ' +
...
//    end;
    ...
    begin
      if ( dtmMain.GetLinkToMIS ) then
      begin
        {#6871 不檢查 preinvoice = 3 的資料 By Kin 2014/09/30}
        ...
        {}
        {Cancel this checking by kin 2016/02/02}
        aSoReader.Close;
        aSoReader.SQL.Text := Format(
          ' select count(1) from so034  ' +
          '  where guino = ''%s''       ' +
      ...
      { 換檢查發票系統內的折讓資料 }
      { #6823 多增加判斷折讓單是否作廢 By Kin 2014/07/08}
      adoQryCommon.Close;
      adoQryCommon.SQL.Text := Format(
        ...
        '    and IsObsolete = ''%s''      ' +
        '    and invid = ''%s''          ',
        [IDENTIFYID1, IDENTIFYID2, dtmMain.getCompID,'N', aParam.InStr1] );
      adoQryCommon.Open;
...
begin
  ...
  begin
    ...
    {#5363 有可能沒有使用CC&B所以BillID與BillIDitemNo如果是Null,要塞一筆空值資料 By Kin 2009/10/27}
    adoQryCommon.Close;
    adoQryCommon.SQL.Text := Format(
      ' INSERT INTO INV024 (                                        ' +
...
begin
  ...
  adoInv028Code.Open;
  { 有捐贈 }
  { #5922 捐贈註記改為用Integer型態 0.全部 1.捐贈 2.非捐贈 By Kin 2011/02/11}
  if ( AIsInvUse = 1 ) then
  begin
...
var
  ...
  aServiceTypeStr : String;
begin
  Result := True;
  aSoDataReader := dtmSO.adoQrySynCD019;
  aWriter := dtmMain.adoComm2;
  {}
  aServiceTypeStr := dtmMain.GetServiceTypeStrEx;
  if aServiceTypeStr <> EmptyStr then
  begin

  end;
  //#7152 Add servicetype conditions by kin 2016/02/02
  aSoDataReader.Close;
  aSoDataReader.SQL.Text := Format(
    ...
    '   FROM %s.CD019 A, %s.CD033 B          ' +
    '  WHERE A.TAXCODE = B.CODENO            ' +
    '    AND A.SERVICETYPE IN ( %s )         ' +
    '    AND B.TAXFLAG = 1                   ' +
    '  ORDER BY A.CODENO ', [dtmMain.getMisDbOwner,
...
var aWhere : String;
begin
  //#5966 增加發票捐贈內容 By Kin 2011/03/29
  aWhere := EmptyStr;
  adoInv040Code.Close;
...
begin
  ...
  begin
    try
      ...
      aEffects := aWriter.Fields[0].AsInteger;
      aWriter.Close;
      {#6371 增加統一編號 By Kin 2013/02/18}
      {#6600 增加愛心碼 By Kin 2013/10/01}
      if ( aEffects > 0 ) then
      begin
        aWriter.SQL.Text := Format(
          ' UPDATE INV041                 ' +
          ...
          [aSoDataReader.FieldByName( 'Description' ).AsString,
          aSoDataReader.FieldByName( 'RefNo' ).AsString,
          dtmMain.getLoginUser, aSoDataReader.FieldByName( 'Businessid' ).AsString,
          aSoDataReader.FieldByName( 'LoveNum' ).AsString,
          IDENTIFYID1, IDENTIFYID2, dtmMain.getCompID,
          aSoDataReader.FieldByName( 'CODENO' ).AsString] );
        aWriter.ExecSQL;
      end else
      begin
        aWriter.SQL.Text := Format(
          ...
          '    COMPID, ITEMID, DESCRIPTION, REFNO, UPDEN,   ' +
          '    UPDTIME, ISSELFCREATED, LOVENUM, BUSINESSID )' +
          '  VALUES ( ''%s'', ''%s'', ''%s'', ''%s'',       ' +
          '           ''%s'', ''%s'', ''%s'',               ' +
          '           SYSDATE,                              ' +
          '           ''N'',''%s'',''%s'' )                 ',
          [IDENTIFYID1, IDENTIFYID2, dtmMain.getCompID,
           ...
           dtmMain.getLoginUser,
           aSoDataReader.FieldByName( 'LOVENUM' ).AsString,
           aSoDataReader.FieldByName( 'Businessid' ).AsString] );
        aWriter.ExecSQL;
      end;
    except
      on E: Exception do
      begin
        ErrorMsg( Format( '同步受贈單位代碼失敗, 訊息:%s。', [E.Message] ) );
        Break;
      end;
    end;
    aSoDataReader.Next;
...
begin
  ...
  {#6371 測試不OK 將INVOICEKIND = 1 條件取消 By Kin 2013/11/06}
  ...
  {#6262 增加作廢上傳註記與作廢上傳時間 By Kin 2012/07/27}
  aSql := Format( '  UPDATE  INV007                    ' +
                  '     SET UPLOADFLAG = 0,            ' +
                  '     UPLOADTIME = NULL,             ' +
                  '     OBUPLOADFLAG = 0,              ' +
  ...
  adoQryCommon.SQL.Clear;
  adoQryCommon.SQL.Text := aSql;
  aRunCnt := aRunCnt + adoQryCommon.ExecSQL;
  if aRunCnt = 0 then
  begin
    Result := '無任何重新授權資料！';
  end else
  begin
    Result := Format('已重新授權 %d 筆資料！',[aRunCnt]);
  end;
...
function TdtmMainJ.AuthorizeDiscountInv(const aDiscDateSt,
  ...
  aRunCnt : Integer;
begin
  {#6371 測試不OK 將 B.INVOICEKIND = 1 條件取消 By Kin 2013/11/06}
  ...
  {#6001 增加折讓單號起迄 By Kin 2011/07/13}
  ...
  {#6629 測試不OK,將BUSINESSID Is Null 取消 By Kin 2013/12/03}
...
begin
  ...
  try
    if ( aParam.OutStr1 = EmptyStr ) then
    begin
      aParam.Msg := Format( '折讓檔內查無此張折讓單號:%s。', [aParam.InStr1] );
      Exit;
    end;
    if ( aParam.InStr2 <> EmptyStr ) and ( aParam.OutStr2 = 'Y' ) then
    begin
      aParam.Msg := Format( '此張折讓單:%s, 已被作廢,不可再被作廢。',
        [aParam.InStr1] );
      Exit;
    end else
    if ( aParam.InStr2 = EmptyStr ) and ( Nvl( aParam.OutStr2, 'N' ) = 'N' ) then
    begin
      aParam.Msg := Format( '此張折讓單:%s, 尚未作廢,不可回復。',
        [aParam.InStr1] );
      Exit;
    end;
    if (aParam.InStr2 <> EmptyStr ) and ( aParam.OutStr3 <> '1' ) then
    begin
      aParam.Msg := Format( '此張折讓單:%s, 尚未上傳,不可被作廢。',
        [aParam.InStr1] );
      Exit;
    end;
...
{#6371 增加客戶載具類別代碼 By Kin 2013/02/25}
procedure TdtmMainJ.SynchronizeCD122;
var
  aSoDataReader: TADOQuery;
  aWriter: TADOQuery;
  aEffects: Integer;
begin
  ...
  {}
  aSoDataReader.Close;
  aSoDataReader.SQL.Text := ' SELECT * FROM CD122 WHERE STOPFLAG <> 1 ORDER BY CodeNo ';
  aSoDataReader.Open;
  aSoDataReader.First;
  while not aSoDataReader.Eof do
  begin
    try
      ...
      begin
        aWriter.SQL.Text := Format(
          ...
          [aSoDataReader.FieldByName( 'Description' ).AsString,
          aSoDataReader.FieldByName( 'RefNo' ).AsString,
          dtmMain.getLoginUser, dtmMain.getCompID,
          aSoDataReader.FieldByName( 'CODENO' ).AsString] );
        aWriter.ExecSQL;
      end else
      begin
        aWriter.SQL.Text := Format(
          ' INSERT INTO INV044 ( COMPID, CODENO, DESCRIPTION,             ' +
          '                      REFNO, UPDEN,UPDTIME )                   ' +
      ...
      on E: Exception do
      begin
        ErrorMsg( Format( '同步客戶載具類別失敗, 訊息:%s。', [E.Message] ) );
        Break;
      end;
    end;
    aSoDataReader.Next;
...
function TdtmMainJ.getAllInv045Data: Integer;
begin
  adoInv045Code.Close;
  adoInv045Code.SQL.Text := ' SELECT * FROM INV045 ORDER BY ITEMID ';
  adoInv045Code.Open;
  Result := adoInv045Code.RecordCount;
end;

procedure TdtmMainJ.PrepareQRCodeData(a_InvoiceNumber, a_AESKey: String;
  var aQRCode1, aQRCode2: String);
   var
...
begin
   try
    try
      ...
      aSQL := Format('SELECT A.*,B.Description,B.Quantity,B.UnitPrice,TotalAmount,      ' +
              ' (SELECT Decode(BusinessId,Null,MainBusinessId,BusinessId) FROM INV001 WHERE COMPID = A.COMPID ) MainBusinessId    ' +
      ...
      begin
        aUnitPrice := dtmMain.adoComm2.FieldByName( 'TotalAmount' ).AsFloat / dtmMain.adoComm2.FieldByName( 'Quantity' ).AsFloat;
        aUnitPrice := CbRoundTo(aUnitPrice,0);
        //aUnitPrice := CbRoundTo(aUnitPrice,0);
        //aUnitPrice := dtmMain.adoComm2.FieldByName( 'TotalAmount' ).AsFloat;
...
function TdtmMainJ.getQrcodeFieldName(aFilePathName: String): String;
 var
   aResult : String;
begin
  aResult := ExtractFilePath(aFilePathName) + ChangeFileExt(ExtractFileName(aFilePathName),'') +
                'QRCode';                                                                                                    
  if ExtractFileExt( aFilePathName ) <> '' then
  begin
    aResult := aResult + '.' + ExtractFileExt( aFilePathName )
  end;
  Result := aResult;
end;

function TdtmMainJ.GetCode39Data(aInvDate: TDateTime; aInvId,
  aRanNum: String): String;

  var aFmtInvDate : String;
      aYear,aMonth:String;
begin
  {#7076 It has to become even number of month if Month is odd number By Kin }
  aFmtInvDate := getYearMonthDay(DateTimeToStr(aInvDate));
  aYear :=IntToStr(strtoint( copy(aFmtInvDate,1,4) )-1911);
  aMonth := copy(aFmtInvDate,5,2);
  case StrToInt( aMonth ) of
       1 : aMonth := '02' ;
       2 : aMonth := '02' ;
       3 : aMonth := '04' ;
       4 : aMonth := '04' ;
  ...
  Result := aYear + aMonth + aInvId + aRanNum; 
end;
