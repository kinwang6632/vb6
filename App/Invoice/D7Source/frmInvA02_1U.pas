...
begin
  ...
  begin
    ...
    begin
      ...
      if FResultDataSet.FieldByName( 'ACCOUNTNO' ).AsString <> EmptyStr then
      begin
        if not dtmSO.IsUseCreditCard( FResultDataSet.FieldByName( 'CUSTID' ).AsString,
                              FResultDataSet.FieldByName( 'ACCOUNTNO').AsString ) then
        begin
          FResultDataSet.FieldByName( 'ACCOUNTNO' ).AsString := '';
          FResultDataSet.FieldByName( 'CardLastNo' ).AsString := '';
        end;
      end;
      FResultDataSet.Post;
  ...
  FLoveNum := dtmSO.GetLoveNum(FTitleDataSet.FieldByName( 'INVSEQNO' ).AsString,
                FTitleDataSet.FieldByName('CUSTID' ).AsString,
                FTitleDataSet.FieldByName('SERVICETYPE' ).AsString,);
  {#7076 以SO033或SO034的資料為主填入會員載具資訊 By Kin 2015/08/20}                
  FResultDataSet.First;
  while Not FResultDataSet.Eof do
  begin
    if FResultDataSet.FieldByName('CarrierId1').AsString <> '' then
    begin
      FCarrierId1 := FResultDataSet.FieldByName('CarrierId1').AsString;
    end;
    if FResultDataSet.FieldByName('CarrierTypeCode').AsString <> '' then
    begin
      FCarrierType := FResultDataSet.FieldByName('CarrierTypeCode').AsString;
    end;
    if FResultDataSet.FieldByName('LoveNum').AsString <> '' then
    begin
      FLoveNum := FResultDataSet.FieldByName('LoveNum').AsString;
    end;
    FResultDataSet.Next;
  end;
  FResultDataSet.First;
