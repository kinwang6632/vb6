...
uses
  ...
  dxSkinCoffee, cxMaskEdit, cxDropDownEdit, cxCalendar;
...
type
  ...
  TfrmInvA08 = class(TForm)
    ...
    chkNoDiscount: TCheckBox;
    chkOnlyDiscount: TCheckBox;
    txtPaperDateEt: TcxDateEdit;
    lbl1: TLabel;
    ...
    procedure chkOnlyDiscountClick(Sender: TObject);
  private
...
begin
  ...
  if ( trim( txtPaperDateSt.Text ) <> '' ) then
  begin
    if ( Trim( txtPaperDateEt.Text ) = '' ) then
    begin
      MessageDlg('請輸入折讓截止日期!',mtError,[mbOK],0);
      exit;
    end;
  end;
...
{====================以下為<計算資料>媒體申報之計算============================}
    ...
    {#6396 銷售客總計與稅額總計順序要反過來 By Kin 2012/12/18}
        //I.	非營業人--電子(應稅)稅　額總計{銷售額 × 0.05}
    lbl_show.Caption := '非營業人--電子(應稅)稅　額總計處理';
    Refresh;
    // #5716 稅額 改成 發票額 - 銷售額 By Kin 2010/07/16
    //tL_Sum.sL_I :=  SimpleRoundTo( tL_Sum.sl_H * 0.05,0 );
    {#6396 "非營業人--電子(應稅)稅　額總計(發票額-銷售額)" -> "非營業人--電子(應稅)稅　額總計(發票額/1.05*0.05)"}
    //tL_Sum.sL_I := tL_Sum.sL_G - tL_Sum.sL_H;
    tL_Sum.sL_I := SimpleRoundTo( tL_Sum.sL_G/1.05*0.05,0);
    MediaList.Add('非營業人--電子(應稅)稅　額總計(發票額/1.05*0.05)：'+FormatFloat('###,###,###,##0',tL_Sum.sL_I));
    ...
    Refresh;
    {#6396 "非營業人--電子(應稅)銷售額總計(發票額 / 1.05)" -> 非營業人--電子(應稅)銷售額總計(發票額-稅額) By Kin 2012/12/18}
    //tL_Sum.sL_H := SimpleRoundTo( tL_Sum.sl_G / 1.05,0);
    tL_Sum.sL_H := tL_Sum.sL_G - tL_Sum.sL_I;
    MediaList.Add('非營業人--電子(應稅)銷售額總計(發票額-稅額)：'+FormatFloat('###,###,###,##0',tL_Sum.sL_H));
    ...
    Refresh;
    // #5716 稅額改成 發票銷售額 - 銷售額 By Kin 2010/07/16
    //tL_Sum.sL_R := SimpleRoundTo( tL_Sum.sL_Q * 0.05 ,0);
    {#6396 "二聯(應稅)稅　額總計(發票額-銷售額)" -> "二聯(應稅)稅　額總計(發票額/1.05*0.05) By Kin 2012/12/18}
    //tL_Sum.sL_R := tL_Sum.sL_P - tL_Sum.sL_Q;
    tL_Sum.sL_R := SimpleRoundTo(tL_Sum.sL_P/1.05*0.05,0);
    MediaList.Add('二聯(應稅)稅　額總計(發票額/1.05*0.05)：'+FormatFloat('###,###,###,##0',tL_Sum.sL_R));


    //Q.	二聯(應稅)銷售額總計{發票額 / 1.05}
    lbl_show.Caption := '二聯(應稅)銷售額總計處理';
    Refresh;
    {#6396 "二聯(應稅)銷售額總計(發票額/1.05)"->"二聯(應稅)銷售額總計(發票額-稅額)" By Kin 2012/12/18}
    //tL_Sum.sL_Q := SimpleRoundTo( tL_Sum.sL_P / 1.05,0);
    tL_Sum.sL_Q := tL_Sum.sL_P - tL_Sum.sL_R;
    MediaList.Add('二聯(應稅)銷售額總計(發票額-稅額)：'+FormatFloat('###,###,###,##0',tL_Sum.sL_Q));
    ...
    Refresh;
    //#6001 增加過濾 IsObsolete= 'N'的資料 By Kin 2011/07/13
    tL_Sum.sL_S := SetMediaData( 4, 'SaleAmount','INV014', 'N','','','','1');
    ...
    Refresh;
    //#6001 增加過濾 IsObsolete= 'N'的資料 By Kin 2011/07/13
    tL_Sum.sL_T := SetMediaData(4,'TaxAmount','INV014', 'N','','','','1');
...
{=========================以下為媒體申報金額統計===============================}
    ...
    Refresh;
    //#6001 增加過濾 IsObsolete= 'N'的資料 By Kin 2011/07/13
    tL_Sum.sL_EE := SetMediaData( 5,'SaleAmount','INV014', 'N','','','','3');
    ...
    Refresh;
    //#6001 增加過濾 IsObsolete= 'N'的資料 By Kin 2011/07/13
    tL_Sum.sL_EE2 := SetMediaData( 5,'SaleAmount','INV014', 'N','','','','2' );
...
begin
  ...
  begin
    ...
    // #6001 增加判斷不要產生折讓資料 By Kin 2011/07/14
    if ( UpperCase(  dTable ) = UpperCase( 'INV014' ) ) and
      ( chkNoDiscount.Checked ) then
    begin
      sl_parma := sl_parma + ' and 1 = 0 ';
    end;
    //#7305
    if ( UpperCase(  dTable ) <> UpperCase( 'INV014' ) ) and
      ( chkOnlyDiscount.Checked ) then
    begin
      sl_parma := sl_parma + ' and 1 = 0 ';
    end;
    //#7305
    if ( chkOnlyDiscount.Checked ) and ( UpperCase( dTable ) = 'INV014' ) then
    begin
      if (txtPaperDateEt.Text <> '') then
      begin
        sl_parma := sl_parma + Format( ' AND PaperDate >= TO_DATE( ''%s'', ''YYYYMMDD'' )  ' +
                                    ' AND PaperDate <= TO_DATE( ''%s'', ''YYYYMMDD'' ) ',
                                    [FormatDateTime( 'YYYYMMDD', txtPaperDateSt.Date ),
                                      FormatDateTime( 'YYYYMMDD', txtPaperDateEt.Date )]);
      end;
...
begin
  ...
  begin
    ...
    //7305
    if ( chkOnlyDiscount.Checked ) then
    begin
      AQry_Inv.Sql.Add( ' and 1 = 0 ' );
    end;
    ...
    begin
      AQry_Inv.close;
      {#6966 取出INV001.EFORMAT2 By Kin 2014/12/18}
      AQry_Inv.Sql[0] :=Format( ' SELECT COMPID, INVFORMAT,ENDNUM,PREFIX,CURNUM,NVL(UPLOADFLAG,0) AS UPLOADFLAG,  ' +
                                '(SELECT EFORMAT2 FROM INV001                                                     ' +
                                ' WHERE IDENTIFYID1 = ''%s''                                                      ' +
                                ' AND IDENTIFYID2 = ''%s''                                                        ' +
                                ' AND COMPID = INV099.COMPID   ) AS EFORMAT2                                      ' +
                                ' FROM INV099                                                                     ',
      ...
      begin
        ...
        begin
          ...
          if AQry_Inv.FieldByName('InvFormat').AsString = '2' then
            ...
            sL_Inv007Format := '31';
          {#6966 如果UploadFlag = 1 則抓取EFormat2的值 By Kin 2014/12/18}
          if AQry_Inv.FieldByName( 'UPLOADFLAG' ).AsInteger = 1 then
          begin
            if AQry_Inv.FieldByName( 'EFORMAT2' ).AsString <> '' then
            begin
              sL_Inv007Format := AQry_Inv.FieldByName( 'EFORMAT2' ).AsString;
            end;
          end;
          //2稅籍資料 ==> P_Inv001TaxID
          //3取流水序號
          ...
          //4取發票民國年月
          // #5212 民國年要取3碼 By Kin 2009/07/28
          a3Year :=StrToInt( Copy( sL_MakeMonth, 1, 4 ) ) - 1911;
          if a3Year<100 then
          begin
            sL_Inv007InvDate := '0'+ IntToStr(StrToInt( Copy( sL_MakeMonth, 1, 4 ) ) -
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          P_RecNo := rightstr(inttostr(10000000+strtoint(P_RecNo)),7);
          //4取發票民國年月
          //#5212 民國年要變3碼 By Kin 2009/07/28
          sL_Inv007InvDate := getYearMonthDay7(fieldByName('realdate').AsDateTime,2);
          //5買受人統一編號變更為發票迄號
          sL_Inv007BusinessID := '        ';
          //6銷售人統一編號 ==> P_Inv01BusID
          //7、8發票字軌
          ...
          sL_Other  := ' ';
          {#5285 ImPortNote填入要課稅別為2的才可填，程式原本寫成3，現在改為2 By Kin 2009/09/10}
          if ( sL_Inv007TaxType = '2' ) then
  ...
  begin
    ...
    begin
      FDebugList.Add( Format( '%s: P_Select = 5 ', [FormatDateTime( 'YYYY-MM-DD HH:MM:SS', Now )] ) );
          ...
          P_RecNo := rightstr(inttostr(10000000+strtoint(P_RecNo)),7);
          //4取發票民國年月
          //#5212 民國年要改3碼 By Kin 2009/07/28
          sL_Inv007InvDate := getYearMonthDay7(StrToDate( FormatMaskText( '####/##/##;0;_', sL_MakeMonth + '01' )),2);
          //5買受人統一編號變更為發票迄號
  ...
  begin
    ...
    AQry_Inv099.SQL.Add( ' SELECT Prefix, EndNum, CurNum, StartNum,Nvl(UploadFlag,0) UploadFlag,     ' +
                          ' (SELECT EFORMAT2 FROM INV001 WHERE COMPID = INV099.COMPID) EFORMAT2      ' +
                          '  FROM INV099 ' );
    ...
    begin
      ...
      if ( chkOnlyDiscount.Checked ) then
      begin
        if ( UpperCase( P_Table ) <> 'INV014' ) then begin
          AQry_Inv.SQL.Text := AQry_Inv.SQL.Text + ' AND 1 = 0 ';
        end else
        begin
          if (txtPaperDateEt.Text <> '') then
          begin
             AQry_Inv.SQL.Text := AQry_Inv.SQL.Text +
                              Format( ' AND PaperDate >= TO_DATE( ''%s'', ''YYYYMMDD'' )  ' +
                                          ' AND PaperDate <= TO_DATE( ''%s'', ''YYYYMMDD'' ) ',
                                          [FormatDateTime( 'YYYYMMDD', txtPaperDateSt.Date ),
                                            FormatDateTime( 'YYYYMMDD', txtPaperDateEt.Date )]);
          end;
        ...
        begin
          ...
          begin
            ...
            FDebugList.Add( '********************************************************' );
              ...
              aIsMainInv := False; {#6269 判斷是否與主發票為同一張 By Kin 2012/08/03}
              { 以主發票號碼申報 }
              ...
              begin
                ...
                begin
                  aIsMainInv := True;
                  {#6269 不是主發票也要輸出資料 (aCanWriteToFile 註解起來 6269就完成了) By Kin 2012/08/03}
              ...
              begin
                ...
                //1取格式代碼
                  ...
                  {#6966 如果UploadFlag = 1 則抓取EFormat2的值 By Kin 2014/12/18}
                  {客戶反應會出錯,因為用了AQry_Inv,要改用AQry_Inv099 By Kin}
                  if AQry_Inv099.FieldByName( 'UPLOADFLAG' ).AsInteger = 1 then
                  begin
                    if AQry_Inv099.FieldByName( 'EFORMAT2' ).AsString <> '' then
                    begin
                      sL_Inv007Format := AQry_Inv099.FieldByName( 'EFORMAT2' ).AsString;
                    end;
                  end;
                  //4取發票民國年月
                  //#5212 民國年要改3碼 By Kin 2009/07/28
                  sL_Inv007InvDate := getYearMonthDay7( AQry_Inv.fieldByName('INVDATE').AsDateTime,2 );
                  //判斷發票稅別
                  ...
                  begin
                    ...
                    //#5604 銷售人統編)，原程式規格是填入8個”0”，改為填入空白(補滿空白) By Kin 2010/04/01
                    sL_Inv007BusinessID := '        ';
                    //9銷售額補0
                    sL_Inv007Amount := '000000000000';
                    //10課稅別改為D
//                    sL_Inv007TaxType := 'D';
                    //#5604 誄稅別改為F By Kin 2010/03/25
                    sL_Inv007TaxType := 'F';
                    //11營業稅額補0
                    sL_TaxAmount := '0000000000';
                    //14彙加註記填入 A
                  ...
                  begin
                    ...
                    begin
                      ...
                      //11營業稅額
                      sL_TaxAmount := '0000000000';
                      {#6269 營業稅額都補滿0，其它欄位都與主發票號碼的取得相同 By Kin 2012/08/03}
                      if aIsMainInv then
                      begin
                        sL_TaxAmount := '0000000000';
                        sL_Inv007Amount := '000000000000';
  ...
  begin
    ...
    //#6001 增加IsObsolete = 'N' Where 條件 By Kin 2011/07/13
    if ( UpperCase( P_Table ) =  UpperCase( 'INV014' )) then
    begin
      AQry_Inv.SQL.Text := AQry_Inv.SQL.Text +
            ' AND A.ISOBSOLETE = ''N'' ';
    end;
    //#6001 判斷是否要產生折讓資料 By Kin 2011/07/13
    //7305
    if ( UpperCase( P_Table ) =  UpperCase( 'INV014' )) and
          ( chkNoDiscount.Checked ) then
    begin
      AQry_Inv.SQL.Text := AQry_Inv.SQL.Text +
          ' AND 1 = 0 ';
    ...
    begin
      if ( chkOnlyDiscount.Checked ) then
      begin
        if (txtPaperDateSt.Text <> '') then
        begin
          AQry_Inv.SQL.Text  :=  AQry_Inv.SQL.Text  + Format( ' AND PaperDate >= TO_DATE( ''%s'', ''YYYYMMDD'' )  ' +
                                    ' AND PaperDate <= TO_DATE( ''%s'', ''YYYYMMDD'' ) ',
                                    [FormatDateTime( 'YYYYMMDD', txtPaperDateSt.Date ),
                                      FormatDateTime( 'YYYYMMDD', txtPaperDateEt.Date )]);
        end;
    ...
    begin
      ...
      //4取發票民國年月
      //5212 民國年要取3碼 By Kin 2009/07/28
      sL_Inv007InvDate := getYearMonthDay7( AQry_Inv.FieldByName('PAPERDATE').AsDateTime,2);
      //判斷發票統編是否為空白，是則補8個0；另可判斷銷售額、營業稅額
      if AQry_Inv.FieldByName('BUSINESSID').AsString = '' then
        //5買受人統一編號
        //sL_Inv007BusinessID := '00000000'
        //#5829 買受人統一編號改成8個空白  By Kin 2010/10/29
        sL_Inv007BusinessID := '        '
      else
        //5買受人統一編號
        sL_Inv007BusinessID := AQry_Inv.FieldByName('BUSINESSID').AsString;
...
begin
  ...
  if chkNoDiscount.Checked then
  begin
    chkOnlyDiscount.Checked := False;
    txtPaperDateSt.Text := '';
    txtPaperDateEt.Text := '';
    txtPaperDateEt.Enabled := False;
    txtPaperDateSt.Enabled := False;
  end;
...
procedure TfrmInvA08.chkOnlyDiscountClick(Sender: TObject);
begin
  if chkOnlyDiscount.Checked then
  begin
    chkNoDiscount.Checked := False;
    txtPaperDateEt.Enabled := True;
    txtPaperDateSt.Enabled := True;
  end else
  begin
    txtPaperDateEt.Enabled := False;
    txtPaperDateSt.Enabled := False;
