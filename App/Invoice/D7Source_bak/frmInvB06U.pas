...
type
  ...
  TfrmInvB06 = class(TForm)
    ...
    Bevel4: TBevel;
    cxLabel9: TcxLabel;
    MSKUNUSUALPRIZE1: TcxDBMaskEdit;
    MSKUNUSUALPRIZE2: TcxDBMaskEdit;
    MSKUNUSUALPRIZE3: TcxDBMaskEdit;
    MSKUNUSUALPRIZE4: TcxDBMaskEdit;
    MSKUNUSUALPRIZE5: TcxDBMaskEdit;
    ...
    procedure mskFIRSTPRIZE1PropertiesValidate(Sender: TObject;
      var DisplayValue: Variant; var ErrorText: TCaption;
      var Error: Boolean);
    procedure mskSPECIALPRIZE1Exit(Sender: TObject);
  ...
  private
    ...
    function IsDataOK(Sender: TObject) : Boolean;
  public
    { Public declarations }
...
begin
  ...
  try
    ...
    //#5966 以下不要再做檢查了 By Kin 2011/03/29
    Result := True;
    Exit;
...
var
  ...
  begin
    ...
    aSQL := Format( 'INSERT INTO INV035 ( YEARMONTH , SPECIALPRIZE1,                ' +
            ...
            'UNUSUALPRIZE1,UNUSUALPRIZE2,UNUSUALPRIZE3,                             ' +
            'UNUSUALPRIZE4,UNUSUALPRIZE5 )                                          ' +
            ' VALUES (''%s'',''%s'',''%s'',''%s'',                                  ' +
            '''%s'',''%s'',''%s'',                                                  ' +
            '''%s'',''%s'',                                                         ' +
            ...
            cdsMaster.FieldByName( 'UNUSUALPRIZE1' ).AsString,
            cdsMaster.FieldByName( 'UNUSUALPRIZE2' ).AsString,
            cdsMaster.FieldByName( 'UNUSUALPRIZE3' ).AsString,
            cdsMaster.FieldByName( 'UNUSUALPRIZE4' ).AsString,
            cdsMaster.FieldByName( 'UNUSUALPRIZE5' ).AsString  ]);
    aDataSet.Close;
  ...
  begin
    ...
    aSQL :=Format('UPDATE INV035 SET  SPECIALPRIZE1=''%s'',SPECIALPRIZE2=''%s'',            ' +
                ...
                'UNUSUALPRIZE1=''%s'',UNUSUALPRIZE2=''%s'',UNUSUALPRIZE3=''%s'',            ' +
                'UNUSUALPRIZE4=''%s'',UNUSUALPRIZE5=''%s''                                  ' +
                'WHERE YEARMONTH = ''%s''                                                   ',
                [cdsMaster.FieldByName( 'SPECIALPRIZE1' ).AsString,
                cdsMaster.FieldByName( 'SPECIALPRIZE2' ).AsString ,
                cdsMaster.FieldByName( 'SPECIALPRIZE3' ).AsString,
...
procedure TfrmInvB06.mskFIRSTPRIZE1PropertiesValidate(Sender: TObject;
  var DisplayValue: Variant; var ErrorText: TCaption; var Error: Boolean);
  var i: Integer;
begin
  if (Sender as TcxDBMaskEdit).Text = EmptyStr then Exit;
...
function TfrmInvB06.IsDataOK(Sender: TObject): Boolean;
begin

end;

procedure TfrmInvB06.mskSPECIALPRIZE1Exit(Sender: TObject);
var i:Integer;
begin
  for i := 0 to  ComponentCount -1  do
    begin
      ...
      begin
        if ( ( Components[i] as TcxDBMaskEdit ).Text <> EmptyStr ) and
          ( (Components[i] as TcxDBMaskEdit ).Name <> (Sender as TcxDBMaskEdit).Name ) then
          begin
            if (Sender as TcxDBMaskEdit).Text = ( Components[i] as TcxDBMaskEdit ).Text then
            begin
              WarningMsg( '發票號碼重複' );
              (Sender as TcxDBMaskEdit).Text := EmptyStr;
              (Sender as TcxDBMaskEdit).SetFocus;
            end;
          end;
      end;
