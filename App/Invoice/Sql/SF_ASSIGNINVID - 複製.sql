	      ...
	      /* ------------------------------------------------------------------- */
              ...
              begin
                ...
                begin
                  ...
                  aSql := 'SELECT INVSEQNO,SERVICETYPE,CUSTID FROM ' || p_MisDbOwner || '.' || aTableName ||
                          ' WHERE BILLNO = ''' || aBillNo || ''' AND ITEM = ' || aItem;                                
                  execute immediate aSql into aInvseqno,aServiceType,aCustId;
	      ...
	      function WriteCustCarrier(aInvId in varchar, aCarrierType in varchar,
	        ...
	        aResult := true;
              ...
              function WriteToInv032A(aSeq in varchar2, aId in varchar2, aBillId in varchar2, aBillidItemno in number,
                aAcctNo in varchar2, aFacisNo in varchar2,aSmartCardNo in varchar2,aCMMac in varchar2)
                 return boolean
              as
                 aResult boolean;
				 aFind boolean;
              begin
                 ...
                 begin
                    ...
                    insert into inv032A ( itemidref, invid, billid, billiditemno, seq, startdate, enddate,
					   ...
					   combcitemname,facisno, accountno,SmartCardNo,CMMac )
                    select b.itemidref, aId, aBillId, aBillidItemno, null, a.startdate, a.enddate,
					   ...
					   combcitemname,aFacisNo, aAcctNo,aSmartCardNo,aCMMac
              ...
              begin
                 ...
                 begin
                    ...
                    insert into inv032 ( identifyid1, identifyid2, invid, billid, billiditemno,
                       ...
                       facisno, accountno,SmartCardNo,CMMac )
			 ...
			 /* #5668 增加取出智慧卡序號 */
              function GetMisInfo(aCId in varchar2, billid in varchar2, billiditemno in number,aInvoicKind in number,
                aAcctNo in out varchar2, aFacisNo in out varchar2,aSmartCardNo in out varchar2,aCMMac in out varchar2) return boolean
              as
                aResult boolean;
                aSql varchar2(1024);
                aTableName varchar2(100);
	...
	*/
               ...
               loop
				  ...
				  aCMMac := null;
                   ...
                   if ( p_LinkToMis = 'Y' ) and ( nvl( c2.linktomis, 'Y' ) = 'Y' ) then
                      if not GetMisInfo( c1.custid, c2.billid, c2.billiditemno, Nvl(c1.InvoiceKind,0), aAccountNo, aFacisNo,aSmartCardNo,aCMMac ) then
                        return p_RetCode;
                      end if;
                   ...
                   */
                   ...
                   if not WriteToInv032A( c2.seq, 'X', c2.billid, c2.billiditemno, aAccountNo, aFacisNo,aSmartCardNo,aCMMac ) then
                     return p_RetCode;
                   end if;
    ...
    begin
       insert into inv008 ( identifyid1, identifyid2, invid, billid, billiditemno, seq,
		  ...
		  saleamount, linktomis, facisno ,SmartCardNo,CMMac)
       select p_IdentifyID1, p_IdentifyID2, b.invid, b.billid, b.billiditemno, b.seq,
		  ...
		  b.saleamount, nvl( b.LinkToMis, 'Y' ), b.facisno,b.SmartCardNo,b.CMMac
