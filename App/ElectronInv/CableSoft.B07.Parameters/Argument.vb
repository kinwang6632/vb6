...
Public Class Argument
    ...
    Public Property MaxDBPool As String = Nothing
    Public Property MinDBPool As String = Nothing
    Public Property LifePoolTime As String = Nothing    
    ...
    Public Function GetConnect() As String
        ...
        Dim aRet As String = String.Empty
        Dim ConnectString As New OracleClient.OracleConnectionStringBuilder()
        ConnectString.PersistSecurityInfo = True
        ConnectString.Unicode = True
        ConnectString.DataSource = lstParameters.Item(0)
        ConnectString.UserID = lstParameters.Item(1)
        ConnectString.Password = lstParameters.Item(2)

        If (Not String.IsNullOrEmpty(MaxDBPool)) AndAlso
            (Not String.IsNullOrEmpty(MinDBPool)) AndAlso
            (Not String.IsNullOrEmpty(LifePoolTime)) Then
            If IsNumeric(MaxDBPool) AndAlso
                IsNumeric(MinDBPool) AndAlso
                IsNumeric(LifePoolTime) Then
                ConnectString.Pooling = True
                ConnectString.MaxPoolSize = Integer.Parse(MaxDBPool)
                ConnectString.MinPoolSize = Integer.Parse(MinDBPool)
                ConnectString.LoadBalanceTimeout = Integer.Parse(LifePoolTime)
            End If
        End If
    ...
    Public Function GetSysDate() As DateTime
        ...
        Dim cn As New OracleClient.OracleConnection(Me.GetConnect)
        Dim cmd As OracleClient.OracleCommand = Nothing
        Try
            cn.Open()
            cmd = cn.CreateCommand
            cmd.CommandText = "select sysdate from dual"
            Result = DateTime.Parse(cmd.ExecuteOracleScalar.ToString)                      
            Return Result
        Catch ex As Exception
            Throw
        Finally
    ...
    Public Shared Sub WriteLog(ByVal aFile As String, ByVal aMsg As String)
        ...
        End Try
    End Sub
    Public Overloads Shared Sub WriteProdcutLog(ByVal FilePath As String,
                                                ByVal aTime As DateTime, ByVal aContext As String,
        ...
        If Not String.IsNullOrEmpty(FileType) Then
            FileType = "_" & FileType
        End If
        WriteProdcutLog(String.Format("{0}\電子發票上傳記錄_{1}{2}.TXT",
                                      Path.GetDirectoryName(FilePath),
                                                aTime.ToString("yyyyMMddHHmmss"),
                                                FileType),
                                            aContext)

    End Sub

    Public Overloads Shared Sub WriteProdcutLog(ByVal FilePath As String, ByVal aTime As DateTime, ByVal aContext As String)
        WriteProdcutLog(FilePath, aTime, aContext, String.Empty)
    End Sub
    Public Overloads Shared Sub WriteProdcutLog(ByVal FileName As String, ByVal aContext As String)
        Try
            Using oWriter As New StreamWriter(FileName, False)
        ...
        End Try
    End Sub
    'Public Overloads Shared Sub WriteProdcutLog(ByVal aPath As String, ByVal aContext As String)
    ...
    Public Shared Function GetAppPath() As String
        Dim aRet As String = String.Empty
        aRet = System.AppDomain.CurrentDomain.BaseDirectory  'Application.StartupPath
        If aRet.EndsWith("\") Then
            Return aRet
        Else
            Return aRet & "\"
