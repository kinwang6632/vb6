...
begin
  ...
  try
    ...
    {#6700 增加停用 By Kin 2014/1/3}
    chkStopFlag.Checked := False;
    {#7605 By Kin 2017/09/26}
    chkPeriodFlag.Checked := False;
    edtBundleMon.Text := '0';
...
begin
  ...
  try
    ...
    {#5318 增加點數行銷辦法 By Kin 2009/10/06}
    ...
    //7605 by Kin 2017/09/26
    chkPeriodFlag.Checked := ( FDataSet.FieldByName('PeriodFlag').AsString = '1');
    {}
    InitCD019Data;
    LoadCD019Data;
    ...
    {}
    //#5913 增加 PFLAG2 By Kin 2011/04/14
    //#6216 資料如果是2要將ItemIndex設成1不然會跳到0 By Kin 2012/02/03
...
//    cmbPFlag2.ItemIndex := FDataSet.FieldByName( 'PFLAG2' ).AsInteger-1;
    ...
    {#6865 增加一個4.應收金額，所以Index要固定停在3 By Kin 2014/09/09}
    ...
    {# 增加 ContractType By Kin 2015/04/21}
    ...
    {# 增加 AuthTunerCount By Kin 2015/04/21}
    ...
    dmSave:
      begin
        ...
        begin
          try
            FDataSet.Post;
            {#6865 勾選長繳別要同步正負項 By Kin 2014/10/07}
            SyncLongPayFlag;
            DoSubDetailSave;
...
begin
  ...
  try
    ...
    {}
    {#7605 By Kin 2017/09/26}
    FDataSet.FieldByName('PeriodFlag').AsString := '0';
    if chkPeriodFlag.Checked then
      FDataSet.FieldByName('PeriodFlag').AsString := '1';
    {}
    {#6700 增加StopFlag By Kin 2014/1/3}
    ...
    {}
    {#5275 增加特定優惠截止日 By Kin 2010/05/05}
    ...
    {}
    //#5913 增加PFlag2 By Kin 2011/04/14
    //#6220 因為少了Index1,所以Index1要存成2 By Kin 2012/03/02
    ...
    {#5318 增加點數行銷欄位 By Kin 2009/10/06}
    ...
    {增加ContractType By Kin 2015/04/21}
    ...
    {# 增加AuthTunerCount By Kin 2015/04/21}
...
begin
  ...
  try
    ...
    { #5692 參考號23也要顯示出來 By Kin 2010/06/17 }
...
begin
  ...
  {#5257 第5個選項不管正負項都要出來 By Kin 2010/05/05}
  if ( ItemIndexOfValue( '5', cmbExpireType.Properties.Items ) < 0 ) then
      cmbExpireType.Properties.Items.Add( '5.約滿不跨階' );
end;
...
begin
  ...
  try
    frmCD078B3_1.Caption := '多階式優惠參數[frmCD078B30]';
...
procedure TfrmCD078B3.MSalePointCodeNamePropertiesInitPopup(
  Sender: TObject);
begin
  //#5318 測試後調整,只需過濾非停用的就好 By Kin 2009/10/30
  // Penny敲訊息說還是要過濾 POINTDOU=0的資料 By Kin 2009/11/06
  Screen.Cursor := crSQLWait;
  try
...
begin
  ...
  begin
    if MCItem.Value = EmptyStr then
    begin
      WarningMsg( '請設定對應正項收費項目' );
    end;
    FilterLinkKeyDataSet;
  end;
  frmCD078B3_7 := TfrmCD078B3_7.Create( FMode,FCD078A1DataSet,aKeys );
  try
    frmCD078B3_7.Caption := '階段式優惠條件設定[frmCD078B37]';
