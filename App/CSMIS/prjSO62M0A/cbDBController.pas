...
begin
  ...
  begin
    ...
    {#6035 使用到COM區的資料 By Kin 2011/06/08}
    ...
    {#6060 讀取 GroupId By Kin 2013/08/15}
    GetGropuId;
    {判別可否選擇到EPG By Kin 2013/08/15}
...
begin
  ...
  { 2. 抓每一個設備的的買賣方式, CD022, CD022A--> 設備型式 }
  ...
  DataReader.Close;
  { 3.檢查是否有重覆的資料, 重覆的話 Update InstCode 欄位, 否則新增
      一筆到 CD078D, 順便把其它欄位的值也填進去 }
  try
    for aIndex := Low( aFacilCodes ) to High( aFacilCodes ) do
    begin
      ...
      begin
        ...
        //#5859 測試不OK，要自動帶1 By Kin 2011/05/30
...
begin
  ...
  ZeroMemory( @aCItemCodes, SizeOf( aCItemCodes ) );
  {
   ...
   aCItemCodes[X,9] = CD078A1.StepNo --> 做為 CD078A1 負項連結正項用
   aCItemCodes[X,10] = CD019.CMCODE
   aCItemCodes[X,11] = CD019.CMNAME
  }
  { 1.先取 CD005 資料放到陣列裏, 收費項目 }
  DataReader.Close;
  DataReader.SQL.Text := Format(
  ...
  begin
    ...
    begin
      { 沒對應到 CD019 的話, 清掉原本取出的 CD005.CItemCode }
    ...
    begin
      {--------------------------------------------------------------------------}
      { #4407 如果是產品隨選則CD019必需符合 REFNO=2 And MasterProduct <>1 才可儲存 By Kin 2009/03/06 }
      { 突然說要取消這個需求 By Kin 2009/03/06 }
      {
      ...
      begin
        ...
        begin
          CodeReader.Close;
          AErrMsg := '此派工類別所附費用不可有付費頻道主商品,請重新設定！';
          Result := False;
          Exit;
        end;
        CodeReader.Close;
  ...
  { 這邊必須用 CD078A1CloneDataSet }
  aCD078A1 := frmCD078B1.CD078A1CloneDataSet;
  { 3.檢查是否有重覆的資料, 重覆的話 Update InstCode 欄位, 否則新增
      一筆到 CD078A, 順便把其它欄位的值也填進去 }
  ...
  try
    ...
    begin
      ...
      begin
        ...
        { 負項時, 若上一筆為正項, 自動填正項的收費項目進 MCItemCode }
        if ( SameText( aCItemCodes[aIndex,8], '-' ) ) then
        begin
      ...
      begin
        ...
        aCD078A1.FieldByName( 'DESCRIPTION' ).AsString := '預設繳別';
        ...
        begin
          ...
          { 若此筆為正項, 將此 StepNo 留下來, 當下一筆為負項時, 自動將此 StepNo 帶進去 }
