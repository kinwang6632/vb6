...
type
  ...
  TfrmCD078B1 = class(TForm)
    ...
    chkLessAmtCantCProm: TcxCheckBox;
    lbl4: TLabel;
    edtOSType: TcxComboBox;
...
begin
  ...
  begin
    ...
    chkLessAmtCantCProm.Enabled := False; //7471 
...
begin
  ...
  CD078DataSet.CommandText := Format(
    ...
    '        A.EPGORD,           ' +
    '        A.BYHOUSE,          ' +
    '        A.LESSAMTCANTCPROM, ' +      //#7471 Add field By Kin 2017/06/14
    '        A.OSTYPE            ' +      //#7456
    '   FROM %s.CD078 A          ' +
...
begin
  ...
  {#7605 Add the field of PeriodFlag by kin 2017/09/26 }
  CD078ADataSet.Close;
...
begin
  ...
  edtKindFunction.ItemIndex := 0;
  edtOSType.ItemIndex := 0; //#7456
  if ( CD078DataSet.FieldByName('OSType' ).AsString = '0' ) then edtOSType.ItemIndex := 1;
  if ( CD078DataSet.FieldByName('OSType' ).AsString = '1' ) then edtOSType.ItemIndex := 2;
  ...
  //7471
  if (CD078DataSet.FieldByName( 'LessAmtCantCProm').AsString = '1' ) then
    chkLessAmtCantCProm.Checked := True;
...
begin
  ...
  {#7456 By Kin 2017/11/17}
  sOSType := IntToStr(edtOSType.ItemIndex - 1);
  if ( sOSType = '-1' ) then sOSType := 'null';
  if ( FMode = dmInsert ) then
  begin
    aWriter.Close;
    aWriter.SQL.Text := Format (
        ...
        OneOrZero(chkLessAmtCantCProm.checked),
        sOSType] );
  end else
  begin
...
begin
  ...
  aInsertSql :=
    ...
    '   AuthTunerCount, ContractType , PERIODFLAG                )      ' +
    ' VALUES ( ''%s'', ''%s'', ''%s'', ''%s'',                          ' +
    '   ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'',                 ' +
    '   ''%s'', ''%s'', ''%s'', ''%s'',                                 ' +
    '   ''%s'', ''%s'', ''%s'', ''%s'', ''%s'', ''%s'',                 ' +
  ...
  try
    ...
    begin
      ...
      aWriter.SQL.Text := Format( aInsertSql, [
         ...
         CD078ADataSet.FieldByName( 'PERIODFLAG').AsString ] );
      aWriter.ExecSQL;
      {}
