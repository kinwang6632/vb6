...
print msg
  ...
  Date: 2002.09.10
	...
	2003.08.25 改: 加上多統計MediaCode=45的資料

	2003.10.28 改bug: "6. 項目為eBox(賣)一次付清，且金額=[eBox賣牌價], 或  項目為eBox(賣)分期, 且金額>0, 
			則[eBox(賣)]數值加1", 程式中第二個v_eBoxSaleFeeCode1改為v_eBoxSaleFeeCode2

  By: Lawrence
  Date: 2003.11.12 改收費項目   舊代碼   新代碼  
                   1台維護費       111-->   331
                   2台維護費       112-->   332
                   3台維護費       113-->   333 
                   4台維護費       114-->   334
...
as
  ...
  v_BCFee2Code2		number := 331;		-- 5.分機訊號費項目代碼2(1台)
  v_BCFee2Code3		number := 332;		-- 5.分機訊號費項目代碼3(2台)
  v_BCFee2Code4		number := 333;		-- 5.分機訊號費項目代碼4(3台)
  v_BCFee2Code5		number := 334;		-- 5.分機訊號費項目代碼5(4台)
  v_BCFee2Code6		number := 335;		-- 5.分機訊號費項目代碼6(5台)
...
*/
    ...
    begin
      v_SQLcsr := 'select IntroId EmpNo, IntroName EmpName, MediaCode, MediaName from '||OwnerName(i)||'.SO007 where '||
		v_SQL||' and FinTime is not null and (MediaCode=10 or MediaCode=18 or MediaCode=45) and IntroId is not null'||
	...
	v_SQL2 := 'select SNo, InstCode WoCode from '||OwnerName(i)||'.SO007';
	  ...
	  loop		-- loop 該受理人員之該張工單之相關收費, 做以下處理
		...
		x_CitemCode=v_eBoxSaleFeeCode2 and x_RealAmt>0 then
	      v_FlagF := 1;
	      v_Cnt6 := v_Cnt6 + 1;
