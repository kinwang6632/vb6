...
@c:\EMC\TRANS\sql\AdjSO033period1
variable nn number
...
create or replace function AdjSO033period1(p_RetMsg out varchar2) return number
  ...
  cursor cc1 is
	...
	and (AreaCode='105' or AreaCode='106');

  -- 短收原因要過濾refno=1 ; stcode not in (?,?,..)
  cursor cc2(v_Cid number,v_Ccd number) is
    select realdate,realperiod,realamt from SO033 where custid=v_custid and citemcode=v_citemcode and realdate is not null and nvl(stcode,0) not in (35,36,38,39,40,41,47,48) order by realdate desc;

  -- 短收原因要過濾refno=1 ; stcode not in (?,?,..)
  cursor cc3(v_Cid number,v_Ccd number) is
    select realdate,realperiod,realamt from SO034 where custid=v_custid and citemcode=v_citemcode and realdate is not null and nvl(stcode,0) not in (35,36,38,39,40,41,47,48) order by realdate desc;
