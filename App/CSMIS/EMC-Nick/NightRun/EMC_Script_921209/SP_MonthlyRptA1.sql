...
exec SP_MonthlyRptA1
  ...
  Date: 2003.05.05
	...
	2003.10.28 改: 
		(1) 客戶狀態請包含正常與停機 custstatuscode in (1,2)
		(2) 為必免計算到重覆資料，所以在計算有效STB數時，請使用Count(Distinct FaciSNo)
...
begin
  ...
  begin
	...
	0, 0, 0, count(distinct FaciSNo) from SO004 A, CD022 B, SO002 C, SO001 D where A.FaciCode=B.CodeNo and B.RefNo=3 and 
	A.CustId=C.CustId and A.ServiceType=v_ServiceType and A.CustId=D.CustId and A.InstDate<v_PrDate2 and 
	(A.PrDate is null or A.PrDate>=v_PrDate1) and C.CustStatusCode in (1,2) and D.ClassCode1<700);
    else
      insert into SO508 (RptYM, CompCode, ServiceType, CitemCode, CitemName, RealAmt, MonthPast, 
	Month01, CustCnt) (select v_RptYM, v_CompCode, v_ServiceType, -1, '有效STB數', 
	0, 0, 0, count(distinct FaciSNo) from SO004 A, CD022 B, SO002 C where A.FaciCode=B.CodeNo and B.RefNo=3 and 
	A.CustId=C.CustId and A.ServiceType=v_ServiceType and A.InstDate<v_PrDate2 and 
	(A.PrDate is null or A.PrDate>=v_PrDate1) and C.CustStatusCode in (1,2));
    end if;
  exception
    when others then
      insert into SO030A (JobId, CompCode, ServiceType, BeginExecTime, EndExecTime, 
