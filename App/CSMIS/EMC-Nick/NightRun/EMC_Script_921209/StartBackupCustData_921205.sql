/* 
  StartBackupCustData_921205.sql
  ...
  Date: 2003.11.14 (1) 改成不會使執行時間偏差的寫法, (2) 修改Night-run執行時間為每月6日01:30
	2003.12.05 改成判斷是否月初, 若仍於每月06日內, 則當月執行, 否則次月執行
...
BEGIN
  ...
  if to_char(sysdate, 'DD') >= substr(v_Time,1,2) then		-- 下個月做第一次執行
    v_ExecTime := to_date(to_char(add_months(sysdate,1), 'YYYYMM')||v_Time, 'YYYYMMDDHH24MI');	-- 第一次執行時間
  else								-- 本月就做第一次執行
    v_ExecTime := to_date(to_char(sysdate, 'YYYYMM')||v_Time, 'YYYYMMDDHH24MI');	-- 第一次執行時間
  end if;
