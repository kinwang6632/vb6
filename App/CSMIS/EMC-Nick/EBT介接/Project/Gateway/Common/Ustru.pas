...
Type
  ...
  public
    class function binToInt(sI_BinValue:STring):Integer;  
...
class function TUstr.binToInt(sI_BinValue: STring): Integer;
var
    sL_Tmp : String;
    nL_Tmp, ii,jj, nL_Result, nL_Base : Integer;
begin
    //將 2 進位之字串轉為 10 進位的數字...

    nL_Result := 0;
    jj := length(sI_BinValue);
    for ii:=length(sI_BinValue) downto  1 do
    begin

      sL_Tmp := Uppercase(sI_BinValue[ii]);
      nL_Base := StrToInt(sL_Tmp);
      nL_Result := nL_Result + nL_Base*trunc(power(2,jj-ii));
