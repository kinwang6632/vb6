...
type
  ...
  private
    ...
    {}
    function ConvertCompCode(const AOringal: String): String;
...
function TCMWebServiceManager.ConvertCompCode(const AOringal: String): String;
begin
  Result := AOringal;
  if ( Result = '13' ) then Result := '17';
end;
...
begin
  Result := Format( 'WORKID=%s&CUSSO=%s&CUSID=%s&CMMAC=%s',
    [aSO311.WorkId, ConvertCompCode( aSO311.CusSo ), aSO311.CusId, aSO311.CMMac] );
end;
...
begin
  Result := Format( 'WORKID=%s&CUSSO=%s&CUSID=%s&PCIP=%s&StartDateTime=%s&EndDateTime=%s',
   [aSO311.WorkId, ConvertCompCode( aSO311.CusSo ), aSO311.CusId, aSO311.PCIP,
    aSO311.StartDateTime, aSO311.EndDateTime] );
end;
...
begin
  Result := Format( 'WORKID=%s&CUSSO=%s&CUSID=%s&CMMAC=%s&EMTAMAC=%s&CLASSID=%s&PCIPNO=%s&OPERTYPE=%s',
   [aSO311.WorkId, ConvertCompCode( aSO311.CusSo ), aSO311.CusId, aSO311.CMMac, aSO311.EMTAMac,
...
begin
  ...
  aSo311.ExtendedInfo := Format( 'WORKID=%s&CUSSO=%s&Zone=%s&Lin=%s&Section=%s&Lane=%s&Alley=%s&SubAlley=%s&NO1=%s&NO2=%s',
   [aSO311.WorkId, ConvertCompCode( aSO311.CusSo ), aZone, aSO311.Lin, aSO311.Section,
