...
type
  ...
  public
    ...
    function connectDB(sI_DbAlias, sI_DbUserID, sI_DbPasswd : String):boolean;
...
uses frmSo18C0U, UdateTimeu, Ustru;
...
function TdtmMain.activeGetPaperData(sI_BeginDate, sI_EndDate, sI_EmpSQL,
...
begin
    ...
    begin
      if sL_Where='' then
        ...
        sL_Where := sL_Where + ' and ' + STR_SEP + sI_Prefix + STR_SEP;
    end;
...
function TdtmMain.canModifyGetPaperData(nI_Seq: Integer): boolean;
var
    sL_SQL : String;
    bL_Result : boolean;
begin
...
function TdtmMain.connectDB(sI_DbAlias, sI_DbUserID,
...
function TdtmMain.getSeq: Integer;
var
    sL_SQL : String;
begin
    with qryCommon do
...
procedure TdtmMain.qrySo126GETPAPERDATEGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
...
procedure TdtmMain.cdsSo126BeforePost(DataSet: TDataSet);
var
...
begin
  ...
  //        sL_TmpPaperNum := TUstr.AddString(IntToStr(ii),'0',false,PAPER_NUMBER_LENGTH);
          ...
          sL_SQL := sL_SQL + 'values(' + frmSo18C0.sG_CompID + ',' + IntToStr(nL_Seq)+',' + STR_SEP + sL_TmpPaperNum + STR_SEP;
          ...
          sL_SQL := sL_SQL + ',NULL,1,' + STR_SEP + frmSo18C0.sG_Operator + STR_SEP;
    ...
    DataSet.FieldByName('COMPCODE').AsString := frmSo18C0.sG_CompID;
    ...
    DataSet.FieldByName('OPERATOR').AsString := frmSo18C0.sG_Operator;
    DataSet.FieldByName('UPDTIME').AsString := sL_Now;
end;

procedure TdtmMain.saveToDB(I_CDS: TClientDataSet);
begin
    with I_CDS do
    begin
...
procedure TdtmMain.cdsSo126GETPAPERDATEGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
    if Sender.AsDateTime=0 then Exit;
    Text := TUdateTime.CDateStr(Sender.AsDateTime,9);
...
procedure TdtmMain.cdsSo126RETURNDATEGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
    if Sender.AsDateTime=0 then Exit;
    Text := TUdateTime.CDateStr(Sender.AsDateTime,9);
...
procedure TdtmMain.cdsSo126CLEARDATEGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
    if Sender.AsDateTime=0 then Exit;
    Text := TUdateTime.CDateStr(Sender.AsDateTime,9);
...
procedure TdtmMain.deletePaperData(nI_Seq: Integer);
begin
    with qryCommon do
    begin
      Close;
...
function TdtmMain.getPaperData(sI_PaperSNum, sI_PaperENum: String): TDataSet;
var
    sL_SQL,sL_Where : String;
...
function TdtmMain.disablePaperData(sI_PaperSNum, sI_PaperENum: String): Boolean;
var
...
begin
    ...
    try
      ...
      sL_SQL := sL_SQL + ',OPERATOR=' + STR_SEP + frmSo18C0.sG_Operator + STR_SEP;
      ...
      sL_SQL := sL_SQL + ' and STATUS<>0 and COMPCODE=' + frmSo18C0.sG_CompID;
      with qryCommon do
      begin
        Close;
        SQL.Clear;
...
function TdtmMain.activeReportData(sI_BeginDate, sI_EndDate, sI_EmpSQL,
...
begin
    ...
    case nI_ReportType of
     0://手開單據領用明細表
      begin
        sL_SQL := 'select EMPNO,EMPNAME,GETPAPERDATE,COUNT(SEQ) COUNT, MIN(PAPERNUM) MIN_PAPER_NUM, MAX(PAPERNUM) MAX_PAPER_NUM from SO127 ';
        ...
        if sL_Where='' then
          sL_Where := ' where CompCode=' + frmSo18C0.sG_CompID
        else
        sL_Where := sL_Where + ' and CompCode=' + frmSo18C0.sG_CompID;
...
procedure TdtmMain.transReport2Data(sI_WhereClause:String);
var
    sL_SQL : String;
...

{
        ...
        begin
          ...
          sL_EmpName := FieldByName('EMPNAME').AsString;
          sL_GetPaperDate := TUdateTime.CDateStr(FieldByName('GETPAPERDATE').AsDateTime,9);
          if ((sL_PriorEmpNo<>sL_EmpNo) and (sL_PriorEmpNo<>'')) or ((sL_PriorGetPaperDate<>sL_GetPaperDate) and (sL_PriorGetPaperDate<>''))then
          begin
            cdsReport2.Append;
{
...
procedure TdtmMain.qryReport2CommonGETPAPERDATEGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
    {
    if Sender.AsDateTime=0 then Exit;
...
function TdtmMain.getReport2PaperCounts(sI_EmpNo,
...
function TdtmMain.getReport2PaperCounts(sI_EmpNo:String; dI_GetPaperDate:TDate): Integer;
begin
    with qryReport2Common do
    begin
...
procedure TdtmMain.transReport3Data;
begin
    with qryReport3 do
    begin
      while not Eof do
...
procedure TdtmMain.qrySo18C2GETPAPERDATEGetText(Sender: TField;
  var Text: String; DisplayText: Boolean);
begin
    if Sender.AsDateTime=0 then Exit;
    Text := TUdateTime.CDateStr(Sender.AsDateTime,9);
...
function TdtmMain.checkDul(sI_Prefix: String; nI_BeginNumLength : Integer;fI_BeginNum,
...
function TdtmMain.getSo034Data(sI_BillNo: String): Integer;
var
    sL_SQL : String;
    nL_RecCount : Integer;
begin
    ...
    sL_SQL := sL_SQL + ' and a.CompCode=' + frmSo18C0.sG_CompID;
...
function TdtmMain.setNewHandPaperNo(sI_CustID,sI_CustName, sI_Tel1, sI_Item:String; dI_RealDate : TDate; sI_BillNo, sI_OldHandPaperNo,
  sI_NewHandPaperNo: String; var sI_ErrorMsg:String): boolean;
    ...
    begin
      ...
      sL_SQL := sL_SQL + 'and Item=' + sI_Item + ' and CompCode=' + frmSo18C0.sG_CompID;
      with qryCommon do
      begin
        Close;
        SQL.Clear;
    ...
    begin
      ...
      sL_SQL := sL_SQL + ' and COMPCODE=' + frmSo18C0.sG_CompID;
      with qryCommon do
      begin
        Close;
        SQL.Clear;
        ...
        begin
          ...
          sL_SQL := sL_SQL + '(' + frmSo18C0.sG_CompID + ',' + STR_SEP + sI_HandPaperNo + STR_SEP;
          sL_SQL := sL_SQL + ',' + STR_SEP + sL_UpdateStatus + STR_SEP;
          sL_SQL := sL_SQL + ',' + STR_SEP + frmSo18C0.sG_Operator + STR_SEP;
    ...
    begin
      ...
      begin
        ...
        sL_SQL := sL_SQL + ' where COMPCODE=' + frmSo18C0.sG_CompID;
...
function TdtmMain.getSo128Data(dI_SDate, dI_EDate: TDate): Integer;
...
function TdtmMain.reusePaper(dI_OriginalGetPaperDate:TDate; sI_OriginalEmpNo, sI_OriginalEmpName, sI_OriginalBeginNum, sI_OriginalSeq, sI_CompCode,  sI_EmpNo, sI_EmpName, sI_Prefix, sI_BeginNum, sI_EndNum,
  sI_GetPaperUser, sI_GetPaperDate, sI_OriginalCount, sI_Count: String): String;
var
...
//          SQL.Add('update SO126 set ENDNUM=' + STR_SEP + IntToStr(StrToInt(sI_BeginNum)-1) + STR_SEP );
          ...
          SQL.Add(', OPERATOR=' + STR_SEP + frmSo18C0.sG_Operator + STR_SEP );
    ...
    //      SQL.SaveToFile('c:\a1.txt');
        ...
        begin
          ...
          SQL.Add(' values(' +  frmSo18C0.sG_CompID + ',' + IntToStr(nL_Seq) + ',' );
          ...
          SQL.Add(sI_Count + ',' + STR_SEP + frmSo18C0.sG_Operator + STR_SEP + ',' );
...
//            SQL.SaveToFile('c:\a3.txt');
        ...
        sL_SQL := sL_SQL + ' values(' + IntToStr(nL_Seq1) + ',' + frmSo18C0.sG_CompID + ',' ;
...
//          SQL.SaveToFile('c:\a4.txt');
        ...
        //2. insert 續用前段手開單資料...
        sL_SQL := 'insert into SO129(SEQ,COMPCODE,SPAPERNUM,EPAPERNUM,TOTALPAPERCOUNT, EMPNO, EMPNAME,GETPAPERDATE,NOTES,OPERATOR,UPDTIME)';
        sL_SQL := sL_SQL + ' values(' + IntToStr(nL_Seq2) + ',' + frmSo18C0.sG_CompID + ',' ;
        sL_SQL := sL_SQL + STR_SEP + sI_Prefix + sI_OriginalBeginNum + STR_SEP + ',';
...
//          SQL.SaveToFile('c:\a5.txt');
        ...
        //3. insert 續用後段手開單資料...
        sL_SQL := 'insert into SO129(SEQ,COMPCODE,SPAPERNUM,EPAPERNUM,TOTALPAPERCOUNT, EMPNO, EMPNAME,GETPAPERDATE,NOTES,OPERATOR,UPDTIME)';
        sL_SQL := sL_SQL + ' values(' + IntToStr(nL_Seq3) + ',' + frmSo18C0.sG_CompID + ',' ;
...
function TdtmMain.get129Seq: Integer;
var
    sL_SQL : String;
begin
    with qryCommon do
