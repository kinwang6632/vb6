...
type
  ...
  private
    ...
    sG_EmpsCode: String;
    sG_EmpsName: String;
    FQueryBeginDate: String;
    FQueryEndDate: String;
...
begin
  ...
  FQueryBeginDate := EmptyStr;
  FQueryEndDate := EmptyStr;
end;
...
begin
    ...
    FQueryBeginDate := sL_BeginDate;
    FQueryEndDate := sL_EndDate;
...
begin
  {}
  if ( FQueryBeginDate <> EmptyStr ) then
    aBeginDate := TUstr.ToROCYMD( FQueryBeginDate );
  if( FQueryEndDate <> EmptyStr ) then
    aEndDate := TUstr.ToROCYMD( FQueryEndDate );
  {}
  if ( aBeginDate = EmptyStr ) then
  begin
  ...
  try
    ...
    if ( aBeginDate <> EmptyStr ) or ( aEndDate <> EmptyStr)  then
