...
type
  ...
  public
    ...
    procedure PrepareCdsReport1;
    procedure qryReport1ToCdsReport1;
    procedure PrepareCdsReport3;
    procedure qryReport3ToCdsReport3;
    procedure PrepareCdsReport5;
    procedure qryReport5ToCdsReport5;
  end;
...
begin
  case aReportIndex of
    0:
      begin
        PrepareCdsReport1;
        qryReport1ToCdsReport1;
        cdsReport1.FieldByName( 'EMPNO' ).DisplayLabel := '員工編號';
        cdsReport1.FieldByName( 'EMPNAME' ).DisplayLabel := '領單人員';
        cdsReport1.FieldByName( 'GETPAPERDATE' ).DisplayLabel := '領單日期';
        cdsReport1.FieldByName( 'MIN_PAPER_NUM' ).DisplayLabel := '單據編號(起)';
        cdsReport1.FieldByName( 'MAX_PAPER_NUM' ).DisplayLabel :=  '單據編號(迄)';
        cdsReport1.FieldByName( 'COUNT' ).DisplayLabel := '數量';
        aText :=
          '報表名稱: 手開單據領用明細表' + '@' +
          ...
          '領單人員: ' + aEmpNames + '@' +
          '單號範圍: ' + aPaperNumber;
         DataSetToXLS( cdsReport1, aFile, aText, EmptyStr );
      end;
    1:
      begin
        cdsReport2.FieldByName( 'EMPNO' ).DisplayLabel := '員工編號';
        cdsReport2.FieldByName( 'EMPNAME' ).DisplayLabel := '領單人員';
        cdsReport2.FieldByName( 'GETPAPERDATE' ).DisplayLabel := '領單日期';
        cdsReport2.FieldByName( 'PAPERNUMBERS' ).DisplayLabel := '單據編號';
        aText :=
          '報表名稱: 未回報手開單據清冊' + '@' +
          '統計時間: ' + DateTimeToStr( Now ) + '@' + '執行人員:' + frmMainMenu.sG_User+'@' +
          '公司名稱: ' + TUstr.replaceStr( frmMainMenu.sG_CompName, '昇', '升' ) + '@' +
          ...
          '領單人員: ' + aEmpNames + '@' +
          '單號範圍: ' + aPaperNumber;
         DataSetToXLS( cdsReport2, aFile, aText, 'GROUPID' );
      end;
    2:
      begin
        PrepareCdsReport3;
        qryReport3ToCdsReport3;
        cdsReport3.FieldByName( 'EMPNO' ).DisplayLabel := '員工編號';
        cdsReport3.FieldByName( 'EMPNAME' ).DisplayLabel := '領單人員';
        cdsReport3.FieldByName( 'GETPAPERDATE' ).DisplayLabel := '領單日期';
        cdsReport3.FieldByName( 'PAPERNUM' ).DisplayLabel := '手開單號';
        cdsReport3.FieldByName( 'BILLNO' ).DisplayLabel := '單據編號';
        cdsReport3.FieldByName( 'CUSTID' ).DisplayLabel := '客編';
        cdsReport3.FieldByName( 'CUSTNAME' ).DisplayLabel := '客戶姓名';
        cdsReport3.FieldByName( 'CUSTTEL' ).DisplayLabel := '電話';
        cdsReport3.FieldByName( 'REALDATE' ).DisplayLabel := '收費日期';
        aText :=
          '報表名稱: 未回報手開單據清冊' + '@' +
          '統計時間: ' + DateTimeToStr( Now ) + '@' + '執行人員:' + frmMainMenu.sG_User+'@' +
          '公司名稱: ' + TUstr.replaceStr( frmMainMenu.sG_CompName, '昇', '升' ) + '@' +
          ...
          '領單人員: ' + aEmpNames + '@' +
          '單號範圍: ' + aPaperNumber;
         DataSetToXLS( cdsReport3, aFile, aText, EmptyStr );
      end;
    3:
      begin
        PrepareCdsReport5;
        qryReport5ToCdsReport5;
        cdsReport5.FieldByName( 'SEQ' ).DisplayLabel := '序號';
        cdsReport5.FieldByName( 'PAPERNUM' ).DisplayLabel := '單號';
        cdsReport5.FieldByName( 'TOTALPAPERCOUNT' ).DisplayLabel := '單據張數';
        cdsReport5.FieldByName( 'EMPNAME' ).DisplayLabel := '領單人員';
        cdsReport5.FieldByName( 'GETPAPERDATE' ).DisplayLabel := '領單日期';
        cdsReport5.FieldByName( 'NOTES' ).DisplayLabel := '備註';
        cdsReport5.FieldByName( 'OPERATOR' ).DisplayLabel := '異動人員';
        cdsReport5.FieldByName( 'UPDTIME' ).DisplayLabel := '異動時間';
        aText :=
          '報表名稱: 領單續用紀錄表' + '@' +
          '統計時間: ' + DateTimeToStr( Now ) + '@' + '執行人員:' + frmMainMenu.sG_User+'@' +
          '公司名稱: ' + TUstr.replaceStr( frmMainMenu.sG_CompName, '昇', '升' ) + '@' +
          ...
          '領單人員: ' + aEmpNames + '@' +
          '單號範圍: ' + aPaperNumber;
         DataSetToXLS( cdsReport5, aFile, aText, EmptyStr );
      end;
  end;
end;
...
procedure TdtmMain3.PrepareCdsReport1;
begin
  if ( cdsReport1.FieldDefs.Count <= 0 ) then
  begin
    cdsReport1.FieldDefs.Add( 'EMPNO', ftString, 20 );
    cdsReport1.FieldDefs.Add( 'EMPNAME', ftString, 30 );
    cdsReport1.FieldDefs.Add( 'GETPAPERDATE', ftString, 20 );
    cdsReport1.FieldDefs.Add( 'MIN_PAPER_NUM', ftString, 20 );
    cdsReport1.FieldDefs.Add( 'MAX_PAPER_NUM', ftString, 20 );
    cdsReport1.FieldDefs.Add( 'COUNT', ftInteger );
    cdsReport1.CreateDataSet;
  end;
  cdsReport1.EmptyDataSet;
end;

{ ---------------------------------------------------------------------------- }

procedure TdtmMain3.qryReport1ToCdsReport1;

var
  aYear, aMonth, aDay: Word;
begin
  qryReport1.First;
  while not qryReport1.Eof do
  begin
    cdsReport1.Append;
    cdsReport1.FieldByName( 'EMPNO' ).AsString :=
      qryReport1.FieldByName( 'EMPNO' ).AsString;
    {}
    cdsReport1.FieldByName( 'EMPNAME' ).AsString :=
      qryReport1.FieldByName( 'EMPNAME' ).AsString;
    {}
    if not VarIsNull( qryReport1.FieldByName( 'GETPAPERDATE' ).Value ) then
    begin
      DecodeDate( qryReport1.FieldByName( 'GETPAPERDATE' ).AsDateTime, aYear,
        aMonth, aDay );
      cdsReport1.FieldByName( 'GETPAPERDATE' ).AsString := Format(
       '%3d/%.2d/%.2d', [aYear-1911, aMonth, aDay] );
    end;
    cdsReport1.FieldByName( 'MIN_PAPER_NUM' ).AsString :=
      qryReport1.FieldByName( 'MIN_PAPER_NUM' ).AsString;
    {}
    cdsReport1.FieldByName( 'MAX_PAPER_NUM' ).AsString :=
      qryReport1.FieldByName( 'MAX_PAPER_NUM' ).AsString;
    {}
    cdsReport1.FieldByName( 'COUNT' ).AsInteger :=
      qryReport1.FieldByName( 'COUNT' ).AsInteger;
    cdsReport1.Post;
    qryReport1.Next;
  end;
  cdsReport1.First;
...
procedure TdtmMain3.PrepareCdsReport3;
begin
  if ( cdsReport3.FieldDefs.Count <= 0 ) then
  begin
    cdsReport3.FieldDefs.Add( 'EMPNO', ftString, 20 );
    cdsReport3.FieldDefs.Add( 'EMPNAME', ftString, 30 );
    cdsReport3.FieldDefs.Add( 'GETPAPERDATE', ftString, 20 );
    cdsReport3.FieldDefs.Add( 'PAPERNUM', ftString, 20 );
    cdsReport3.FieldDefs.Add( 'BILLNO', ftString, 20 );
    cdsReport3.FieldDefs.Add( 'CUSTID', ftString, 20 );
    cdsReport3.FieldDefs.Add( 'CUSTNAME', ftString, 50 );
    cdsReport3.FieldDefs.Add( 'CUSTTEL', ftString, 20 );
    cdsReport3.FieldDefs.Add( 'REALDATE', ftString, 20 );
...
procedure TdtmMain3.qryReport3ToCdsReport3;
var
  aYear, aMonth, aDay: Word;
begin
  qryReport3.First;
  while not qryReport3.Eof do
  begin
    cdsReport3.Append;
    cdsReport3.FieldByName( 'EMPNO' ).AsString :=
      qryReport3.FieldByName( 'EMPNO' ).AsString;
    {}
    cdsReport3.FieldByName( 'EMPNAME' ).AsString :=
      qryReport3.FieldByName( 'EMPNAME' ).AsString;
    {}
    if not VarIsNull( qryReport3.FieldByName( 'GETPAPERDATE' ).Value ) then
    begin
      DecodeDate( qryReport3.FieldByName( 'GETPAPERDATE' ).AsDateTime, aYear,
        aMonth, aDay );
      cdsReport3.FieldByName( 'GETPAPERDATE' ).AsString := Format(
        '%3d/%.2d/%.2d', [aYear-1911, aMonth, aDay] );
    end;
    {}
    cdsReport3.FieldByName( 'PAPERNUM' ).AsString :=
      qryReport3.FieldByName( 'PAPERNUM' ).AsString;
    {}
    if ( qryReport3.FieldByName( 'STATUS' ).AsInteger = 0 ) then
    begin
      cdsReport3.FieldByName( 'BILLNO' ).AsString := '做廢'
    end else
    begin
      cdsReport3.FieldByName( 'BILLNO' ).AsString :=
        qryReport3.FieldByName( 'BILLNO' ).AsString;
    end;
    {}
    cdsReport3.FieldByName( 'CUSTID' ).AsString :=
      qryReport3.FieldByName( 'CUSTID' ).AsString;
    {}
    cdsReport3.FieldByName( 'CUSTNAME' ).AsString :=
      qryReport3.FieldByName( 'CUSTNAME' ).AsString;
    {}
    cdsReport3.FieldByName( 'CUSTTEL' ).AsString :=
      qryReport3.FieldByName( 'CUSTTEL' ).AsString;

    if not VarIsNull( qryReport3.FieldByName( 'REALDATE' ).Value ) then
    begin
      DecodeDate( qryReport3.FieldByName( 'REALDATE' ).AsDateTime, aYear,
        aMonth, aDay );
      cdsReport3.FieldByName( 'REALDATE' ).AsString := Format(
        '%3d/%.2d/%.2d', [aYear-1911, aMonth, aDay] );
    end;
    {}
    cdsReport3.Post;
...
procedure TdtmMain3.PrepareCdsReport5;
begin
  if ( cdsReport5.FieldDefs.Count <= 0 ) then
  begin
    cdsReport5.FieldDefs.Add( 'SEQ', ftString, 20 );
    cdsReport5.FieldDefs.Add( 'EMPNAME', ftString, 30 );
    cdsReport5.FieldDefs.Add( 'GETPAPERDATE', ftString, 20 );
    cdsReport5.FieldDefs.Add( 'PAPERNUM', ftString, 50 );
    cdsReport5.FieldDefs.Add( 'TOTALPAPERCOUNT', ftInteger );
    cdsReport5.FieldDefs.Add( 'NOTES', ftString, 200 );
    cdsReport5.FieldDefs.Add( 'OPERATOR', ftString, 20 );
    cdsReport5.FieldDefs.Add( 'UPDTIME', ftString, 20 );
    cdsReport5.CreateDataSet;
  end;
  cdsReport5.EmptyDataSet;
end;

{ ---------------------------------------------------------------------------- }

procedure TdtmMain3.qryReport5ToCdsReport5;
var
  aYear, aMonth, aDay: Word;
begin
  qryReport5.First;
  while not qryReport5.Eof do
  begin
    cdsReport5.Append;
    cdsReport5.FieldByName( 'SEQ' ).AsString :=
      qryReport5.FieldByName( 'SEQ' ).AsString;
    {}  
    cdsReport5.FieldByName( 'PAPERNUM' ).AsString :=
      qryReport5.FieldByName( 'SPAPERNUM' ).AsString + '~' +
      qryReport5.FieldByName( 'EPAPERNUM' ).AsString;
    {}
    cdsReport5.FieldByName( 'TOTALPAPERCOUNT' ).AsInteger :=
      qryReport5.FieldByName( 'TOTALPAPERCOUNT' ).AsInteger;
    {}
    cdsReport5.FieldByName( 'EMPNAME' ).AsString :=
      qryReport5.FieldByName( 'EMPNAME' ).AsString;
    {}
    if not VarIsNull( qryReport5.FieldByName( 'GETPAPERDATE' ).Value ) then
    begin
      DecodeDate( qryReport5.FieldByName( 'GETPAPERDATE' ).AsDateTime, aYear,
        aMonth, aDay );
      cdsReport5.FieldByName( 'GETPAPERDATE' ).AsString := Format(
       '%3d/%.2d/%.2d', [aYear-1911, aMonth, aDay] );
    end;
    {}
    cdsReport5.FieldByName( 'OPERATOR' ).AsString :=
      qryReport5.FieldByName( 'OPERATOR' ).AsString;
    {}
    cdsReport5.FieldByName( 'UPDTIME' ).AsString :=
      qryReport5.FieldByName( 'UPDTIME' ).AsString;
    {}
    cdsReport5.Post;
    qryReport5.Next;
  end;
