...
Type
  ...
  public
    ...
    class function ToROCYMD(aInDate: String): String;
    class function ToROCYM(aInDate: String): String;
  end;
...
class function TUstr.ToROCYM(aInDate: String): String;
var
  aYear: Integer;
  aHaveSlash: Boolean;
  aSlash: String;
begin
  aHaveSlash := ( Pos( '/', aInDate ) > 0 );
  aSlash := EmptyStr;
  if ( aHaveSlash ) then
  begin
    while Pos( '/', aInDate ) > 0 do
      Delete( aInDate, Pos( '/', aInDate ), 1 );
    aSlash := '/';
  end;
  aYear := StrToInt( Copy( aInDate, 1, 4 ) ) - 1911;
  if aYear < 100 then
    Result := '0' + IntToStr( aYear ) + aSlash + Copy( aInDate, 5, 2 )
  else
    Result := IntToStr( aYear ) + aSlash + Copy( aInDate, 5, 2 );
end;

class function TUstr.ToROCYMD(aInDate: String): String;
var
  aYear: Integer;
  aHaveSlash: Boolean;
  aSlash: String;
begin
  ...
  aYear := StrToInt( Copy( aInDate, 1, 4 ) ) - 1911;
  if aYear < 100 then
    Result := '0' + IntToStr( aYear ) + aSlash + Copy( aInDate, 5, 2 ) + aSlash + Copy( aInDate, 7, 2 )
  else
    Result := IntToStr( aYear ) + aSlash + Copy( aInDate, 5, 2 ) + aSlash + Copy( aInDate, 7, 2 );
end;
