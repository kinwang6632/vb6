...
procedure setUnvisibleFileNames(sI_FieldNames:String);    // howard
...
var c,r,fc:Integer;
  xls:TXLSWriter;
  sL_Title,sL_Temp : String;
  L_StrList : TStringList;
  nL_Column, ii : Integer;
...
//拆解SQLCommand '查詢條件:@公司別:開博科技@收費項目:情色頻道'
    ...
    begin
      ...
      begin
        sL_Title := L_StrList.Strings[ii];
        xls.Cellstr(ii,0,sL_Title);
      end;
    ...
    {
    for c:=0 to fc-1 do
      xls.Cellstr(ii+2,c,ds.Fields[c].DisplayLabel);
    }
    ...
    begin
      ...
      begin
        xls.Cellstr(ii+2,nL_Column,ds.Fields[c].DisplayLabel);
        Inc(nL_Column);
      end
    end;
      ...
      if r <= xls.maxRows then
       begin
        ...
        begin
          if AnsiPos(UpperCase(ds.Fields[c].FieldName), UpperCase(sI_UnvisableFieldsName))=0 then //howard
          begin
            xls.WriteField(r+ii+2,nL_Column,ds.Fields[c]);
            Inc(nL_Column);
          end;
        end;
