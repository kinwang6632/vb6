...
--資料檔轉換用
  ...
  cursor ccCD057A is
    select Rowid,InstCode,InstName from CD057A;
  ...
  cursor ccSO007 is
    select Rowid,SNo,InstCode,InstName,MediaCode,MediaName,ReturnCode,ReturnName,
           SatiCode,SatiName,BulletinCode,BulletinName from SO007;
           
  cursor ccSO008 is
    select Rowid,SNo,ServiceCode,ServiceName,MFCode2,MFName2,
           ReturnCode,ReturnName,SatiCode,SatiName from SO008;
           
  cursor ccSO009 is
    select Rowid,SNo,PRCode,PRName,ReasonCode,ReasonName,
...
*/
  --測試用
  --cursor ccCD006_MAP is
  --  select Rowid,OldCode from CD006_MAP;
...
/*測試用
  FOR crCD006_MAP in ccCD006_MAP loop
      v_NewCode1 := GetNewCode(crCD006_MAP.OldCode,CD006CodeMap);
      v_NewName1 := GetNewName(crCD006_MAP.OldCode,CD006NameMap);

      v_NewCode2 := v_NewCode1;
      v_NewName2 := v_NewName1;
...
--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crCD057A in ccCD057A loop
      BEGIN
        v_NewCode1 := GetNewCode(crCD057A.InstCode,CD005CodeMap);
        v_NewName1 := GetNewName(crCD057A.InstCode,crCD057A.InstName,CD005NameMap);


        UPDATE CD057A set InstCode=v_NewCode1,InstName=v_NewName1
      ...
      EXCEPTION
        WHEN others THEN
          ...
          INSERT INTO TransFormLog_2(TYPE,DESCRIPTION,TABLENAME)
            VALUES(C_UpdDataErrorType,'UPDATE 資料檔CD057A有誤  InstCode='||crCD057A.InstCode||'  InstName='||crCD057A.InstName,'CD057A');
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO007 in ccSO007 loop
      BEGIN
        ...
        v_NewCode2 := GetNewCode(crSO007.MediaCode,CD009CodeMap);
        v_NewName2 := GetNewName(crSO007.MediaCode,crSO007.MediaName,CD009NameMap);

        v_NewCode3 := GetNewCode(crSO007.ReturnCode,CD015CodeMap);
        v_NewName3 := GetNewName(crSO007.ReturnCode,crSO007.ReturnName,CD015NameMap);

        v_NewCode4 := GetNewCode(crSO007.SatiCode,CD026CodeMap);
        v_NewName4 := GetNewName(crSO007.SatiCode,crSO007.SatiName,CD026NameMap);

        v_NewCode5 := GetNewCode(crSO007.BulletinCode,CD049CodeMap);
        v_NewName5 := GetNewName(crSO007.BulletinCode,crSO007.BulletinName,CD049NameMap);
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO008 in ccSO008 loop
      BEGIN
        ...
        v_NewCode2 := GetNewCode(crSO008.MFCode2,CD012CodeMap);
        v_NewName2 := GetNewName(crSO008.MFCode2,crSO008.MFName2,CD012NameMap);

        v_NewCode3 := GetNewCode(crSO008.ReturnCode,CD015CodeMap);
        v_NewName3 := GetNewName(crSO008.ReturnCode,crSO008.ReturnName,CD015NameMap);

        v_NewCode4 := GetNewCode(crSO008.SatiCode,CD026CodeMap);
        v_NewName4 := GetNewName(crSO008.SatiCode,crSO008.SatiName,CD026NameMap);
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO071 in ccSO071 loop
      BEGIN
        ...
        UPDATE SO071 set InstCode=v_NewCode1 WHERE Rowid=crSO071.Rowid;
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO072 in ccSO072 loop
      BEGIN
        ...
        UPDATE SO072 set ServiceCode=v_NewCode1 WHERE Rowid=crSO072.Rowid;
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO073 in ccSO073 loop
      BEGIN
        ...
        UPDATE SO073 set PRCode=v_NewCode1 WHERE Rowid=crSO073.Rowid;
