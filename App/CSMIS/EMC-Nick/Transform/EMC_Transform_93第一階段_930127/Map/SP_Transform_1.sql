...
/*
    ...
    2003.12.27   Jackal 	1.CD005、CD006及CD007三個代碼檔移至第二階段再轉。
                          2.將舊的未繳費原因代碼CD013,存於SO033,SO034,SO035及SO036的NOTE內
                          
    2004.01.07   Jackal 	1.CD014、CD025及CD049三個代碼檔移至第二階段再轉。
                          2.將舊的短收原因代碼CD016,存於SO033,SO034,SO035及SO036的NOTE內
                          
    2004.01.08   Jackal 	1.CD008、CD008A及CD008C三個代碼檔移至第二階段再轉。
*/
...
--資料檔轉換用
  ...
  cursor ccSO001 is
    select Rowid,CustId,CompCode,PwCode,PwName from SO001;
    
  cursor ccSO002 is
    select Rowid,CustId,CompCode,ServiceType,MediaCode,MediaName,
           DelCode,DelName from SO002;
    
  cursor ccSO004 is
    select Rowid,SEQNo,MediaCode,MediaName from SO004;
    
  cursor ccSO006 is
    select Rowid,SEQNo,MediaCode,MediaName from SO006;
  ...
  cursor ccSO009 is
    select Rowid,SNo,ReturnCode,ReturnName,SatiCode,SatiName from SO009;
  ...
  cursor ccSO098 is
    select Rowid,MediaCode,MediaName from SO098;
    
  cursor ccSO101 is
    select Rowid,DelCode,DelName from SO101;
    
  cursor ccSO105 is
    select Rowid,OrderNo,MediaCode,MediaName,
           ReturnCode,ReturnName,CATVReturnCode,CATVReturnName,
           DVSReturnCode,DVSReturnName from SO105;
  ...
  cursor ccSO105T is
    select Rowid,OrderNo,CompCode,MediaCode,MediaName,ReturnCode,ReturnName,
           ...
           DVSReturnCode,DVSReturnName from SO105T;
...
--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO006 in ccSO006 loop
      BEGIN
        v_NewCode1 := GetNewCode(crSO006.MediaCode,CD009CodeMap);
        v_NewName1 := GetNewName(crSO006.MediaCode,crSO006.MediaName,CD009NameMap);


        UPDATE SO006 set MediaCode=v_NewCode1,MediaName=v_NewName1
...
--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO009 in ccSO009 loop
      BEGIN
        v_NewCode1 := GetNewCode(crSO009.ReturnCode,CD015CodeMap);
        v_NewName1 := GetNewName(crSO009.ReturnCode,crSO009.ReturnName,CD015NameMap);

        v_NewCode2 := GetNewCode(crSO009.SatiCode,CD026CodeMap);
        v_NewName2 := GetNewName(crSO009.SatiCode,crSO009.SatiName,CD026NameMap);


        UPDATE SO009 set ReturnCode=v_NewCode1,ReturnName=v_NewName1,
                         SatiCode=v_NewCode2,SatiName=v_NewName2
    ...
    FOR crSO098 in ccSO098 loop
      ...
      EXCEPTION
        WHEN others THEN
          ...
          INSERT INTO TransFormLog_2(TYPE,DESCRIPTION,TABLENAME)
            VALUES(C_UpdDataErrorType,'UPDATE 資料檔SO098有誤  MediaCode: '||crSO098.MediaCode,'SO098');
...
--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO105 in ccSO105 loop
      BEGIN
        v_NewCode1 := GetNewCode(crSO105.MediaCode,CD009CodeMap);
        v_NewName1 := GetNewName(crSO105.MediaCode,crSO105.MediaName,CD009NameMap);

        v_NewCode2 := GetNewCode(crSO105.ReturnCode,CD015CodeMap);
        v_NewName2 := GetNewName(crSO105.ReturnCode,crSO105.ReturnName,CD015NameMap);

        v_NewCode3 := GetNewCode(crSO105.CATVReturnCode,CD015CodeMap);
        v_NewName3 := GetNewName(crSO105.CATVReturnCode,crSO105.CATVReturnName,CD015NameMap);

        v_NewCode4 := GetNewCode(crSO105.DVSReturnCode,CD015CodeMap);
        v_NewName4 := GetNewName(crSO105.DVSReturnCode,crSO105.DVSReturnName,CD015NameMap);


        UPDATE SO105 set MediaCode=v_NewCode1,MediaName=v_NewName1,
                         ReturnCode=v_NewCode2,ReturnName=v_NewName2,
                         CATVReturnCode=v_NewCode3,CATVReturnName=v_NewName3,
                         DVSReturnCode=v_NewCode4,DVSReturnName=v_NewName4
