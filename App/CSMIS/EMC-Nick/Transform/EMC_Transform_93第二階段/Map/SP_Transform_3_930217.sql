...
--資料檔轉換用
  ...
  cursor ccSO504 is
    select Rowid,DataCode from SO504 where DataType=3;--當DataType=3時,DataCode=CD004.ClassCode
...
--%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  IF v_TempCounts=0 THEN --轉代碼前先確定沒有轉過
    ...
    FOR crSO504 in ccSO504 loop
      ...
      EXCEPTION
        WHEN others THEN
          ...
          INSERT INTO TransFormLog_2(TYPE,DESCRIPTION,TABLENAME)
            VALUES(C_UpdDataErrorType,'UPDATE 資料檔SO504有誤  DataCode: '||crSO504.DataCode,'SO504');
    ...
    INSERT INTO TransFormLog_1(TABLENAME,DESCRIPTION,OKCOUNTS,ERRORCOUNTS,STARTTIME,STOPTIME,ALLTIME)
      VALUES('SO504','資料檔SO504代碼轉換完成',v_OKCounts,v_ErrorCounts,v_StartTime,SysDate,v_AllTiem);
