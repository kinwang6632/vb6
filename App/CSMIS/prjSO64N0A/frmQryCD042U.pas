...
begin
  if Length(FSelectCode)>0 then
    ...
    //#7673 According the field of CodeNo and Description search for the result by kin 2017/11/14
    if edtCodeNo.Text <> EmptyStr then
    begin
      FSQLWhere := ( FSQLWhere + Format( ' AND INSTR(CODENO,''%s'',-1,1)> 0 ', [edtCodeNo.Text] ) );
    end;
    if edtDescription.Text <> EmptyStr then
    begin
      FSQLWhere := ( FSQLWhere + Format( ' AND INSTR(DESCRIPTION,''%s'',-1,1)> 0 ', [edtDescription.Text] ) );
    end;
  end;
