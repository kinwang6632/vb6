CREATE OR REPLACE FUNCTION SF_ENTITLEMENTQUERY
  ...
  RETURN  number AS
/*
--@D:\App\SGS\Script\SF_EntitlementQuery;
VARIABLE ReturnNo NUMBER
VARIABLE ErrCode NUMBER
VARIABLE ErrMsg VARCHAR2(1000)
VARIABLE SrcMsgID VARCHAR2(30)
EXEC :ReturnNo := SF_EntitlementQuery('7','110000S01','110000G01','','0x4a01','1','2003/03/24','2003/03/25 23:59:59','''B1'',''B2''',:SrcMsgID,:ErrCode,:ErrMsg)
PRINT ReturnNo
PRINT ErrCode
PRINT ErrMsg
PRINT SrcMsgID
  ...
  授權資訊查詢應答介面用於服務平臺將對IC卡進行授權的明細記錄返回給監管平臺
  檔名: SF_EntitlementQuery.sql
...
*/
  ...
  cursor ccFirstData is--第一次查詢完整資料
    Select A.SmartCardNo,A.ChCode,B.ChannelID AS ProductCode,A.SetTime
      from SO005 A,CD024 B
    Where A.CompCode=p_CompCode And A.ChCode=B.CodeNo And B.PayFlag<>0;



  cursor ccDiffDataA is--某日新授權的頻道
    Select A.SmartCardNo,B.ChannelID AS ProductCode,A.SetTime
      from SO005 A,CD024 B
    Where A.CompCode=p_CompCode And A.ChCode=B.CodeNo And B.PayFlag<>0
    And A.SetTime Between To_Date(v_StartDateTime,'YYYY/MM/DD HH24:MI:SS')
...
--將資料儲存置SO457或SO458
  FUNCTION SaveDataToDB(I_Table varchar2,I_CompCode Number,I_HandleEDateTime varchar2,
                        ...
                        I_CasID varchar2,I_ProductCode varchar2)
  RETURN varchar2
  AS
    L_SQL varchar2(4000);
  ...
  BEGIN
    BEGIN
      L_SQL := 'Insert Into '||I_Table||' (CompCode,HandleEDateTime,RealDateTime,'||
               'ExecDateTime,Status,ICCardNo,ICCardID,CasID,ProductCode) '||
               'Values('||I_CompCode||
    ...
    IF To_Date(To_Char(v_CurrDateTime,'YYYY/MM/DD'),'YYYY/MM/DD') > To_Date(v_FirstDate,'YYYY/MM/DD') THEN
      ...
      IF To_Date(p_HandleEDateTime,'YYYY/MM/DD HH24:MI:SS') = To_Date(v_FirstDate||' 23:59:59','YYYY/MM/DD HH24:MI:SS') THEN
        FOR crFirstData in ccFirstData loop
          ...
          v_Status := 0;--第一次歷史資料皆為New(0:Create  1:Delete)
          v_ICCardNo := crFirstData.SmartCardNo;
          v_ICCardID := '';--目前沒有
          v_ProductCode := crFirstData.ProductCode;

          v_ErrMsg := SaveDataToDB('SO457',p_CompCode,p_HandleEDateTime,
                        v_RealDateTime,v_ExecDateTime,v_Status,
                        v_ICCardNo,v_ICCardID,p_CasID,v_ProductCode);
...
--歷史資料(非第一次資料)及即時查詢當日異動資料
    ...
    IF To_Date(p_HandleEDateTime,'YYYY/MM/DD HH24:MI:SS') > To_Date(v_FirstDate||' 23:59:59','YYYY/MM/DD HH24:MI:SS') THEN
      --處理前先把原來的資料清空
      --歷史查詢:SO457   立即查詢:SO456
      IF p_InstQuery = '0' THEN--歷史資料
        v_Table := 'SO457';

        Delete SO457 Where CompCode=p_CompCode And
...
--當日新授權的頻道
      FOR crDiffDataA in ccDiffDataA loop
        ...
        v_Status := 0;--新授權的頻道歷史資料皆為New(0:Create  1:Delete)
...
--當日取消授權的頻道
      ...
      --3.loop取每一筆資料來處理
      LOOP
        FETCH cDynamic INTO v_ICCardNo,v_ProductCode,v_RealDateTime;
...
EXCEPTION
 WHEN OTHERS THEN
  ...
  RETURN -1;
END; -- Function SF_EntitlementQuery
/