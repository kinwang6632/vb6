...
type
  ...
  public
    ...
    sG_ExePath,sG_TcOrSc, sG_LocalIP, sG_LocalIisPort : String;
    ...
    function isValidRemoteIP(sI_RemoteIP:String):boolean;        
    ...
    function getRealXmlFilePath(sI_CompCode,sI_InstQuery,sI_QueryType,sI_HandleEDateTime : String;bI_IsZipFile, bI_DeleteExistFile : Boolean) : String;
    ...
    function checkIsAlreadyAction(sI_CompCode,sI_ModeType,sI_HandleEDateTime : String) : Boolean;
  end;
...
begin
    ...
    if nG_NightRunExecTime < 0 then
      nG_NightRunExecTime := 1
    ...
    sG_LocalIisPort := L_IniFile.ReadString('SYSINFO','LOCAL_IIS_PORT','8080');
...
function TdtmMain.getRealXmlFilePath(sI_CompCode, sI_InstQuery,
  sI_QueryType, sI_HandleEDateTime: String; bI_IsZipFile, bI_DeleteExistFile: Boolean): String;
var
    sL_DataPath1,sL_DataPath2,sL_SunFileName,sL_FileName,sL_XmlFilePath : String;
    nL_FileHandel : Integer;
begin
    ...
    if not FileExists(sL_XmlFilePath) then
      nL_FileHandel := FileCreate(sL_XmlFilePath)
    else
    begin
      if bI_DeleteExistFile then
      begin
...
function TdtmMain.checkIsAlreadyAction(sI_CompCode, sI_ModeType,
  sI_HandleEDateTime: String): Boolean;
var
    sL_SQL : String;
    L_AdoQry : TADOQuery;
    nL_ActionCount : Integer;
begin
    sL_SQL := 'select count(*) ActionCount from SO459 where CompCode=' + sI_CompCode +
    ...
    if L_AdoQry <> nil then
    begin
      nL_ActionCount := L_AdoQry.FieldByName('ActionCount').AsInteger;
      if nL_ActionCount > 0 then
        Result := true
      else
        Result := false;
    end
...
function TdtmMain.isValidRemoteIP(sI_RemoteIP: String): boolean;
var
    bL_Result : boolean;
    ii : Integer;
    sL_TmpIp : String;
begin
    ...
    for ii:=0 to G_SGSParamIniStrList.Count  -1 do
    begin
      sL_TmpIp := (G_SGSParamIniStrList.Objects[ii] as TSGSParamIniData).sDstIP;
      if AnsiPos(sI_RemoteIP,sL_TmpIp)>0 then
      begin
        bL_Result := true;
        break;
      end;
