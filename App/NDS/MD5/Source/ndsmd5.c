...
/***************************************************************************/
/*static void MD5_memset(unsigned char *, unsigned char *, unsigned int);*/
/***************************************************************************/
 ...
 */
/*static void MD5_memset (unsigned char * output,unsigned char * value, unsigned int len)
{
	
	memset((void*)output,(char)value,len);
	    /*unsigned int i;
        for (i = 0; i < len; i++)
...
*/
/***************************************************************************
  MD5Res returns result of MD5 algorithm on input	
***************************************************************************/
int MD5Res(unsigned char *input,unsigned long input_len, unsigned char *out)
{
	MD5_CTX context;
	MD5Init (&context);
...
/***************************************************************************
 NDS_RealMD5_16 returns result of RSA MD5 algorithm on input + key
***************************************************************************/
int NDS_RealMD5_16(unsigned char *input,unsigned short input_len,unsigned char *key,unsigned short key_len, unsigned char *out)
{
	int            retval;
	/*   int            status;*/
	...
	in_buffer = (unsigned char*)(malloc(input_len+key_len));
	if (!in_buffer)
		return -1;
	...
	out_buffer = (unsigned char*)(malloc(MD5_SIGN_LEN));
	if (!out_buffer)
		return -1;
	
	
    retval = MD5Res(in_buffer, (unsigned long)input_len + key_len, out_buffer);
	
	memcpy(out, out_buffer, MD5_SIGN_LEN);
	free(in_buffer);
	free(out_buffer);
	return(retval);
}
...
{
	unsigned char ret[MD5_SIGN_LEN];
	int i;
	NDS_RealMD5_16(input,input_len,key,key_len,ret);
	for(i = 0; i < MD5_SIGN_LEN/2; i++)
	{
		out[i] = ret[i] ^ ret[i + MD5_SIGN_LEN/2];
	}
	return 1;
}
...
/*
  Description: 
   This is a test for MD5 methods - a regular RSA MD5(returns 16 bytes),
   and a special NDS version of MD5(returns 8 bytes). 
   Every input is given with its expected output and if the result is 
   not identical an error message is printed.
...
{ 
   {"0001M0035010001000119940206125158ER01","8F772E8552CF7A19"}
  ,{"0001M0032010002000119950906114824O"   ,"36D69337D720F381"}
  ,{"0001M0035010002000119950906114844ES04","5B387EAD17A0A328"}
  ,{"0001M0035010002000119950906114904ES04","26399514C5767C57"}
...
int main(int argc,char **argv)
{
  ...
  {
    p = test_data[i].input;
    assert(NDS_RealMD5_8(p,(unsigned short)strlen((char*)p),(unsigned char*)"12345678",8,output));
    for(j=0;j<8;j++)
      sprintf((char*)buf+2*j,"%02X",(unsigned int)output[j]);
    assert(!strcmp((char*)buf,(char*)test_data[i].output));
  }
