...
uses
  Windows, Messages, SysUtils, Classes, Controls, Variants, Math ;
const
    CHINESEZERO='零';
    AMT_TAIL = '.00';
    LEFT_MINUS_FLAG = '(';
...
Type
  ...
  public
    class function isHexValue(sI_HexValue : String):boolean;  
    class function getMultiBytesHexValue(sI_TmpString:STring; var bI_HasMultiByte:boolean):STring;  
    class function hexToInt(sI_HexValue:STring):Integer; 
    class function AddString(sI_SrcString, sI_AddString:String; bI_Right:boolean; nI_TargetStrTotalLength:Integer):String;  
...
class function TUstr.AddString(sI_SrcString, sI_AddString: String;
  bI_Right: boolean; nI_TargetStrTotalLength: Integer): String;
var
    ii : Integer;
    sL_Result : String;  
begin
    sL_Result := sI_SrcString;
    for ii:=length(sI_SrcString) to nI_TargetStrTotalLength -1 do
    begin
      if bI_Right then
        sL_Result := sL_Result + sI_AddString
      else
        sL_Result := sI_AddString + sL_Result;
    end;
    result := sL_Result;
end;

class function TUstr.hexToInt(sI_HexValue: STring): Integer;
var
    sL_Tmp : String;
    nL_Tmp, ii,jj, nL_Result, nL_Base : Integer;
begin
    nL_Result := 0;
    jj := length(sI_HexValue);    
    for ii:=length(sI_HexValue) downto  1 do
    begin

      sL_Tmp := Uppercase(sI_HexValue[ii]);
      if sL_Tmp='A' then
        nL_Base := 10
      else if sL_Tmp='B' then
        nL_Base := 11
      else if sL_Tmp='C' then
        nL_Base := 12
      else if sL_Tmp='D' then
        nL_Base := 13
      else if sL_Tmp='E' then
        nL_Base := 14
      else if sL_Tmp='F' then
        nL_Base := 15
      else
        nL_Base := StrToInt(sL_Tmp);
      nL_Result := nL_Result + nL_Base*trunc(power(16,jj-ii));

    end;
    result := nL_Result;
end;

class function TUstr.getMultiBytesHexValue(sI_TmpString: STring; var bI_HasMultiByte:boolean): STring;
var
    sL_Result : String;
    L_TmpUcs4String : UCS4String;
    nL_HexLength, ii : Integer;
begin
    //CCTV中央電視台 => 00430043005400564E2D592E96FB899653F0

    bI_HasMultiByte := false;
    for ii:=0 to length(sI_TmpString) -1 do
    begin
      if Ord(sI_TmpString[ii+1]) > 127 then   //>127者才是中文
      begin
        bI_HasMultiByte := true;
        break;
      end;
    end;

    if bI_HasMultiByte then
      nL_HexLength := 4
    else
      nL_HexLength := 2;

    L_TmpUcs4String := WideStringToUCS4String(sI_TmpString);

    for ii:=0 to length(L_TmpUcs4String)-1 do
    begin
      if L_TmpUcs4String[ii]<>0 then
        sL_Result := sL_Result + IntToHex(L_TmpUcs4String[ii],nL_HexLength);
    end;
    result := sL_Result;

end;

class function TUstr.isHexValue(sI_HexValue: String): boolean; //檢查輸入值是不是 16 進位值
var
    sL_TmpValue : String;
    ii, nL_IntValue : Integer;
    bL_Result : boolean;
begin
    bL_Result := true;

    for ii:=1 to length(sI_HexValue) do
    begin
      sL_TmpValue := UpperCase(sI_HexValue[ii]);
      nL_IntValue :=  StrToIntDef(sL_TmpValue,-1);

      if ((nL_IntValue >=0) and (nL_IntValue <=9)) or
         ((sL_TmpValue='A') or (sL_TmpValue='B') or (sL_TmpValue='C') or
         (sL_TmpValue='D') or (sL_TmpValue='E') or (sL_TmpValue='F') ) then
        bL_Result := true
      else
      begin
        bL_Result := false;
