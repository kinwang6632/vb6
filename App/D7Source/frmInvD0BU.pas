...
type
  TfrmInvD0B = class(TForm)
    ...
    Label1: TLabel;
    lblCMSend: TcxLabel;
    edtEndCMNotify: TcxCurrencyEdit;
    lblCMSend2: TcxLabel;
...
begin
  ...
  {#5800 測試不OK,CM導流沒有啟動,則導流通知結束排程不要顯示 By Kin 2012/09/21}
  lblCMSend.Visible := dtmMain.sG_StarCMSend;
  lblCMSend2.Visible := dtmMain.sG_StarCMSend;
  edtEndCMNotify.Visible := dtmMain.sG_StarCMSend;
...
begin
  ...
  begin
    ...
    {#5800 增加顯示EndCMNotify By Kin 2012/07/12}
    edtEndCMNotify.EditValue := adoINV042.FieldByName( 'EndCMNotify' ).AsString;
...
begin
  ...
  begin
    ...
    adoINV042.FieldByName( 'EndCMNotify' ).AsInteger := 0;
    {#5800 增加儲存EndCMNotify By Kin 2012/07/02}
    if edtEndCMNotify.Value > 0 then
      adoINV042.FieldByName( 'EndCMNotify' ).AsFloat := edtEndCMNotify.Value;
