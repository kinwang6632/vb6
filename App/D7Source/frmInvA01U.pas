...
type
  TfrmInvA01 = class(TForm)
    ...
    Label16: TLabel;
    Label17: TLabel;
    lblNoESaleAmt: TLabel;
    lblESaleAmt: TLabel;
    lblNoETaxAmt: TLabel;
    lblNoEInvAmt: TLabel;
    lblNoEInvCount: TLabel;
    lblETaxAmt: TLabel;
    lblEInvAmt: TLabel;
    lblEInvCount: TLabel;
    Label24: TLabel;
    cobBusinessId: TComboBox;
    chkNormalInv: TCheckBox;
    chkElectronInv: TCheckBox;
    lbl1: TLabel;
    ...
    procedure cobBusinessIdExit(Sender: TObject);
    procedure chkElectronInvExit(Sender: TObject);
    procedure chkNormalInvClick(Sender: TObject);
    procedure chkElectronInvClick(Sender: TObject);
  private
  ...
  public
    ...
    nG_OrderPrefixNo,nG_FilterBusinessId : Integer;
...
begin
  ...
  {#6370 判斷是否可選取營業人 By Kin 2012/11/22}
  {#6370 判斷發票種類是否可選取 By Kin 2012/12/05}
  cobBusinessId.Enabled := False;
  chkNormalInv.Enabled := False;
  chkElectronInv.Enabled := False;

  if dtmMain.sG_StarAllUpload then
  begin
    cobBusinessId.Enabled := True;
    chkNormalInv.Enabled := True;
  chkElectronInv.Enabled := True;
  end;
...
var
    ...
    sL_BusinessId,sL_InvoiceKind : Integer;
begin
    sL_TargetDate := '';
    sL_YaerMonth := '';
    if chbInvAndCharge.Checked then
    ...
    if dtmMain.sG_StarAllUpload then
    begin
      if cobBusinessId.ItemIndex = 0 then
        sL_BusinessId := 1
      else
        sL_BusinessId := 2;
    end;
    {#6370 強迫設定成3，因為判斷是否營業人條件取消了 By Kin 2012/12/03}
    sL_BusinessId := 3;
    {#6370 判斷發票種類 By Kin 2012/12/05}
    sL_InvoiceKind := 2;
    if dtmMain.sG_StarAllUpload then
    begin
      if ( not chkNormalInv.Checked ) and ( not chkElectronInv.Checked ) then
      begin
        sL_InvoiceKind := 2;
      end else
      begin
        {#6370 測試不OK,只要勾選電子發票,則選取的字軌只能是UploadFlag=1}
        if ( chkNormalInv.Checked ) and ( chkElectronInv.Checked ) then
        begin
          sL_InvoiceKind := 1;
        end else
        begin
          if  ( chkNormalInv.Checked ) and ( not chkElectronInv.Checked ) then
          begin
            sL_InvoiceKind := 0;
          end else
          begin
    ...
    dtmMainJ.listLastInvDate( dtmMain.getCompID, sL_YaerMonth,sL_BusinessId,sL_InvoiceKind);
end;
...
begin
  ...
  {#6370 判斷是否要選擇營業別 By Kin 2012/11/22}
  {#6370 判斷是否要選擇營業別條件取消 By Kin 2012/12/03}
  if ( dtmMain.sG_StarAllUpload ) and ( 1=0 ) then
  begin
    if ( cobBusinessId.Text = EmptyStr ) then
    begin
      WarningMsg('請選擇營業別。' );
      cobBusinessId.SetFocus;
      Exit;
    end;
  end;
  if ( dtmMain.sG_StarAllUpload ) then
...
var
  ...
  sL_InvType, sL_Email, sL_Contmobile, sL_GiveId, sL_GiveName : String;
  ...
  sL_CombCitemCode,sL_CombCitemName : String;
...
begin
  ...
  DETAIL_DATA_COLUMN_COUNT := 18;   //#5439 加合併的資料 長度2(原本是16,改成18 )
...
//5#13732#14911576#1#1徐洪彩雲#1中壢市志明路2巷20號3樓#1320#1中壢市志明路2巷20號3樓
          ...
          begin
            ...
            if ( L_Master.Count > 16 ) or ( L_Master.Count < 9 ) then
            begin
          ...
          sL_InvType := Trim( L_Master.Strings[11] );
          sL_Email := Trim( L_Master.Strings[12] );
          sL_Contmobile := Trim( L_Master.Strings[13] );
          sL_GiveId := Trim( L_Master.Strings[14] );
          sL_GiveName := Trim( L_Master.Strings[15] );
          ...
          dtmMainJ.InsertInv016( aProcSeq, sL_CompID, sL_CustID, sL_Tel,
            ...
            sL_BeAssignedInvID, sL_IsValid, sL_IsPreCreated,sL_UserName,
            sL_ChargeTitle, aInvUseId, aInvUseDesc,sL_InvType, sL_Email,
            sL_Contmobile, sL_GiveId, sL_GiveName );
        ...
        begin
          ...
          if ( aIsWizText ) then
          begin
            DETAIL_DATA_COLUMN_COUNT := 19;   //#5439 增加收費資料合併項目,所以原本17 改成19
          end;
          ...
          sL_CombCitemCode := Trim(L_Detail.Strings[16]);     //#5439 增加合併項目代碼
          sL_CombCitemName := Trim(L_Detail.Strings[17]);     //#5439 增加合併項目名稱
          sL_AccountNo := EmptyStr;
          if ( DETAIL_DATA_COLUMN_COUNT >= 19 ) then    //#5439 原本是17改成19
          begin
            sL_AccountNo := Trim(L_Detail.Strings[18]);
            {}
          ...
          begin
           ...
           begin
             ...
             sL_CombCitemCode, sL_CombCitemName ) ;
             ...
             begin
               ...
               dtmMainJ.InsertInv016( aNewSeq, sL_CompID, sL_CustID, sL_Tel,
                 ...
                 aInvUseId, aInvUseDesc,sL_InvType,sL_Email,sL_Contmobile,sL_GiveId,sL_GiveName );
               Inc( nL_MasterRecordCount );
...
begin
     //設定權限
     dtmMain.getCompetence('A01',sL_QueryCompetence,sL_InsertCompetence,sL_DeleteCompetence,
...
var
  sL_MaxInvDate, aHaveInvSQL, aNoInvSQL, aOrginal,sL_BusinessIdWhere,sL_InvoiceKind: String;
  nL_InvNo: integer;
begin
  ...
  try                                
    ...
    sL_BusinessIdWhere := EmptyStr;
    nG_FilterBusinessId := 3;
    if dtmMain.sG_StarAllUpload then
    begin
      if cobBusinessId.ItemIndex = 0 then
      begin
        nG_FilterBusinessId := 1;
        sL_BusinessIdWhere := ' AND A.BusinessId IS NOT NULL ';
      end else
      begin
        nG_FilterBusinessId := 2;
        sL_BusinessIdWhere := ' AND A.BusinessId IS NULL ';
      end;
      {#6385 規格異動,營業人條件取消 By Kin 2012/12/05}
      sL_BusinessIdWhere := EmptyStr;
    end;
    sL_InvoiceKind := EmptyStr;
    {#6385 判斷抓取Inv016發票種類 By Kin 2012/12/05}
    if  ( chkNormalInv.Checked ) and ( not chkElectronInv.Checked ) then
    begin
      sL_InvoiceKind := ' AND NVL(A.INVOICEKIND,0) = 0 ';
    end;
    if ( not chkNormalInv.Checked ) and ( chkElectronInv.Checked ) then
    begin
      sL_InvoiceKind := ' AND NVL(A.INVOICEKIND,0) = 1 ';
    end;
    ...
    begin
     ...
     // 未開立發票的查詢畫面資料 SQL
     sG_NoDataSQL := '';
     {#6248 換掉欄位 a.taxrate,a.saleamount, a.taxamount, a.invamount,
     請調整為 (b. taxrate, b.unitprice,  b.taxamount, b.totalamount  By Kin 2012/04/18}
     ...
     begin
       sG_NoDataSQL := Format(
         ...
         '        A.TAXTYPE,                                              ' +
         '        B.TAXRATE,                                              ' +
         '        B.UNITPRICE SALEAMOUNT,                                 ' +
         '        B.TAXAMOUNT,                                            ' +
         '        B.TOTALAMOUNT INVAMOUNT,                                ' +
//         '        A.TAXRATE,                                              ' +
//         '        A.SALEAMOUNT,                                           ' +
//         '        A.TAXAMOUNT,                                            ' +
//         '        A.INVAMOUNT,                                            ' +
...
begin
  ...
  begin
    ...
    SCreen.Cursor := crSQLWait;
    try
      {#6080 後端增加 ShowCMTvMail 參數 By Kin 2011/08/23}
      if dtmMain.GetStarCMTVMail then
        aShowCMTVmail := 1
      else
        aShowCMTVmail := 0;
      {#6370 增加判斷是否要過濾營業別 By Kin 2012/11/23}
      {#6370 增加判斷是否要過濾營業別條件取消,所以nG_FilterBusinessId 強迫設定為3 By Kin 2012/12/03}
      nG_FilterBusinessId := 3;
      {#6370 增加發票種類條件 By Kin 2012/12/05}
      nL_InvoiceKind := 2;
      if ( chkNormalInv.Checked ) and ( not chkElectronInv.Checked ) then
        nL_InvoiceKind := 0;
      if ( not chkNormalInv.Checked ) and ( chkElectronInv.Checked ) then
      ...
      aExecResult := dtmMainJ.assignInvoiceWithSF( nG_TotalUnit, fG_SaleAmount,
        ...
        bL_InvDateEqualChargeDate,dtmMain.GetShowFaci,aShowCMTVmail,nG_FilterBusinessId,nL_InvoiceKind, sL_LogDateTime);
    finally
      SCreen.Cursor := crDefault;
    end;
...
procedure TfrmInvA01.cobBusinessIdExit(Sender: TObject);
begin
  listInvPrefixData;
end;

procedure TfrmInvA01.chkElectronInvExit(Sender: TObject);
begin
  listInvPrefixData;
end;

procedure TfrmInvA01.chkNormalInvClick(Sender: TObject);
begin
  listInvPrefixData;
end;

procedure TfrmInvA01.chkElectronInvClick(Sender: TObject);
begin
  listInvPrefixData;
end;
