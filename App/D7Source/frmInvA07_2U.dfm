object frmInvA07_2: TfrmInvA07_2
  Left = 383
  Top = 159
  BorderStyle = bsDialog
  Caption = #25240#35731#36039#26009#26597#35426
  ClientHeight = 391
  ClientWidth = 424
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 14
  object Panel2: TPanel
    Left = 0
    Top = 350
    Width = 424
    Height = 41
    Align = alBottom
    BevelOuter = bvNone
    TabOrder = 0
    DesignSize = (
      424
      41)
    object Bevel1: TBevel
      Left = 0
      Top = 0
      Width = 424
      Height = 3
      Align = alTop
      Shape = bsTopLine
    end
    object btnOk: TcxButton
      Left = 242
      Top = 10
      Width = 82
      Height = 26
      Anchors = [akTop, akRight]
      Caption = #30906#23450
      Default = True
      TabOrder = 0
      OnClick = btnOkClick
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object btnCancel: TcxButton
      Left = 333
      Top = 10
      Width = 82
      Height = 26
      Anchors = [akTop, akRight]
      Caption = #21462#28040
      ModalResult = 2
      TabOrder = 1
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 424
    Height = 350
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 2
    TabOrder = 1
    object ScrollBox3: TScrollBox
      Left = 2
      Top = 2
      Width = 420
      Height = 346
      HorzScrollBar.Style = ssFlat
      VertScrollBar.Style = ssFlat
      Align = alClient
      BevelInner = bvNone
      BorderStyle = bsNone
      TabOrder = 0
      object cxGroupBox1: TcxGroupBox
        Left = 7
        Top = 8
        Caption = ' '#26597#35426#26781#20214' '
        TabOrder = 0
        Height = 198
        Width = 403
        object Label1: TLabel
          Left = 222
          Top = 26
          Width = 9
          Height = 14
          Caption = '~'
        end
        object Label2: TLabel
          Left = 223
          Top = 81
          Width = 9
          Height = 14
          Caption = '~'
        end
        object Label3: TLabel
          Left = 222
          Top = 109
          Width = 9
          Height = 14
          Caption = '~'
        end
        object Label4: TLabel
          Left = 222
          Top = 137
          Width = 9
          Height = 14
          Caption = '~'
        end
        object Label5: TLabel
          Left = 222
          Top = 166
          Width = 9
          Height = 14
          Caption = '~'
        end
        object rdoCondition1: TcxRadioButton
          Left = 23
          Top = 25
          Width = 75
          Height = 17
          Caption = #21934#25818#26085#26399
          TabOrder = 0
        end
        object rdoCondition2: TcxRadioButton
          Left = 23
          Top = 53
          Width = 75
          Height = 17
          Caption = #30003#22577#24180#26376
          TabOrder = 3
        end
        object txtPaperDateSt: TcxDateEdit
          Left = 95
          Top = 23
          Properties.DateButtons = [btnToday]
          Properties.OnValidate = txtPaperDateStPropertiesValidate
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 1
          OnEnter = txtPaperDateStEnter
          OnExit = txtPaperDateStExit
          Width = 121
        end
        object txtPaperDateEd: TcxDateEdit
          Left = 239
          Top = 23
          Properties.OnValidate = txtPaperDateStPropertiesValidate
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 2
          OnEnter = txtPaperDateStEnter
          Width = 121
        end
        object rdoCondition3: TcxRadioButton
          Left = 23
          Top = 80
          Width = 75
          Height = 17
          Caption = #30332#31080#34399#30908
          TabOrder = 5
        end
        object rdoCondition4: TcxRadioButton
          Left = 23
          Top = 107
          Width = 75
          Height = 17
          Caption = #25240#35731#21934#34399
          TabOrder = 8
        end
        object txtAllowanceNoSt: TcxTextEdit
          Left = 95
          Top = 105
          Properties.MaxLength = 12
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 9
          OnEnter = txtAllowanceNoStEnter
          OnExit = txtAllowanceNoStExit
          Width = 121
        end
        object txtAllowanceNoEd: TcxTextEdit
          Left = 239
          Top = 105
          Properties.MaxLength = 12
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 10
          OnEnter = txtAllowanceNoStEnter
          Width = 121
        end
        object txtYearMonth: TcxMaskEdit
          Left = 95
          Top = 50
          Properties.MaskKind = emkRegExprEx
          Properties.EditMask = '([123][0-9])? [0-9][0-9] / (0?[1-9] | 1[012])'
          Properties.MaxLength = 0
          Properties.OnValidate = txtYearMonthPropertiesValidate
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 4
          OnEnter = txtYearMonthEnter
          Width = 102
        end
        object txtInvIdSt: TcxMaskEdit
          Left = 95
          Top = 77
          Properties.CharCase = ecUpperCase
          Properties.MaskKind = emkRegExpr
          Properties.EditMask = '[a-zA-Z0-9][a-zA-Z0-9]\d\d\d\d\d\d\d\d'
          Properties.MaxLength = 0
          Properties.OnValidate = txtInvIdStPropertiesValidate
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 6
          OnEnter = txtInvIdStEnter
          OnExit = txtInvIdStExit
          Width = 121
        end
        object txtInvIdEd: TcxMaskEdit
          Left = 239
          Top = 77
          Properties.CharCase = ecUpperCase
          Properties.MaskKind = emkRegExpr
          Properties.EditMask = '[a-zA-Z0-9][a-zA-Z0-9]\d\d\d\d\d\d\d\d'
          Properties.MaxLength = 0
          Properties.OnValidate = txtInvIdStPropertiesValidate
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 7
          OnEnter = txtInvIdStEnter
          Width = 121
        end
        object rdoCondition5: TcxRadioButton
          Left = 23
          Top = 135
          Width = 75
          Height = 17
          Caption = #21934#25818#32232#34399
          TabOrder = 11
        end
        object txtPaperNoSt: TcxTextEdit
          Left = 95
          Top = 133
          Properties.MaxLength = 15
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 12
          OnEnter = txtPaperNoStEnter
          OnExit = txtPaperNoStExit
          Width = 121
        end
        object txtPaperNoEd: TcxTextEdit
          Left = 239
          Top = 133
          Properties.MaxLength = 15
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 13
          Width = 121
        end
        object rdoCondition6: TcxRadioButton
          Left = 23
          Top = 164
          Width = 75
          Height = 17
          Caption = #23458'      '#32232
          TabOrder = 14
        end
        object txtCustIdSt: TcxTextEdit
          Left = 95
          Top = 162
          Properties.MaxLength = 8
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 15
          OnEnter = txtCustIdStEnter
          OnExit = txtCustIdStExit
          Width = 121
        end
        object txtCustIdEd: TcxTextEdit
          Left = 239
          Top = 162
          Properties.MaxLength = 8
          Style.StyleController = dtmMain.cxEditStyle
          TabOrder = 16
          Width = 121
        end
      end
      object cxGroupBox2: TcxGroupBox
        Left = 8
        Top = 253
        Caption = ' '#20844#21496#21029' '
        TabOrder = 1
        Height = 42
        Width = 201
        object rdoComp1: TcxRadioButton
          Left = 110
          Top = 17
          Width = 77
          Height = 17
          Caption = #20491#21029#20844#21496
          TabOrder = 0
        end
        object rdoComp2: TcxRadioButton
          Left = 10
          Top = 17
          Width = 77
          Height = 17
          Caption = #25152#26377#20844#21496
          TabOrder = 1
        end
      end
      object cxGroupBox3: TcxGroupBox
        Left = 218
        Top = 253
        Caption = ' '#21934#25818#20358#28304' '
        TabOrder = 2
        Height = 42
        Width = 195
        object rdoSourceAll: TcxRadioButton
          Left = 9
          Top = 17
          Width = 49
          Height = 17
          Caption = #20840#37096
          TabOrder = 0
        end
        object rdoSource0: TcxRadioButton
          Left = 53
          Top = 17
          Width = 72
          Height = 17
          Caption = #33258#34892#36664#20837
          TabOrder = 1
        end
        object rdoSource1: TcxRadioButton
          Left = 119
          Top = 17
          Width = 72
          Height = 17
          Caption = #25209#27425#29986#29983
          TabOrder = 2
        end
      end
      object cxGroupBox4: TcxGroupBox
        Left = 6
        Top = 300
        Caption = #30332#31080#38283#31435#31278#39006
        TabOrder = 3
        Height = 42
        Width = 241
        object rdoInvKind1: TcxRadioButton
          Left = 102
          Top = 17
          Width = 77
          Height = 17
          Caption = '1.'#38651#23376#30332#31080
          TabOrder = 0
        end
        object rdoInvKind0: TcxRadioButton
          Left = 10
          Top = 17
          Width = 91
          Height = 17
          Caption = '0.'#38651#23376#35336#31639#27231
          TabOrder = 1
        end
        object rdoInvKind2: TcxRadioButton
          Left = 182
          Top = 17
          Width = 55
          Height = 17
          Caption = '2.'#20840#37096
          Checked = True
          TabOrder = 2
          TabStop = True
        end
      end
      object cxGroupBox5: TcxGroupBox
        Left = 252
        Top = 300
        Caption = #19978#20659#35387#35352
        TabOrder = 4
        Height = 42
        Width = 161
        object rdoUpdFlag1: TcxRadioButton
          Left = 54
          Top = 17
          Width = 49
          Height = 17
          Caption = '1.'#26377
          TabOrder = 0
        end
        object rdoUpdFlag0: TcxRadioButton
          Left = 6
          Top = 17
          Width = 47
          Height = 17
          Caption = '0.'#28961
          TabOrder = 1
        end
        object rdoUpdFlag2: TcxRadioButton
          Left = 102
          Top = 17
          Width = 55
          Height = 17
          Caption = '2.'#20840#37096
          Checked = True
          TabOrder = 2
          TabStop = True
        end
      end
      object cxGroupBox6: TcxGroupBox
        Left = 10
        Top = 209
        Caption = #25490#24207#26781#20214
        TabOrder = 5
        Height = 42
        Width = 391
        object rdoInvId: TcxRadioButton
          Left = 146
          Top = 17
          Width = 77
          Height = 17
          Caption = #30332#31080#34399#30908
          TabOrder = 0
        end
        object rdoPaperDate: TcxRadioButton
          Left = 10
          Top = 17
          Width = 77
          Height = 17
          Caption = #21934#25818#26085#26399
          Checked = True
          TabOrder = 1
          TabStop = True
        end
        object rdoAllowanceNo: TcxRadioButton
          Left = 296
          Top = 17
          Width = 77
          Height = 17
          Caption = #25240#35731#21934#34399
          TabOrder = 2
        end
      end
    end
  end
end
