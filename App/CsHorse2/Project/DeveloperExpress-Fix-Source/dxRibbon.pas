...
begin
  if not CheckGdiPlus then
    raise Exception.CreateFmt(cxGetResourceString(@dxSBAR_GDIPLUSNEEDED), [AClass.ClassName]);
  if (AOwner = nil) and (Application.MainForm <> nil) then
    AOwner := Application.MainForm;
  if not (AOwner is TCustomForm) then
    raise Exception.CreateFmt(cxGetResourceString(@dxSBAR_RIBBONBADOWNER), [AClass.ClassName]);
  if HasComponentOnForm(TCustomForm(AOwner), AClass) then
    raise Exception.CreateFmt(cxGetResourceString(@dxSBAR_RIBBONMORETHANONE), [AClass.ClassName]);
end;
...
begin
  ...
  if UseAeroNCPaint(AData) then
    ...
    cxTextOut(ACanvas.Handle, PChar(ADocumentName + ACaption), R,
      CXTO_PREVENT_LEFT_EXCEED or CXTO_CENTER_HORIZONTALLY or
      CXTO_CENTER_VERTICALLY or CXTO_SINGLELINE or CXTO_END_ELLIPSIS,
      0, Length(ADocumentName), clNone, ViewInfo.GetDocumentNameTextColor(AData.Active));
end;
...
begin
  ...
  begin
    ...
    Calculate(ABounds);
    if Ribbon.Hidden and (Ribbon.ActiveTab <> nil) then
      Ribbon.ActiveTab.UpdateDockControl;
  end
  else
  begin
...
begin
  if IsQATVisible and IsQATAtBottom and not Ribbon.IsPopupGroupsMode then
...
begin
  ...
  begin
    Result := GetNonClientAreaHeight + TabsHeight + GetTabGroupsHeight;
...
begin
  ...
  begin
    ...
    if (FQATBarControlSize.cx <> 0) and (Ribbon.PopupMenuItems = []) then
    begin
...
begin
  ...
  begin
    Result := (F <> nil) and ( IsIconic(F.Handle) or FForceHide );
  end;
end;
...
procedure TdxRibbonViewInfo.SetSize;
begin
  if Ribbon.Hidden and not Ribbon.IsDesigning then
    Ribbon.Height := GetNonClientAreaHeight
  else
    Ribbon.Height := GetRibbonHeight;
end;
...
begin
  Result := Ribbon.PopupMenuItems - [rpmiItems];
  if not Ribbon.IsQuickAccessToolbarValid then
    Exclude(Result, rpmiQATAddRemoveItem);
end;
...
var
  ...
  B: TcxBitmap;
begin
  B := TcxBitmap.CreateSize(Width, Height);
  try
    AIndex := SaveDC(DC);
    R := ClientRect;
    ...
    begin
      P := cxNullPoint;
      H := Ribbon.FGroupsDockControlSite.Handle;
      Windows.GetWindowRect(H, R);
      OffsetRect(R, -R.Left, -R.Top);
      MapWindowPoint(Handle, H, P);
      Dec(P.X, BR.Left);
      Dec(P.Y, BR.Top);
...
procedure TdxRibbonGroupBarControl.UpdateCaptionButtons(ACaptionButton: TdxBarCaptionButton);
var
  I: Integer;
  AButtonRect: TRect;
  AButtonWidth: Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      MapWindowPoint(Handle, H, APoint);
...
procedure TdxRibbonGroupBarControl.FillParentBackground;
var
  P: TPoint;
  R, BR: TRect;
  H: HWND;
begin
  ...
  begin
    ...
    Ribbon.Painter.DrawGroupsArea(Canvas, R);
    Canvas.RestoreState;
  end;
end;
...
begin
  if (FLoadedToolbarName <> '') and (Tab <> nil) and
    IsAncestorComponentDifferencesDetection(Tab) then
...
begin
  Result := Ribbon.PopupMenuItems - [rpmiQATAddRemoveItem];
end;
...
begin
  ...
  case AHitInfo.HitTest of
    rhtTab:
      if not Ribbon.IsDesigning and (ssDouble in Shift) and AHitInfo.Tab.Active then
...
begin
  ...
  try
    AForm := RibbonForm;
    if (AForm <> nil) and (AForm.FormStyle = fsMDIForm) then
    begin
      AForm := AForm.ActiveMDIChild;
...
begin
  ...
  if (FMouseHook = 0) and not IsDesigning and (IsWin2KOrXP or IsWinVista) then
    FMouseHook := SetWindowsHookEx(WH_MOUSE, @dxRibbonMouseHook, 0, GetCurrentThreadId);
end;
...
begin
  ...
  //text part
  if ViewInfo.UseGlass then
    H := GetSystemMetrics(SM_CYCAPTION) + GetDefaultWindowBordersWidth(RibbonForm.Handle).Top
  else
...
procedure TdxCustomRibbon.DrawApplicationMenuHeader(ADC: THandle; AIsClientArea: Boolean);
  ...
  begin
    ...
    GetWindowRect(ASubMenuControl.Handle, AMenuWindowRect);
...
begin
  AItems := PopupMenuItems - [rpmiItems, rpmiQATAddRemoveItem];
  if not IsQuickAccessToolbarValid then
    Exclude(AItems, rpmiQATAddRemoveItem);
...
procedure TdxCustomRibbon.PopulatePopupMenuItems(ALinks: TdxBarItemLinks;
  ...
  begin
    if QuickAccessToolbar.Contains(BarDesignController.CustomizingItemLink) then
      ...
      AddMenuItem(GetAddMessage, False, rpmiQATAddRemoveItem,
        BarDesignController.CustomizingBarControl).Enabled := GetEnabled;
  end;
...
procedure TdxCustomRibbon.PopupMenuItemClick(Sender: TObject);
  ...
  procedure AddItemToQAT;
    ...
    var
      AGroupButton: TdxRibbonQuickAccessGroupButton;
    begin
      AGroupButton := TdxRibbonQuickAccessGroupButton(QuickAccessToolbar.Toolbar.ItemLinks.AddItem(TdxRibbonQuickAccessGroupButton).Item);
      AGroupButton.Toolbar := GetBar(TCustomdxBarControl(TdxBarItem(Sender).Data));
    end;
...
begin
  if not SupportNonClientDrawing then Exit;
...
begin
  ...
  begin
    ...
    if GroupsDockControlSite.HandleAllocated then
      SendMessage(GroupsDockControlSite.Handle, WM_SETREDRAW, 0, 0);
    try
      if FActiveTab <> nil then
        FActiveTab.Activate;
      Changed;
      ...
      if GroupsDockControlSite.HandleAllocated then
      begin
        SendMessage(GroupsDockControlSite.Handle, WM_SETREDRAW, 1, 0);
        GroupsDockControlSite.InvalidateWithChildren;
      end;
    end;
    Modified;
  end;
...
begin
  AForm := RibbonForm;
  if not IsDesigning and (AForm <> nil) and (AForm.FormStyle = fsMDIForm) then
  begin
...
begin
  ...
  if Result then
    Exit;
  Result := AKey in [VK_DOWN, VK_SPACE];
  if Result and (Ribbon.ApplicationButton.Menu <> nil) then
...
function TdxRibbonApplicationButtonAccessibilityHelper.IsNavigationKey(
  AKey: Word): Boolean;
begin
  Result := inherited IsNavigationKey(AKey) or (AKey in [VK_ESCAPE, VK_SPACE]);
end;
