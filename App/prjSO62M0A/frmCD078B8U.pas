...
begin
  ...
  try
    ...
    FWriter.Close;
    //#5232 測試不OK調整,起迄日要清空
    //#5318 測試不OK,少了計費機制,頻道挑選模式之值  By Kin 2009/10/30
    //#6267 增加SYNCBPCODE欄位 By Kin 2012/07/19
    FWriter.SQL.Text := Format (
      ' INSERT INTO %s.CD078 (                                        ' +
      '   CODENO, DESCRIPTION,                                        ' +
    ...
    //CD078A1, PK 是 StepNo, Copy 功能要在取一次 StepNo, 不然會 PK重覆
    FReader.Close;
    FReader.SQL.Text := Format (
        ...
        [TableOwner, BPCode.Value] );
    FReader.Open;
    {#6945 Period 沒有複制到 By Kin 2014/11/28}
    while not FReader.Eof do
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        aErrMsg := Format( '新代碼 ''%s'' 已經存在了。',[FCloneCodeNo]);
        aControl := edtNewCodeNo;
        Exit;
      end;
    end;
