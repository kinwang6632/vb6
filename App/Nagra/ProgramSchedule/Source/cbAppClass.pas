...
{ TApplication User Define Option, Load from Access }
  ...
  private
    ...
    FFtpPassword: String;
    FFtpDexSrc: String;
    FFtpDexBackup: String;
    FFtpWrapperSrc: String;
    FFtpWrapperBackup: String;
    FFtpAsRunSrc: String;
    FFtpAsRunBackup: String;
  protected
  ...
  public
    ...
    property FtpAccount: String read FFtpAccount write FFtpAccount;
    property FtpPassword: String read FFtpPassword write FFtpPassword;
    property FtpDexSrc: String read FFtpDexSrc write FFtpDexSrc;
    property FtpDexBackup: String read FFtpDexBackup write FFtpDexBackup;
    property FtpWrapperSrc: String read FFtpWrapperSrc write FFtpWrapperSrc;
    property FtpWrapperBackup: String read FFtpWrapperBackup write FFtpWrapperBackup;
    property FtpAsRunSrc: String read FFtpAsRunSrc write FFtpAsRunSrc;
    property FtpAsRunBackup: String read FFtpAsRunBackup write FFtpAsRunBackup;  
...
begin
  ...
  begin
    ...
    TAppParam( Dest ).FtpServer := Self.FtpServer;
    TAppParam( Dest ).FtpAccount := Self.FtpAccount;
    TAppParam( Dest ).FtpPassword := Self.FtpPassword;
    TAppParam( Dest ).FtpDexSrc := Self.FtpDexSrc;
    TAppParam( Dest ).FtpDexBackup := Self.FtpDexBackup;
    TAppParam( Dest ).FtpWrapperSrc := Self.FtpWrapperSrc;
    TAppParam( Dest ).FtpWrapperBackup := Self.FtpWrapperBackup;
    TAppParam( Dest ).FtpAsRunSrc := Self.FtpAsRunSrc;
    TAppParam( Dest ).FtpAsRunBackup := Self.FtpAsRunBackup;
  end else
...
begin
  ...
  begin
    ...
    begin
      ...
      {}
      { 奇怪, 會沒有 Event Id ? }
      a161.EventId := EmptyStr;
      if Assigned( ( aEventNode as IDOMNodeSelect ).selectNode( 'EventId' ) )  then
      begin
...
begin
  ...
  try
    ...
    begin
      if FDataConnection.InTransaction then FDataConnection.RollbackTrans;
      FDataReader.Close;
      aMsg := Format( '%s - %s - %s', ['SO154', E.Message,
        FSo160.FieldByName( 'eventid' ).AsString] );
    end;
  end;
end;
...
begin
  ...
  DoNothing1:
    ...
    on E: Exception do aMsg := Format( '%s - %s', ['SO155Log', E.Message] );
  end;
end;
...
begin
  ...
  try
    ...
    begin
      if FDataConnection.InTransaction then FDataConnection.RollbackTrans;
      aMsg := Format( '%s - %s', ['SO155Log', E.Message] );
    end;
  end;
end;
