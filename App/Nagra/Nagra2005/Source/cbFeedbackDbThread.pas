...
uses
  ...
  {$IFDEF DEBUG} CodeSiteLogging, {$ENDIF} cbClass;
...
begin
  ...
  try
    ...
    FSoInfo.LastCriticalErrorTickCount  := 0;
    FSoInfo.CriticalError := 0;
  except
    on E: Exception do
    begin
      FSoInfo.LastCriticalErrorTickCount := GetTickCount;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        Inc( aWriteError );
        FSoInfo.CriticalError := ( FSoInfo.CriticalError + 1 );
  ...
  if FSoInfo.CriticalError > 0 then
  begin
    FSoInfo.CriticalError := 0;
    FSoInfo.LastCriticalErrorTickCount := GetTickCount;
