...
uses
  ...
  {$IFDEF DEBUG} CodeSiteLogging, {$ENDIF} cbClass, ActiveX;
...
type
  ...
  private
    ...
    FLastGetDataTickCount: Cardinal;
...
uses cbMain, cbResStr, cbUtilis, ComObj, DateUtils;
...
begin
  ...
  aTick := GetTickCount - FLastGetDataTickCount;
...
begin
  ...
  try
    ...
    FCurrentActiveSo.LastCriticalErrorTickCount  := 0;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        FLastGetDataTickCount := GetTickCount;  
      end;
      if Stop then Break;
...
var
  ...
  begin
    ...
    {$IFDEF DEBUG}
      {$IFDEF SQL}
        CodeSite.Send( Format( 'CompCode=%s, SQL',
          [FCurrentActiveSo.CompCode] ), aADO.DataReader.SQL );
      {$ENDIF}
    {$ENDIF}
...
begin
  ...
  begin
    if Stop then Break;
    CodeSite.SendFmtMsg( 'CMD=%s, STATUS=%s, SEQNO=%s',
