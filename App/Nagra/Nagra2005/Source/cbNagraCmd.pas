...
begin
  ...
  aDetailCmdId := GetValue( 'HIGH_LEVEL_CMD', 'LOW_LEVEL_CMD',
    [FPrimalObj.HighCmdId] );
  { 先將高階轉成低階指令 }
...
var
  aTransNum, aCmdType, aSourceId, aDestId, aMopPpid, aCreateDate: String;
...
begin
  ...
  begin
    ...
    aMopPpid := Lpad( FCASEnv.FeedbackMopPPId, 5, '0' );
  end else
  begin
    ...
    aMopPpid := Lpad( FCASEnv.ControlMopPPId, 5, '0' );
  end;
  ...
  Result := aTransNum + aCmdType + aSourceId + aDestId + aMopPpid + aCreateDate;
end;
...
begin
  ...
  begin
    ...
    if aAdrType = 'U' then
      aSection := ( aSection + Rpad( Copy( aObj.IccNo, 1, 10 ), 10, '0' ) );
  end else
  { Control }
  if ( aCmdType = '02' ) then
  begin
...
begin
  ...
  { E2=傳送訊息, E11= Pop up 訊息 }
  if ( ( aObj.HighCmdId = 'E2' ) or ( aObj.HighCmdId = 'E11' ) ) then
  begin
    aBigNum := StrToInt( FTemporaryList.Strings[aBaseIndex] );
