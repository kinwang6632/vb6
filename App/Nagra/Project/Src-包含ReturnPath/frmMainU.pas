...
uses
  ...
  syncobjs,   UdateTimeu, Sockets, ConstObjU, Provider, ReturnPathSvrU, ProcessReturnPathDataU;
...
type
  ...
  TfrmMain = class(TForm)
    ...
    procedure TcpServer11Accept(Sender: TObject;
  ...
  private
    ...
    G_ReturnPathSvr_ExitCode : Cardinal;
...
//    G_ReturnPathClntSocket: TClientSocket;    
    ...
    bG_HandleUIThread, bG_HandleResponseThread, bG_RunFetchCommandThread, bG_RunReceivingCommandThread,bG_RunSendingCommandThread, bG_ReturnPathSvrThread, bG_ProcessReturnPathDataThread : boolean;
...
begin
    ...
    if GetExitCodeThread(G_ReturnPathSvr.ThreadID,G_ReturnPathSvr_ExitCode) then
    begin
      if not bG_AutoInvoke then
...
{
      if Assigned(G_ReturnPathClntSocket) then
      begin
        G_ReturnPathClntSocket.Close;
        G_ReturnPathClntSocket := nil;
      end;
}
...
begin
    ...
    begin
      if G_ReturnPathSvr<> nil then
       (G_ReturnPathSvr as ReturnPathSvr).buildConnection(ReturnPathClntSocket, G_ReturnPathWStream);
//         (G_ReturnPathSvr as ReturnPathSvr).buildConnection(ReturnPathClntSocket);
    end;
...
begin
    G_ReturnPathSvr.sendCmd('1000', '123');
end;

procedure TfrmMain.TcpServer11Accept(Sender: TObject;
  ClientSocket: TCustomIpClient);
var
...
begin
    ...
    //down, for return path...
    if Assigned(G_ReturnPathWStream) then
    begin
      G_ReturnPathWStream.Free;
      G_ReturnPathWStream := nil;
    end;



    if (ReturnPathClntSocket.Active) then
      ReturnPathClntSocket.Close;
    //up, for return path...
...
{
procedure TFormprocedure TfrmMain.Button6Click(Sender: TObject);
begin
