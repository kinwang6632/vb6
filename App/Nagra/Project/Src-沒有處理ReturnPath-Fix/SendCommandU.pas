...
uses
  ...
  IniFiles, IdGlobal, ConstObjU, Ustru, UdateTimeu, CsIntf;


    procedure OnTimerStartAction;
    procedure BuildSendCmdConnection(AClientSocket : TClientSocket);overload;
...
begin
    ...
    sL_EventName := TUstr.AddString( I_CmdInfoObj.EVENT_NAME , ' ', True,
      EVENT_NAME_LENGTH );

    sL_EventNameLength := Format( '%.2d',[Length( sL_EventName )] );
    ...
    sL_Price := Format( '%.3d00', [StrToIntDef( I_CmdInfoObj.PRICE, 0 )] );
   ...
   sL_CleanDate := TUstr.replaceStr( sL_DateFirstCall, '/', '' );
...
begin
    ...
    begin
      ...
      L_Socket := frmMain.SendCmdSocket;

      L_WStream := TWinSocketStream.Create( L_Socket.Socket , 10000 );
      try
        writeSendCmdLog(sI_CommandNo);
        try
          if frmMain.bG_HasBuildSendCmdConnection  and Assigned(L_WStream) then
          begin
            nL_Words := L_WStream.Write(L_DeviceData, nL_Len+2);
            //CodeSite.SendInteger( csmMemoryStatus, 'Send Byte', nL_Words );
            CodeSite.SendMsg( csmInfo, 'Command ID=' + sI_CommandNo );
            CodeSite.SendMsg( csmInfo, 'Command Body=' + sL_TmpFullCommand );
...
var
    ...
    sL_RealTotalSegment, aTemp, aTemp2: String;
    sL_IrdCmdID, sL_IrdOperation, sL_IrdDataLength, sL_IrdData: String;
begin
...
begin
  ...
  case aIrdCmdId of
     1: { 親子密碼 }
       begin
         ...
         aIrdData := Nvl( aMisIrdCmdData, '0000' );
     ...
     3: { Force Tune 強制換台 }
       begin
         ...
         begin
           ...
           aRealIrdData := ( aIrdData + aIrdData );
           { Calc service_id ---> 16 進制 }
           aIrdData:= IntToHex( StrToInt( aMisIrdCmdData ), 4 );
           { 每一個 16 進制字元 ---> ASCII }
           for aIndex := 1 to Length( aIrdData ) do
             aRealIrdData := ( aRealIrdData + IntToStr( Ord( aIrdData[aIndex] ) ) );
         end;
  ...
  aIrdInfoObj.s_IrdDataLength := Lpad( IntToStr( Length( aRealIrdData ) div 2 ), 2, '0' );
...
{ ---------------------------------------------------------------------------- }
procedure BuildSendCmdConnection(AClientSocket: TClientSocket);
const
...
begin
  ...
  try
    ...
    AClientSocket.Port := frmMain.getSendCommandPort;
    if not AClientSocket.Active then
      AClientSocket.Open;
    ...
    try
      ...
      AWStream.Read( ADeviceAnswer, SizeOf( ADeviceAnswer ) );
      {
      aReadOK := False;
      aReadCount := 0;
      while not aReadOK do
      begin
        Sleep( 2000 );
        Inc( aReadCount );
        try
          ...
          //AWStream.Read( ADeviceAnswer, SizeOf( ADeviceAnswer ) );
          aReadOK := True;
          frmMain.memErrorLog.Lines.Add( 'Build Connection OK' );
        except
          aReadOK := False;
        end;
        if ( aReadCount >= 10 ) and ( aReadOK = False ) then
    ...
    begin
      frmMain.memErrorLog.Lines.Add( 'NAGRA Reject the connection.' );
    end
    else begin
      frmMain.memErrorLog.Lines.Add( 'NAGRA Accet the connection.' );
