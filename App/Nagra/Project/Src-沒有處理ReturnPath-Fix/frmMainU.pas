...
uses
  ...
  DBClient, Math, scktcomp, Buttons, IniFiles, Winsock,
...
var
  AInfo: String;
begin
  AInfo := GetLanguageSettingInfo('frmMain_Msg_25_Content');

  G_ReturnPathSvr := ReturnPathSvr.Create;

  {
  if GetExitCodeThread( G_ReturnPathSvr.ThreadID,
    G_ReturnPathSvr_ExitCode ) then
  begin
    if not bG_AutoInvoke then
      MessageDlg('執行緒 7 產生失敗!' + AInfo, mtError, [mbOK],0 );
    Application.Terminate;
    Exit;
  end;
  }
  ...
  {
  if GetExitCodeThread( G_ProcessReturnPathData.ThreadID,
    G_ProcessReturnPathData_ExitCode ) then
  begin
    if not bG_AutoInvoke then
      MessageDlg('執行緒 8 產生失敗!' + AInfo, mtError, [mbOK],0 );
    Application.Terminate;
    Exit;
  end;
  }
...
begin
    ...
    begin
      ...
      ReturnPathClntSocket.Active := False;
...
begin
  ...
  begin
    SendCommandU.BuildSendCmdConnection( SendCmdSocket );
  end;
end;
...
begin
  ...
  begin
    ...
    if G_ReturnPathSvr <> nil then
     ( G_ReturnPathSvr as ReturnPathSvr ).BuildConnection(
       ReturnPathClntSocket, G_ReturnPathWStream );
  end;
end;
