...
type
  SlaveReturnPathSvr = class(TThread)
  private
  ...
  public
    ...
    G_SocketStream : TWinSocketStream;
    ...
    constructor Create(bI_CreateSocketStream, bI_Suspended : boolean);overload;    
...
procedure SlaveReturnPathSvr.AddReturnPathData;
var
...
procedure SlaveReturnPathSvr.AddReturnPathData(I_DeviceData: TDeviceData);
var
    L_ReturnPathDataObj : TReturnPathDataObj;
begin
...
procedure SlaveReturnPathSvr.AddReturnPathData(sI_CallbackData: String);
begin
    frmMain.G_ReturnPathDataStrList.Add(sI_CallbackData);

end;

procedure SlaveReturnPathSvr.buildConnection(I_ClientSocket: TClientSocket;
...
constructor SlaveReturnPathSvr.Create;
begin
    inherited Create(false);
end;

constructor SlaveReturnPathSvr.Create(bI_CreateSocketStream,
  bI_Suspended: boolean);
begin
    if bI_CreateSocketStream then
      G_SocketStream := TWinSocketStream.Create(frmMain.ReturnPathClntSocket.Socket, 60000);

//    inherited Create(bI_Suspended);
...
procedure SlaveReturnPathSvr.Execute;
begin
...
function SlaveReturnPathSvr.Increment(Ptr: Pointer;
  Increment: Integer): Pointer;
begin
    Result := Pointer(Longint(Ptr) + Increment);
end;

function SlaveReturnPathSvr.isValidCmd(sI_Cmd: String): boolean;
begin
    //§PÂ_¬O§_¬O¥¿½Tªº«ü¥O®æ¦¡
    result := true;
...
procedure SlaveReturnPathSvr.realExecute;
var
    ...
    L_AryOfDeviceData : array[0..4] of TDeviceData;
begin
    try
      if not frmMain.ReturnPathClntSocket.Socket.connected then Exit;

//      FillChar(L_DeviceData.data, length(L_DeviceData.data), 0); { initialize the buffer }

      for ii:=0 to high(L_AryOfDeviceData) do
      begin
        FillChar(L_AryOfDeviceData[ii].len, length(L_AryOfDeviceData[ii].len), 0); { initialize the buffer }
        FillChar(L_AryOfDeviceData[ii].data, length(L_AryOfDeviceData[ii].data), 0); { initialize the buffer }
      end;
      {
...
//          Stream := TWinSocketStream.Create(frmMain.ReturnPathClntSocket.Socket, 60000);
          Stream := G_SocketStream;


          try
            { give the client 20 seconds to start writing }
            if Stream.WaitForData(60000) then
            begin
//              if not frmMain.bG_MasterReturnPathSvrIsAvailable then
//              begin
//                Stream.Read(L_DeviceData, sizeof(L_DeviceData));
              for ii:=0 to high(L_AryOfDeviceData) do
              begin
                nL_Words := Stream.Read(L_AryOfDeviceData[ii], sizeof(L_AryOfDeviceData[ii]));
                if nL_Words>0 then
                  frmMain.G_ReturnPathDataStrList1.Add('words=>' + IntToStr(nL_Words));
                if L_AryOfDeviceData[ii].len<>'' then
                  frmMain.G_ReturnPathDataStrList1.Add('len=>' + L_AryOfDeviceData[ii].len);
                if L_AryOfDeviceData[ii].data<>'' then
                  frmMain.G_ReturnPathDataStrList1.Add('S' + Format('%.10d',[self.ThreadID])+ ':' + L_AryOfDeviceData[ii].data);
              end;
...

{
                ...
                {
                if frmMain.nG_ActivedSlaveReturnPathSvr=SLAVE_RETURN_PATH_SVR_COUNT-1 then
                  frmMain.nG_ActivedSlaveReturnPathSvr := 0
                else
                  Inc(frmMain.nG_ActivedSlaveReturnPathSvr);
...
{
                if Assigned(frmMain.G_ReturnPathDataStrList1) then
                  frmMain.G_ReturnPathDataStrList1.Add('S' + Format('%.10d',[self.ThreadID])+ ':' + L_DeviceData.data);
0509}
//                  frmMain.G_ReturnPathDataStrList1.Add('S' + IntToStr(frmMain.nG_ActivedSlaveReturnPathSvr)+ ':' + L_DeviceData.data);
...
procedure SlaveReturnPathSvr.receiveReturnPathData;
var
...
procedure SlaveReturnPathSvr.receiveReturnPathData1;
var
    Stream : TWinSocketStream;
    L_DeviceData : TDeviceData;
begin
...
procedure SlaveReturnPathSvr.sendCmd(sI_CmdID, sI_SrcTransNum: String);
var
...
function SlaveReturnPathSvr.Write(I_Buf: Pointer; I_Count: Integer;
