...
begin
  ...
  //    sI_DbConnStr := 'Provider=OraOLEDB.Oracle.1;Password=' + sI_DbPassword +';Persist Security Info=True;User ID='+sI_DbUserName+';Data Source='+sI_DbAlias; //=> 適用於my nb
      ...
      result := '資料庫連線失敗!請檢查ini檔之設定!' + ' Alias:'+ sL_DbAlias + '  UserID:' +sL_DbUserName;
    end;
end;
...
begin
  ...
  //        Close;
      ...
      frmMain.showLogOnMemo(EXECUTE_ORACLE_SP_ERROR_FLAG,'後端程式 : SF_INSERTCACALLBACKDATA' + ' .程式即將結束,然後重新啟動. '); //執行完此行程式後, 程式會結束掉
...
begin
  ...
  //        Close;
      ...
      frmMain.showLogOnMemo(EXECUTE_ORACLE_SP_ERROR_FLAG,'後端程式 : SF_INSERTSENDCMDLOG' + ' .程式即將結束,然後重新啟動. '); //執行完此行程式後, 程式會結束掉
//      reConnectCallbackSvr;
    end;
...
begin
  ...
  //        Close;
      ...
      frmMain.showLogOnMemo(EXECUTE_ORACLE_SP_ERROR_FLAG,'後端程式 : SF_INSERTCMDRESPONSELOG' + ' .程式即將結束,然後重新啟動. '); //執行完此行程式後, 程式會結束掉
//      reConnectCallbackSvr;
