...
const
    ...
    { 暫時給 LAB 測試用 }
    WAREHOURSE_SUPER_USER_ID = 'lab';
    WAREHOURSE_SUPER_USER_PASSWD = 'lab0630';
    STR_SEP = '''';
    MAX_ACTION_TIMES = 2;
    ...
    case nI_ActionType of
      ...
      3://開頻道
       begin
         ...
         sL_AuthorStartDate := getOracleSQLDateStr( aUTCTime );
...
function TfrmMain.CanAppRun: Boolean;
var
  aReg: TRegistry;
  aInstallDate: String;
begin
  ...
  try
    ...
    Result := aReg.KeyExists( 'SOFTWARE\LABTEST' );
    if not Result then Exit;
    aReg.OpenKey( 'SOFTWARE\LABTEST', False );
    aInstallDate := aReg.ReadString( 'INSTALL' );
    aReg.CloseKey;
    if aInstallDate = EmptyStr then
    begin
      Result := False;
      Exit;
    end;
    if ( Date - StrToDate( aInstallDate ) ) > 180 then
    begin
      Result := False;
      Exit;
    end;
