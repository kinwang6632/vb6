...
uses
  Classes, SysUtils, Dialogs, ExtCtrls, ConstObjU, SyncObjs, CsIntf, Forms;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              { 假如 1002 Ack 回來的是 1001, 則抓出資料後, 不送到 NAGRA }
              { 必須等 1002 Ack 回來的是 1000, 才可以送指令 }

              if _NAGRA_REJECT then
              begin
                nL_Index := 1;
                if sL_SeqNos = '' then
              ...
              begin
                ...
                L_CmdInfoObj.STB_FLAG := Trim(FieldByName('STB_FLAG').AsString);
                }
                L_CmdInfoObj.STB_FLAG := '0';
            ...
            try
              ...
              begin
                { 不是第一次啟動 }
                if not _FIRST_START then UpdateCmdStatusToProcessing( sL_SeqNos );
              end
              else begin
          ...
          begin
            ...
            if frmMain.bG_FirstRun then
            begin
              bL_SendTestCmd := True;
              frmMain.bG_FirstRun := False;
            end else
            begin
              bL_SendTestCmd := False;
              { 每 10 秒送一次, 因為 timer 在尖峰時段是每 3 秒 觸發一次, 所以在這
                邊計算 }
              Inc( aDelayNoCommand );
              if ( aDelayNoCommand >= 3 ) then
              begin
                bL_SendTestCmd := True;
                aDelayNoCommand := 0;
              end;
