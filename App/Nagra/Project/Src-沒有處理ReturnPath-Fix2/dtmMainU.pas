...
begin
  ...
  CodeSite.SendStringList( 'SQL', G_AdoSendNagra[nI_ActiveConn].SQL );
...
var
  ...
  aValue: String;
begin
  { 假如拒絕連線, 則一律把該筆資料標示成錯誤 }
  if _NAGRA_REJECT then
  begin
    while not ( sI_SeqNos <> EmptyStr ) do
    begin
      aValue := ExtractValue( sI_SeqNos );
      G_AdoTmp[nI_ActiveConn].SQL.Clear;
      G_AdoTmp[nI_ActiveConn].SQL.Text :=
        ' UPDATE SEND_NAGRA                                         ' +
        '    SET CMD_STATUS = ''E'',                                ' +
        '        ERR_CODE = ''0000--FATAL_ERROR'',                  ' +
        '        ERR_MSG = ''0048--EXTERNAL_SYSTEM_NOT_RESPONDING'' ' +
        '  WHERE SEQNO = ' + QuotedStr( aValue );
       G_AdoTmp[nI_ActiveConn].ExecSQL;
    end;
  end else
  begin
