...
uses
  Windows, SysUtils, Classes, Forms, Sockets, CsIntf, ConstObjU, SendCommandU;
...
type
  ...
  public
    ...
    procedure BuildConnection(const aSocket : TTcpClient);
  end;
...
procedure ReturnPathSvr.BuildConnection(const aSocket : TTcpClient);
const
...
begin
  ...
  try
    ...
    aSocket.RemotePort := IntToStr( frmMain.getReturnPathPort );
    aSocket.Open;
  ...
  begin
    try
      ...
      if ( not Self.Terminated ) and
         ( frmMain.FeedbackSocket.Active ) then
      begin
...
begin
     ...
     if frmMain.FeedbackSocket.WaitForData( 10000 ) then
     begin
       if Self.Terminated then Exit;
       AReadBytes := frmMain.FeedbackSocket.ReceiveBuf( ARecvAry, SizeOf( ARecvAry ) );
...
begin
    try
        ...
        frmMain.FeedbackSocket.SendBuf( L_DeviceData, Length( sL_FullCommand ) + 2 );
