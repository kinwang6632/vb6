...
begin
    ...
    begin
      ...
      begin
        //錯, 只要是 1001, 一定是錯的指令 回傳command 1000 或回傳之錯誤代碼為0者,均視為OK
        //if ( nL_ResCmdID = ACKNOWLEDGE_CMD_ID) or
        //   ( nL_TmpResponseErrorCode = 0 ) then
        if ( nL_ResCmdID = ACKNOWLEDGE_CMD_ID) then
        begin
      ...
      begin
        ...
        begin
          nL_UiStatus := 2;
          _NAGRA_REJECT := False;
         if frmMain.memErrorLog.Lines.Count >= CA_UI_MAX_COUNT then
           frmMain.memErrorLog.Lines.Clear;
          frmMain.memErrorLog.Lines.Add(
            '['+ DateTimeToStr(now)+']' + ' NAGRA 接收指令傳送.' );
        end
        else begin
          nL_UiStatus := 3;
          _NAGRA_REJECT := True;
         if frmMain.memErrorLog.Lines.Count >= CA_UI_MAX_COUNT then
           frmMain.memErrorLog.Lines.Clear;
          frmMain.memErrorLog.Lines.Add(
            '['+ DateTimeToStr(now)+']' + ' NAGRA 拒絕指令傳送.' );
        end;  
      end;
