...
begin
    ...
    if RadioGroup1.ItemIndex = 0 then
    begin
      if edtStbNo.Text='' then
      begin
        bL_Result := false;
        MessageDlg(frmMain.getLanguageSettingInfo('Sim_CA_Msg_23_Content'), mtWarning, [mbOK],0);
        edtStbNo.SetFocus;
      end
      else
        sI_Cond := ' STB_NO=' + STR_SEP + Trim(edtStbNo.Text) + STR_SEP + ' ';
    end
    else if RadioGroup1.ItemIndex = 1 then
    begin
      ...
      begin
        bL_Result := false;
        MessageDlg(frmMain.getLanguageSettingInfo('Sim_CA_Msg_24_Content'), mtWarning, [mbOK],0);
        edtIccNo.SetFocus;
      end
      else
...
begin
    ...
    begin
      sL_SQL := 'select * from CaWareHouseLog where ' + sL_Cond + ' order by UPDTIME ,ACTION';
      nL_RecordCount := frmMain.activeReport1Data(sL_SQL);
      if nL_RecordCount=0 then
      begin
        MessageDlg(frmMain.getLanguageSettingInfo('Sim_CA_Msg_25_Content'), mtInformation, [mbOK],0);
...
procedure TfrmReport1.setLanguageString;
begin
    Caption := frmMain.getLanguageSettingInfo('frmSimCA_2_Caption');
    btnPrint.Caption := frmMain.getLanguageSettingInfo('Sim_CA_btnPrint_Caption');
