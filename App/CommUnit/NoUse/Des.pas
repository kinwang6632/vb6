...
function EncryStrHex(Str, Key: string): string; //以十六?制加密(有些?侯加密的
//密文含有特殊字符文本框不能?示、?据?文本字段也不能存?。以十六?制形式表示可
//保???不?出??种情?)
function DecryStr(Str, Key: string): string; //解密
function DecryStrHex(StrHex, Key: string): string; //解密十六?制加密的密文
...
begin
  ...
  for i := 0 to 5 do outBuf[i] := outBuf[i] xor subKey[i];
                                                // outBuf       xxxxxxxx xxxxx
//xxx xxxxxxxx xxxxxxxx xxxxxxxx xxxxxxxx
  buf[0] := outBuf[0] shr 2; //xxxxxx -> 2
  buf[1] := ((outBuf[0] and $03) shl 4) or (outBuf[1] shr 4); // 4 <- xx xxxx
//-> 4
  buf[2] := ((outBuf[1] and $0F) shl 2) or (outBuf[2] shr 6); //        2 <- x
//xxx xx -> 6
