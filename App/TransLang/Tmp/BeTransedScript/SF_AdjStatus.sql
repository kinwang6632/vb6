...
BEGIN
  ...
  exception
    when others then
      p_RetMsg := 'CD005無停機復機的代碼(RefNo=7)';
      return -2;
  end;
  ...
  for rwa1 in wa1 loop
	...
	values (v_RcdNo, rwa1.CustId, 11, rwa1.SNo, '裝機檔有未完工單子,裝機未完工log檔卻無資料. 派工類別='||v_WorkCode||',參考號='||v_RefNo1);
  end loop;
  ...
  for rwb1 in wb1 loop
    ...
    v_RcdNo := v_RcdNo + 1;
    insert into SO080 (RcdNo, CustId, Type, SNo, Content)
	values (v_RcdNo, rwb1.CustId, 21, rwb1.SNo, '維修檔有未完工單子,維修未完工log檔卻無資料. 派工類別='||v_WorkCode||',參考號='||v_RefNo2);
  end loop;
  ...
  for rwc1 in wc1 loop
    ...
    v_RcdNo := v_RcdNo + 1;
    insert into SO080 (RcdNo, CustId, Type, SNo, Content)
	values (v_RcdNo, rwc1.CustId, 13, rwc1.SNo, '停拆移機檔有未完工單子,停拆移機未完工log檔卻無資料. 派工類別='||v_WorkCode||',參考號='||v_RefNo3);
  end loop;
  ...
  for cr1 in cc1 loop
    ...
    -- *************************************************************
    -- (14)若有拆機中工單, 則調整客戶狀態為'拆機中', 否則: 若非註銷戶, 才決定新的客戶狀態
    -- 否則: no action
    v_NewTime := null;
    v_NewStatus := 5;
	...
	values (v_RcdNo, v_CustId, 0, null, '客戶狀態: 由 '||cr1.CustStatusCode||' 改為 6(拆機中)');
      v_Cmd := v_Cmd || ',CustStatusCode=6,CustStatusName='||chr(39)||'拆機中'||chr(39);
  ...
  p_RetMsg := '執行完畢, 共花費'||to_char(trunc(86400*(v_StopExecTime-v_StartExecTime)))||'秒';
  commit;
  return v_RcdNo;
