...
begin
  ...
  if v_FinTime is not null then
    p_RetMsg := '派工單已完工結案, 不可退單結案';
    return -3;
  end if;

  if v_ReturnCode is not null then
    p_RetMsg := '派工單已退單結案, 不可退單結案';
    return -4;
  end if;
  ...
  if v_Count > 0 then
    p_RetMsg := '派工單所附之設備資料登錄為已安裝, 不可退單結案';
    return -5;
  end if;
  ...
  if v_Count > 0 then
    p_RetMsg := '派工單所附之收費資料已登錄或已作廢, 不可退單結案';
    return -6;
  end if;
  ...
  select count(*) into v_Count from SO034
    where BillNo=p_SNo;
  if v_Count > 0 then
    p_RetMsg := '派工單所附之收費資料登錄為已日結, 不可退單結案';
    return -7;
  end if;
  ...
  select count(*) into v_Count from SO035
    where BillNo=p_SNo;
  if v_Count > 0 then
    p_RetMsg := '派工單所附之收費資料登錄為已日結(作廢), 不可退單結案';
    return -8;
  end if;
