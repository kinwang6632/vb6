...
CREATE OR REPLACE FUNCTION SF_GetAddrMap3
  (p_StrtCode number, p_AddrSort IN VARCHAR2, 
   ...
   p_MduName OUT VARCHAR2,
   p_CompCode out number,
   p_CompName out varchar2) RETURN NUMBER
  as
...
begin
  -- Check parameters
  if p_StrtCode is null or p_AddrSort is null then
    return -1;
  end if;
  ...
  if v_Flag = 1 then
	...
	MduId, MDUName, CompCode, CompName
	...
	v_MduId, v_MduName, v_CompCode, v_CompName
      from SO016 where StrtCode=p_StrtCode and Ord=v_Ord;
    ...
    p_MduId     := v_MduId;
    p_MduName   := v_MduName;
    p_CompCode	:= v_CompCode;
    p_CompName	:= v_CompName;
