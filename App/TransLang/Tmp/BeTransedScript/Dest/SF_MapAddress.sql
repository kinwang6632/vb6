...
create or replace function SF_MapAddress
  ...
  -- 所有地址資料
  cursor cc1 is
    select * from SO014 order by AddrSort;
    --select * from SO014 order by StrtCode;
...
begin
  ...
  --dbms_output.put_line('<1> '||to_char(trunc(86400*(sysdate-v_StartExecTime)))||'second');
 
  -- loop執行存於待異動地址檔中(Tmp002)的資料
