...
create or replace function SF_CanDel3
...
begin
  ...
  if v_FinTime is not null then
    p_RetMsg := 'Cannot delete a finished w/o.';
    return -3;
  end if;
 
  if v_ReturnCode is not null then
    p_RetMsg := 'Cannot delete a returned w/o.';
    return -4;
  end if;
  ...
  if v_Count > 0 then
    p_RetMsg := 'Cannot delete, since associated charge data is entered or cancelled.';
    return -6;
  end if;
