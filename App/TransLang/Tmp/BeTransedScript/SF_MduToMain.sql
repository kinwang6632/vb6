...
begin
  ...
  exception
    when no_data_found then
      p_RetMsg := '無此使用者代號: '||chr(39)||p_UserId||chr(39);
      return -6;
  end;
  ...
  /*
	...
	c.街道安裝戶數+1, 大樓有效戶數+1, 大樓停拆/待拆戶數-1
  */
  if p_Option1 = 1 or p_Option2 = 1 then	  -- 取'正常'客戶狀態名稱
  ...
  if p_Option1 = 1 then		-- 出自動結案裝機單(拆機復機單), 先取相關代碼與名稱值
    ...
    if v_InstCode is null then
      p_RetMsg := 'CD005檔: 無參考號=5(拆機復機)資料';
      return -2;
    end if;
  end if;
...
--DBMS_OUTPUT.PUT_LINE(v_CustId||', 將拆機單退單: '||v_SNo);
  ...
  */
  ...
  commit;
  p_RetMsg := '執行完畢, 處理戶數 = '||to_char(v_Count);
  return 0;
