...
begin
     //設定權限
     dtmMain.getCompetence('D02',sL_QueryCompetence,sL_InsertCompetence,sL_DeleteCompetence,sL_UpdateCompetence,sL_ExecuteCompetence);
...
var
    ...
    sL_SQL : String;
    bL_HavePKValue, aIfPrintTitle : Boolean;
begin
    if isDataOK then
    begin
      sL_CustId := Trim(medCustId.Text);
...
begin
  ...
  dtmMain.adoComm.SQL.Text := Format(
    ...
    '    AND COMPID = ''%s''       ' +
    '    AND CUSTID = ''%s''       ',
    [IDENTIFYID1, IDENTIFYID2, dtmMain.getCompID, medCustId.Text] );
