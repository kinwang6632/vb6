...
uses
  ...
  cbClass, cbNagraCmd, cbNagraDevice,Forms,
...
begin
  ...
  begin
    ...
    if Stop then Break;
    { 超過 10 秒仍然等不到 OK 信號 }
    aTimeout := ( SecondsBetween( Now, aPreSecond ) >= 10 );
...
//        aManager.Items[aIndex].BeginRead;
        aManager.Items[aIndex].BeginWrite;  //By Kin
        try
          ...
          begin
            ...
            aManager.Items[aIndex].Delete(aLowObjIndex); // By Kin
          end;
        finally
          //aManager.Items[aIndex].EndRead;
          aManager.Items[aIndex].EndWrite;     // By Kin
        end;
