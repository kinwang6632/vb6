...
var
  aFrm : TfrmDbSelect ;
begin
  aFrm := TfrmDbSelect.Create(Application) ;
...
procedure TfrmDbSelect.SetDisplayFields(const Value: string);
begin
  if Value = '' then Exit ;
...
procedure TfrmDbSelect.SetDisplayCaptions(const Value: string);
begin
  if Value = '' then Exit ;
  {parese the input string to get all display fields}
...
procedure TfrmDbSelect.FormClose(Sender: TObject; var Action: TCloseAction);
var
...
procedure TfrmDbSelect.FormShow(Sender: TObject);
var
  ii, wid : Integer ;
  fld : TField ;
  clm : TColumn ;
...
procedure TfrmDbSelect.dbgDataKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmDbSelect.dbgDataDblClick(Sender: TObject);
begin
    btnOkClick(Sender);
//  ModalResult := mrOk ;
end;

procedure TfrmDbSelect.SetDelete(const Value: TNotifyEvent);
begin
end;

procedure TfrmDbSelect.SetNew(const Value: TNotifyEvent);
begin
end;

procedure TfrmDbSelect.SetUpdate(const Value: TNotifyEvent);
begin
end;

procedure TfrmDbSelect.SetFunc(fNew, fUpdate, fDelete: TNotifyEvent);
begin
  NewFunc := fNew;
  UpdateFunc := fUpdate;
  DeleteFunc := fDelete;
...
procedure TfrmDbSelect.AddFunc(sCaption: string; Callback: TNotifyEvent;
  Modal: TModalResult);
var
  Btn: TButton;
begin
...
procedure TfrmDbSelect.SetDataSet(const Value: TDataSet);
begin
  FDataSet := Value;
  dsrData.DataSet := FDataSet;
end;

procedure TfrmDbSelect.FormCreate(Sender: TObject);
begin
  FLeftPos := 0;
end;

procedure TfrmDbSelect.SetConfirmState(EnableOk, EnableCanCel: Boolean;
  OKCap, CancelCap: string);
begin
...
procedure TfrmDbSelect.btnQueryClick(Sender: TObject);
var
...
procedure TfrmDbSelect.btnOkClick(Sender: TObject);
begin
...
procedure TfrmDbSelect.edtKeyKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmDbSelect.SetDefaultQryStr(const Value: String);
begin
  FDefaultQryStr := Value;
end;

procedure TfrmDbSelect.SpeedButton1Click(Sender: TObject);
begin
    edtKey.Text := '';
    btnQueryClick(Sender);
end;

procedure TfrmDbSelect.btnCancelClick(Sender: TObject);
begin
    cancelFilter;

end;

procedure TfrmDbSelect.cancelFilter;
begin
    if dsrData.DataSet is TClientDataSet then
    begin
      with (dsrData.DataSet As TClientDataSet) do
...
procedure TfrmDbSelect.dbgDataTitleClick(Column: TColumn);
var
   ii : Integer;
   bFlag : Boolean;
   FDataSet : TDataSet;
