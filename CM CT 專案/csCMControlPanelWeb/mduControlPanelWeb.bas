...
Public Function Call_CMCP_WebCmd(strID_Code As String, _
                                                            ...
                                                            Optional strCP_No As String = "", Optional strClass_ID As String = "", _
                                                            Optional strPC_IP_NO As String = "", Optional strPC_IP As String = "", _
                                                            ...
                                                            Optional ByRef strErrMsg As String = "", Optional ByRef objResult As Object, _
  ...
  On Error GoTo ChkErr
    ...
    With objWebCmd
            Call_CMCP_WebCmd = .Web_Command(gcnGi.ConnectionString, _
                                                                            ...
                                                                            strNodeNo, strCircuitNo, _
                                                                            frmSO1623A.gdtResvTime.Text)
            strErrMsg = .GetErrMsg
    End With
...
'    MsgBox "參數 PC Mac : " & strPCMac
'    MsgBox "參數 InstAddrNo : " & lngInstAddrNo
'    MsgBox "參數 Class ID : " & strClassID
'    MsgBox "參數 Dyna IP : " & strDynaIP
...
'        MsgBox "02 OpenCM 命令不成功 !" & vbCrLf & "( " & Err.Number & " ) " & Err.Description
    Else
'        On Error Resume Next
'        strGroupID = rsResult("GroupID") & ""
        '#3271測試不OK,從工單流程進來的時候還是會出現錯誤,所以要Update時,先指定Recordset的AbsolutePosition By Kin 2007/06/08
        Dim lngPosition
        lngPosition = rsData.Bookmark
        If frmSO1623A.gdtResvTime.Text <> Empty Then
            strResult = "1"
            OpenCM = True
            GoTo 66
            Exit Function
...
'            MsgBox "Error Message : " & strErrMsg
        ...
        If strCTIP <> Empty Then
            ...
            rsData.Update
        End If
        'If strResult = "1" Or strCTIP <> Empty Then rsData.Update
        OpenCM = True
    End If
...
'                    " AND INSTDATE IS NOT NULL" & _
    ...
    ' ==================================================
    ' QQ : 2502
    ' Doc : EMC-工務流程＆CMCP指令整合規格調整_20070329.doc
    ' V0.5 增加第(4)點規格異動
    '   (4) SO1171A CM主控台，當選擇CP申租、CP退租選項時，
    '       畫面條件須增加顯示此CM設備下所帶之正常CP門號，供USER選擇。
    ' Modify by Power Hammer 2007.04.10
    ' ==================================================
    ...
    strQry = "SELECT PORTNO,FACISNO FROM " & GetOwner & "SO004" & _
                    ...
                    " AND FACISNO='" & Trim(frmSO1623A.cboCPno.Text) & "'"
...
'        If strResult = "1" Then
'            rsResult.MoveFirst
'            If GetRS(rsTarget, "SELECT * FROM " & GetOwner & "SO164 WHERE ROWID=''", gcnGi, adUseClient, adOpenKeyset, adLockOptimistic) Then
'                Debug.Print clsModule.CopyRecordset(rsResult, rsTarget, False, True)
'            End If
'        End If
'
'        On Error Resume Next
...
Public Function Cmd_Acc_24(ByRef rsData As Object, strCMD As String, Optional strCmdSeqNo As String = "") As Boolean
  On Error GoTo ChkErr
    ...
    If Cmd_Acc_24 Then
        rsData("FaciStatusCode") = IIf(LCase(strCMD) = "enable", 4, 3) 'SO004.FaciStatusCode 停權 = 3, 復權 = 4
        rsData.Update
        MsgBox "已完成開通記錄 !", vbInformation, "訊息"
    End If
