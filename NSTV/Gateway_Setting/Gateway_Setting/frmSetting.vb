...
Public Class frmSetting
    Private Const xmlFileName As String = "GatewaySetting.Set"
    ...
    Private Sub btnOK_Click(sender As System.Object, e As System.EventArgs) Handles btnOK.Click
        Try
            ...
            Else
                MessageBox.Show("儲存失敗!", "訊息", MessageBoxButtons.OK)
            End If
            ...
            Using mem As New MemoryStream
                Using XmlFile As New Xml.XmlTextWriter(mem, New UTF8Encoding(False))
                    ...
                    For Each Node As TreeListNode In TreLstSourceId.Nodes
                        ...
                        XmlFile.WriteAttributeString("IP", Node.Item("Server"))
                        XmlFile.WriteAttributeString("Port", Node.Item("Port"))
                        XmlFile.WriteValue(Node.Item("RootKey"))
                        XmlFile.WriteEndElement()            'Server
                    Next
                    XmlFile.WriteStartElement("SMS_ID")    'SMS_ID
                    XmlFile.WriteValue(txtSMS_ID.Text)
                    XmlFile.WriteEndElement()                                             'SMS_ID
                    XmlFile.WriteStartElement("OPE_ID")    'OPE_ID
                    XmlFile.WriteValue(txtOPE_ID.Text)
                    XmlFile.WriteEndElement()                                             'OPE_ID
                    'XmlFile.WriteStartElement("DisconnectRetry")    'DisconnectRetry
                    'XmlFile.WriteValue(3)
                    'XmlFile.WriteEndElement()                                             'DisconnectRetry
                    ...
                    XmlFile.WriteValue(spinSndDelay.Value)
                    XmlFile.WriteEndElement()     'SendDelayTime
                    XmlFile.WriteStartElement("Record")     'Record
                    If chkCanRecord.Checked Then
                        XmlFile.WriteValue("1")
                    Else
                        XmlFile.WriteValue("0")
                    End If
                    XmlFile.WriteEndElement()                             'Record
                    ...
                    XmlFile.WriteStartElement("EmailTitle")                     'EmailTitle
                    XmlFile.WriteValue(txtEmailTitle.Text)
                    XmlFile.WriteEndElement()                                                 'EmailTitle
                    ...
                    For Each Node As TreeListNode In TreLstErrCode.Nodes
                        ...
                        'Node.Item(colErrorCode)
                        ' XmlFile.WriteAttributeString("Value", Convert.ToInt32(Node.Item("ErrorCodeToHex"), 16))
                        XmlFile.WriteAttributeString("Value", Node.Item("ErrorCode"))
    ...
    Private Function ReadNstv() As Boolean
        Try
            Dim tbNSTV As DataTable = xmlIO.ReadNstv(IsEncryFile)
            If tbNSTV.Rows.Count = 1 Then
                With tbNSTV.Rows(0)
                    spinChkStatus.Value = Int32.Parse(.Item("CheckTime"))
                    txtSMS_ID.Text = .Item("SMS_ID")
                    'spinNstvPort.Value = Int32.Parse(.Item("Port"))
                    txtOPE_ID.Text = .Item("OPE_ID")
                    spinCrypt_Ver.Value = Int32.Parse(.Item("CryptVer"))
                    spinProtoVer.Value = Int32.Parse(.Item("ProtoVer"))
                    Select Case Int32.Parse(.Item("KeyType"))
                        Case 2
                            cobKeyType.SelectedIndex = 0
                        Case 1
                            cobKeyType.SelectedIndex = 1
                        Case Else
                            cobKeyType.SelectedIndex = 0
                    End Select
                    spinSndDelay.Value = Decimal.Parse(.Item("SendDelayTime"))
                    spinSendTimeout.Value = Int32.Parse(tbNSTV.Rows(0).Item("SendTimeOut"))
                    spinReceiveTimeout.Value = Int32.Parse(tbNSTV.Rows(0).Item("ReceiveTimeout"))
                    spinRetryCount.Value = Int32.Parse(tbNSTV.Rows(0).Item("RetryProcCount"))
                    chkCanRecord.Checked = False
                    chkUseUTC.Checked = False
                    If Int32.Parse(tbNSTV.Rows(0).Item("Record")) = 1 Then
                        chkCanRecord.Checked = True
                    End If
                    If Int32.Parse(tbNSTV.Rows(0).Item("UTC")) = 1 Then
                    ...
                    cmbEncoding.Text = tbNSTV.Rows(0).Item("Encoding")
                    spinDisconnectRetry.Value = Int32.Parse(tbNSTV.Rows(0).Item("DisconnectRetry"))
                    spinRetryCount.Value = Int32.Parse(tbNSTV.Rows(0).Item("RetryProcCount"))
                    cobProcKind.SelectedIndex = Integer.Parse("0" & tbNSTV.Rows(0).Item("ProcKind"))
                    txtEmailTitle.EditValue = tbNSTV.Rows(0).Item("EmailTitle")
                End With
            End If
        Catch ex As Exception
            Throw
    ...
    Private Sub frmSetting_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            xmlIO = New CableSoft.NSTV.XMLFileIO.XmlFileIO(Application.StartupPath & "\" & xmlFileName)
            If Not File.Exists(Application.StartupPath & "\" & xmlFileName) Then
                MessageBox.Show("設定檔不存在，請儲存檔案", "訊息",
