...
Public Class Client
    ...
    Private _chkStateTime As Int32 = 30
    Private tmrNstvStatus As Threading.Timer = Nothing
    Private LowerCmd As CableSoft.NSTV.BuilderLowerCmd.LowCmd = Nothing
    Private HighCmd As HighCmd = Nothing
    Private _Host As String
    Private _Port As Integer
    Private IsChkStatus As Boolean = False
    ...
    Private tbError As DataTable = Nothing    
    Private sckClient As Socket = Nothing    
    Public Property Proto_Ver As Integer = 1
    Public Property Crypt_Ver As Integer = 1
    Public Property Key_Type As Integer = 2
    Public Property OPE_ID As Integer = 1
    Public Property SMS_ID As Integer = 1
    Public Property RootKey As String = "ABCDEFGHABCDEFGH"
    Public Property TalkKey As String = Nothing
    Public Property SendTimeout As Integer = 5
    Public Property ReceiveTimeout As Integer = 5
    Public Property DisconnectRetry As Integer = 3  '斷線後每幾秒再重連一次
    ...
    Private Sub ReadxmlFileData(ByVal xmlFile As String, ByVal xmlIsEncrypt As Boolean)
        Try
            Using xmlIO As New CableSoft.NSTV.XMLFileIO.XmlFileIO(xmlFile)
                ...
                tbSetNstv = xmlIO.ReadNstv(xmlIsEncrypt)
            ...
            Me.DisconnectRetry = tbSetNstv.Rows(0).Item("DisconnectRetry")
        Catch ex As Exception
            Throw
        End Try

    End Sub
    Public Sub New(ByVal SocketInformation As SocketInformation,
                   ByVal Host As String, ByVal Port As String, ByVal RootKey As String,
                           ByVal xmlFile As String, xmlIsEncrypt As Boolean)
        Try
            sckClient = New Socket(SocketInformation)
            ReadxmlFileData(xmlFile, xmlIsEncrypt)
            sckClient.SendTimeout = tbSetNstv.Rows(0).Item("SendTimeOut")
            sckClient.ReceiveTimeout = tbSetNstv.Rows(0).Item("ReceiveTimeout")
            _AddressFamily = sckClient.AddressFamily
            _SocketType = sckClient.SocketType
            _ProtocolType = sckClient.ProtocolType
            Me._Host = Host
            Me._Port = Port
            Me.RootKey = RootKey
            FirstTime = True
        ...
        End Try
    End Sub
    Public Sub New(ByVal AddressFamily As AddressFamily,
                   ByVal SocketType As SocketType, ByVal ProtocolType As ProtocolType,
                   ByVal Host As String, ByVal Port As String, ByVal RootKey As String,
                   ByVal xmlFile As String, xmlIsEncrypt As Boolean)
        Try
            sckClient = New Socket(AddressFamily, SocketType, ProtocolType)
            ReadxmlFileData(xmlFile, xmlIsEncrypt)
            sckClient.SendTimeout = tbSetNstv.Rows(0).Item("SendTimeOut")
            sckClient.ReceiveTimeout = tbSetNstv.Rows(0).Item("ReceiveTimeout")
    ...
    Public Function SendHighCmd(ByVal rw As Dictionary(Of String, Object)) As Dictionary(Of String, Object)
        ...
        Try
            ...
            'End If
            If String.IsNullOrEmpty(TalkKey) OrElse IsChkStatus Then
                Do
                    If String.IsNullOrEmpty(TalkKey) Then
                        Return rwReturn.Value                        
                    End If
                    If (Not String.IsNullOrEmpty(TalkKey)) AndAlso (Not IsChkStatus) Then
                        Exit Do
                    End If
                    Thread.Sleep(100)
                Loop
                ...
                SyncLock sckClient
                    ...
                    If SendLowCmdList.Value.Count > 0 Then                        
                        For i As Int32 = 0 To SendLowCmdList.Value.Count - 1
                            tmrNstvStatus.Change(Timeout.Infinite, Timeout.Infinite)
                            Try
                                ...
                                Else
                                    rwReturn.Value.Item("RecvLog".ToUpper) = rwReturn.Value.Item("RecvLog".ToUpper) & "," & _
                                         IIf(String.IsNullOrEmpty(response.Value.RecvDataString),
                                                                                 "0", response.Value.RecvDataString)
                                End If
                                tmrNstvStatus.Change(Me._chkStateTime * 1000, Me._chkStateTime * 1000)
                            End Try
                        Next
                    Else
                        rwReturn.Value.Item("CMD_STATUS".ToUpper) = "E"
            ...
            Catch ex As Exception
                ...
                Return rwReturn.Value
            Finally
                If tmrNstvStatus IsNot Nothing Then
                    If (sckClient Is Nothing) OrElse (Not sckClient.Connected) Then
                        tmrNstvStatus.Change(Me.DisconnectRetry * 1000, Me.DisconnectRetry * 1000)
                    Else
    ...
    Public Sub RetryConnect()
        ...
        End Try
    End Sub
    'Private Function GetLowCmdList(ByVal rw As DataRow) As List(Of SendLowCmd)
    ...
    '    Try
    '        rwHigh.Value = tbSetHighCmd.Select(String.Format("CodeNo='{0}'", rw.Item("HIGH_LEVEL_CMD_ID")))
    '        If (rwHigh.Value Is Nothing) OrElse (rwHigh.Value.Length = 0) Then
    '            Throw New Exception(String.Format("找不到SEQNO={0}高階命令{1}!",
    '                                              rw.Item("SEQNO"), rw.Item("HIGH_LEVEL_CMD_ID")))
    '        End If
    '        For Each strCode As String In rwHigh.Value(0).Item("RunLowCmd").ToString.Split(",")
    '            rwLow.Value = tbSetLowCmd.Select(String.Format("CodeNo='{0}'", strCode.Trim))
    '            If (rwLow.Value Is Nothing) OrElse (rwLow.Value.Length = 0) Then
    '                Throw New Exception(String.Format("高階命令[ {0} ] 找不到低階命令[ {1} ]",
    '                                                  rwHigh.Value("CodeNo"),
    '                                                  strCode.Trim))
    '            Else
    '                result.Value.Add(GetLowCmd(rw, rwLow.Value(0).Item("MsgId")))
    '            End If
    ...
    Public Sub Connect(ByVal host As String, ByVal Port As Integer)
        ...
        End Try
    End Sub
    Public Sub ConnectNstv()
        Try
            ConnectErrCount = 0
            ConnectNstv(_Host, _Port)
        Catch ex As Exception
            Throw
        End Try
    End Sub
    Public Sub ConnectNstv(ByVal host As String, ByVal Port As Int32)
        Try
            _Host = host
            _Port = Port
            ConnectErrCount = 0
            If LowerCmd Is Nothing Then
                LowerCmd = New CableSoft.NSTV.BuilderLowerCmd.LowCmd(Proto_Ver,
                                                               Crypt_Ver,
                                                               Key_Type,
                                                               OPE_ID,
                                                               SMS_ID,
                                                               ...
                                                               Me.tbSetNstv,
                                                               Me.tbError)
                'LowerCmd.CanRecord = Int32.Parse(tbSetNstv.Rows(0).Item("Record")) = 1
                'LowerCmd.UseUTC = Int32.Parse(tbSetNstv.Rows(0).Item("UTC")) = 1
                'LowerCmd.Encoding = tbSetNstv.Rows(0).Item("Encoding")

            End If

            If HighCmd Is Nothing Then
                HighCmd = New HighCmd(tbSetHighCmd, tbSetLowCmd, tbSetNstv, LowerCmd)
            End If

            If tmrNstvStatus Is Nothing Then
                tmrNstvStatus = New Threading.Timer(AddressOf chkStatus, Nothing, 0, _chkStateTime * 1000)

            Else
                tmrNstvStatus.Change(0, _chkStateTime * 1000)
            End If
        Catch ex As Exception
            Throw ex
    ...
    Public Overloads Function SendSynchronize(ByVal SendData() As Byte) As Byte()
        ...
        Catch ex As Exception
            ...
            Finally
                ...
                sckClient.Dispose()
                FirstTime = True
                tmrNstvStatus.Change(0, Me.DisconnectRetry * 1000)
            End Try
        ...
        Finally
            ByteRecv.Dispose()
            If tmrNstvStatus IsNot Nothing Then
                tmrNstvStatus.Change(_chkStateTime * 1000, _chkStateTime * 1000)
            End If
    ...
    Private Sub chkStatus(ByVal status As Object)
        ...
        Try
            If (sckClient Is Nothing) OrElse (Not sckClient.Connected) Then
                FirstTime = True
                sckClient = New Socket(_AddressFamily, _SocketType, _ProtocolType)
                sckClient.SendTimeout = Me.SendTimeout * 1000
                sckClient.ReceiveTimeout = Me.ReceiveTimeout * 1000
            ...
            Else
                If FirstTime Then
                    sckClient.Send(LowerCmd.Builder_SMS_CA_CREATE_SESSION_REQUEST().SendData)
                    sckClient.Receive(recvData.Value)
                    Dim result As CableSoft.NSTV.BuilderLowerCmd.ResponseLowCmd = LowerCmd.AnalyseLowCmd(recvData.Value)
    ...
    Protected Overridable Sub Dispose(disposing As Boolean)
        If Not Me.disposedValue Then
            If disposing Then
                If tmrNstvStatus IsNot Nothing Then
                    tmrNstvStatus.Change(Timeout.Infinite, Timeout.Infinite)
                    tmrNstvStatus.Dispose()
                    tmrNstvStatus = Nothing
                End If
                If sckClient IsNot Nothing Then
                    Try
                        If sckClient.Connected Then
