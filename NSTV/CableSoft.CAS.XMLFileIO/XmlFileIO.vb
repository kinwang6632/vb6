...
Public Class XmlFileIO
    ...
    Public Function ReadErrorList(ByVal IsEncryFile As Boolean) As DataTable
        ...
        tbError.Columns.Add(New DataColumn("ErrorCodeToHex", GetType(String)))
        tbError.Columns.Add(New DataColumn("ErrorName", GetType(String)))
        tbError.Columns.Add(New DataColumn("ErrorCode", GetType(String)))
        Try
            Dim Value As List(Of Object) = ReadXmlNodeAttributes("CableSoft/ErrorList", "Error", "Value", IsEncryFile)
            'Dim HexValue As List(Of Object) = ReadXmlNodeAttributes("CableSoft/ErrorList", "Error", "HexValue", IsEncryFile)
            ...
            If Value IsNot Nothing Then
                For i As Int32 = 0 To Value.Count - 1
                    ...
                    'data(2) = HexValue.Item(i)
                    Dim rwNew As DataRow = tbError.NewRow()
                    'rwNew.Item("ErrorCodeToHex") = data(0)
                    rwNew.Item("ErrorCode") = Convert.ToInt32(data(0), 16)
                    rwNew.Item("ErrorCodeToHex") = data(0)
                    'rwNew.Item("ErrorCode") = data(0)
                    rwNew.Item("ErrorName") = data(1)
    ...
    End Function
    '讀取Nstv參數
    Public Function ReadNstv(ByVal IsEncryFile As Boolean) As DataTable
        Dim tbNSTV As New DataTable("NSTV")
        tbNSTV.Columns.Add(New DataColumn("CheckTime", GetType(Int32)))
        tbNSTV.Columns.Add(New DataColumn("SMS_ID", GetType(Int32)))
        tbNSTV.Columns.Add(New DataColumn("OPE_ID", GetType(Int32)))
        'tbNSTV.Columns.Add(New DataColumn("Server", GetType(String)))
        'tbNSTV.Columns.Add(New DataColumn("Port", GetType(Int32)))
        'tbNSTV.Columns.Add(New DataColumn("RootKey", GetType(String)))
        tbNSTV.Columns.Add(New DataColumn("CryptVer", GetType(Byte)))
        tbNSTV.Columns.Add(New DataColumn("ProtoVer", GetType(Byte)))
        tbNSTV.Columns.Add(New DataColumn("KeyType", GetType(Byte)))
        tbNSTV.Columns.Add(New DataColumn("SendTimeOut", GetType(Int32)))
        tbNSTV.Columns.Add(New DataColumn("ReceiveTimeout", GetType(Int32)))
        tbNSTV.Columns.Add(New DataColumn("RetryProcCount", GetType(Int32)))
        tbNSTV.Columns.Add(New DataColumn("SendDelayTime", GetType(Double)))
        tbNSTV.Columns.Add(New DataColumn("DisconnectRetry", GetType(Integer)))
        tbNSTV.Columns.Add(New DataColumn("Record", GetType(Int16)))
        tbNSTV.Columns.Add(New DataColumn("UTC", GetType(Int16)))        
        tbNSTV.Columns.Add(New DataColumn("Encoding", GetType(String)))
        tbNSTV.Columns.Add(New DataColumn("ProcKind", GetType(Int32)))
        tbNSTV.Columns.Add(New DataColumn("EmailTitle", GetType(String)))
        Try
            Dim rwNew As DataRow = tbNSTV.NewRow
            rwNew.Item("CheckTime") = Int32.Parse(ReadXmlNodeValue("CableSoft/NSTV", "CheckTime", IsEncryFile))
            rwNew.Item("SMS_ID") = Int32.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "SMS_ID", IsEncryFile))
            rwNew.Item("OPE_ID") = Int32.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "OPE_ID", IsEncryFile))
            'rwNew.Item("RootKey") = ReadXmlNodeValue("CableSoft/NSTV", "RootKey", IsEncryFile)
            rwNew.Item("CryptVer") = Int32.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "CryptVer", IsEncryFile))
            rwNew.Item("ProtoVer") = Int32.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "ProtoVer", IsEncryFile))
            rwNew.Item("SendDelayTime") = Decimal.Parse(ReadXmlNodeValue("CableSoft/NSTV", "SendDelayTime", IsEncryFile))
            rwNew.Item("SendTimeOut") = Decimal.Parse(ReadXmlNodeValue("CableSoft/NSTV", "SendTimeOut", IsEncryFile))
            rwNew.Item("ReceiveTimeout") = Decimal.Parse(ReadXmlNodeValue("CableSoft/NSTV", "ReceiveTimeout", IsEncryFile))
            rwNew.Item("RetryProcCount") = Decimal.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "RetryProcCount", IsEncryFile))
            rwNew.Item("DisconnectRetry") = Decimal.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "DisconnectRetry", IsEncryFile))
            'rwNew.Item("SendDelayTime") = Decimal.Parse(ReadXmlNodeValue("CableSoft/NSTV", "SendDelayTime", IsEncryFile))
            Select Case Int32.Parse(ReadXmlNodeValue("CableSoft/NSTV", "KeyType", IsEncryFile))
                Case 1
                    rwNew.Item("KeyType") = 1
                Case Else
                    rwNew.Item("KeyType") = 2
            End Select
            rwNew.Item("Record") = Decimal.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "Record", IsEncryFile))
            rwNew.Item("UTC") = Decimal.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "UTC", IsEncryFile))
            rwNew.Item("Encoding") = ReadXmlNodeValue("CableSoft/NSTV", "Encoding", IsEncryFile)
            rwNew.Item("ProcKind") = Int32.Parse("0" & ReadXmlNodeValue("CableSoft/NSTV", "ProcKind", IsEncryFile))
            rwNew.Item("EmailTitle") = ReadXmlNodeValue("CableSoft/NSTV", "EmailTitle", IsEncryFile)
            tbNSTV.Rows.Add(rwNew)
            tbNSTV.AcceptChanges()
        Catch ex As Exception
            Throw
        End Try
        Return tbNSTV
    ...
    Public Function ReadServer(ByVal IsEncryFile As Boolean, ByVal Server As String) As DataTable
        ...
        tbServer.Columns.Add(New DataColumn("IPAddress", GetType(String)))
        tbServer.Columns.Add(New DataColumn("Port", GetType(String)))
        tbServer.Columns.Add(New DataColumn("RootKey", GetType(String)))
        Try
            Dim IPERADDRESS As List(Of Object) = ReadXmlNodeAttributes("CableSoft/NSTV", "Server", "IP", IsEncryFile)
            Dim Port As List(Of Object) = ReadXmlNodeAttributes("CableSoft/NSTV", "Server", "Port", IsEncryFile)
            Dim RootKey As List(Of Object) = ReadXmlNodeValues("CableSoft/NSTV", "Server", IsEncryFile)
            If IPERADDRESS IsNot Nothing Then
                For i As Int32 = 0 To IPERADDRESS.Count - 1
                    Dim data(2) As Object
                    If String.IsNullOrEmpty(IPERADDRESS.Item(i).ToString) Then
                        data(0) = "127.0.0.1"
                    Else
                        data(0) = IPERADDRESS.Item(i)
                    End If
                    If String.IsNullOrEmpty(Port.Item(i).ToString) Then
                        data(1) = "7364"
                    Else
                        data(1) = Port.Item(i)
                    End If

                    If String.IsNullOrEmpty(RootKey.Item(i).ToString) Then
                        data(2) = "ABCDEFGHABCDEFGH"
                    Else
                        data(2) = RootKey.Item(i)
                    End If
                    Dim rwNew As DataRow = tbServer.NewRow
                    rwNew.Item("IPAddress") = data(0)
                    rwNew.Item("Port") = data(1)
                    rwNew.Item("RootKey") = data(2)
    ...
    Public Function ReadXmlNodeValue(ByVal ParentNode As String, ByVal NodeName As String,
        ...
        Return aResult
    End Function
    Public Shared Function ReadDBID(ByVal xmlDBIdFile As String,
                                    ByVal ParentNodeName As String,
                                    ByVal ChildNodeName As String) As Int32
        Dim aResult As String = Nothing
        Try
    ...
    Public Shared Function WriteDBId(ByVal xmlDBIdFile As String,
