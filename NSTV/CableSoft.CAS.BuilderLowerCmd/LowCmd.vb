...
Public Class LowCmd
    ...
    Public Property CanRecord As Boolean = True
    Public Property UseUTC As Boolean = False
    ...
    Public Sub New(ByVal Proto_Ver As Byte, ByVal Crypt_Ver As Byte,
                   ByVal Key_Type As Key_TypeEnum, ByVal OPE_ID As Integer,
                    ByVal SMS_ID As Integer,
                    ByVal Root_Key As String,
                    ByVal tbSetNstv As DataTable,
                    ByVal ErrorList As DataTable)
        ...
        Try
            ...
            End If
            xmlFile = New CableSoft.NSTV.XMLFileIO.XmlFileIO(Application.StartupPath & "\" & xmlFileName)
            'DBId = xmlFile.ReadXmlNodeValue(SmsNodeName, DBIdNodeName, False)
            Dim maxTemp As Object = 65535
            Try
                DBId = XMLFileIO.XmlFileIO.ReadDBID(Application.StartupPath & "\" & xmlFileName, SmsNodeName, DBIdNodeName)
    ...
    Public Overloads Function Builder_SMS_CA_SEND_SUPEROSD_REQUEST(ByVal Exp As String,
        ...
        Try
            ...
            If (DBNull.Value.Equals(rwHigh.Item("Notes".ToUpper))) OrElse
                ...
                Throw New Exception("SEND_SUPEROSD Notes 資料不正確!")
            End If
        ...
        End Try
    End Function
    Public Overloads Function Builder_SMS_CA_SET_STBNotify_REQUST(ByVal Exp As String,
                                                                  ByVal rwHigh As Dictionary(Of String, Object)) As SendLowCmd
        Try
            If (DBNull.Value.Equals(rwHigh.Item("Notes"))) OrElse
                (String.IsNullOrEmpty(rwHigh.Item("Notes").ToString)) Then
                Throw New Exception("SEBNotify Notes 資料不正確!")
            End If
            If Not IsNumeric(rwHigh.Item("Notes").ToString.Split(",").ToList(0)) Then
                Throw New Exception("SEBNotify 數值資料不正確!")
            End If
            Return CreateFixData(Create_Data_Section(&H40C,
                                                  Create_Data_Body(&H40C,
                                                                    CreateSTBNotify_REQUST_Count(Exp, rwHigh.Item("Notes").ToString))))
        Catch ex As Exception
            Throw
        End Try
    End Function
    ...
    Public Function Builder_SMS_CA_ENTITLE_REQUEST(ByVal Card_SN As String,
        ...
        End Try
    End Function
    Public Overloads Function Builder_SMS_CA_REPAIR_REQUEST(ByVal Card_SN As String,
                                                            ByVal STBID As String,
                                                            ByVal FaciKindValue As String, ByVal SNoType As Integer) As SendLowCmd
        Try
            '#6892 增加FaciKindValue 參數 By Kin 2014/10/21
            Return CreateFixData(Create_Data_Section(&H206,
                                                     Create_Data_Body(&H206,
                                                                      CreateRepair_Data_Count(Card_SN, STBID, FaciKindValue, SNoType))))
        ...
        End Try
    End Function
    Public Overloads Function Builder_SMS_CA_REPAIR_REQUEST(ByVal Card_SN As String, ByVal STBID As String) As SendLowCmd
        Try
            Return CreateFixData(Create_Data_Section(&H206,
                                                     Create_Data_Body(&H206,
    ...
    ''' <param name="Notes">NOTES 欄位</param>
    ''' <returns>Byte Array</returns>
    ''' <remarks> NOTES 格式 => 1,ABCDABCD ，只有当参数“Control”的值在范围[7, 200]时，参数“Des_Len”和“Des_Char”才有效</remarks>
    Private Function CreateSTBNotify_REQUST_Count(ByVal Exp As String,
                                                  ByVal Notes As String) As Byte()
        Dim byteData As New ThreadLocal(Of List(Of Byte))
        Dim EncodingType As New ThreadLocal(Of Encoding)
        Dim Des As New ThreadLocal(Of String)
        Dim Control As New ThreadLocal(Of Byte)
        byteData.Value = New List(Of Byte)
        Try
            '控制行为
            Control.Value = Notes.Split(",").ToList.Item(0)
            '控制描述
            If Notes.Split(",").Count > 1 Then
                Des.Value = Notes.Split(",").ToList.Item(1)
            Else
                Des.Value = Nothing
            End If
            ...
            '控制行为
            byteData.Value.Add(Control.Value)
            '控制描述長度與控制描述
            '只有当参数“Control”的值在范围[7, 200]时，参数“Des_Len”和“Des_Char”才有效
            If (Control.Value >= 7) AndAlso (Control.Value <= 200) Then
                If Not String.IsNullOrEmpty(Des.Value) Then
                    byteData.Value.AddRange(GetConText(Des.Value, &H40C))
                Else
    ...
    End Function
    Private Overloads Function CreateRepair_Data_Count(ByVal Card_SN As String,
                                                       ByVal STBID As String,
                                                       ByVal FaciKindValue As String, ByVal SNoType As Integer) As Byte()
        Dim byteData As New ThreadLocal(Of List(Of Byte))
        Dim arySTB As New ThreadLocal(Of List(Of UInt64))
        Dim STBNoValue As New ThreadLocal(Of UInt64)
        Dim FaciKindValueLength As New ThreadLocal(Of Byte)
        byteData.Value = New List(Of Byte)
        ...
        Try
            For Each Str As Char In Card_SN
                byteData.Value.AddRange(System.Text.Encoding.ASCII.GetBytes(Str))
            Next
            '#6918 判斷要抓8碼或7碼 By Kin 2014/11/18
            Select Case FaciKindValue.ToUpper
                Case "12B00".ToUpper
                    FaciKindValueLength.Value = 7
                Case Else
                    FaciKindValueLength.Value = 8
            End Select        
            arySTB.Value.AddRange(New UInt64() {0, 0, 0, 0, 0})
            For aIndex As Integer = 0 To STBID.Split(",").Count - 1
                If aIndex > 4 Then
                    Exit For
                End If
                '#6892 增加FaciKindValue 參數 By Kin 2014/10/21
                ...
                Select Case SNoType
                    Case 0
                        STBNoValue.Value = UInt64.Parse(Right(New String("0", FaciKindValueLength.Value) & STBID.Split(",")(aIndex),
                                                              FaciKindValueLength.Value))
                    Case Else
                        STBNoValue.Value = Convert.ToUInt64(Right(New String("0", FaciKindValueLength.Value) & STBID.Split(",")(aIndex),
                                                                  FaciKindValueLength.Value), 16)
                End Select

                arySTB.Value.Item(aIndex) = Convert.ToUInt64(Left(FaciKindValue & "000000000000", 12), 16) +
                                                                STBNoValue.Value

                '廠商格式改成,"12B000000000(固定)+STBNO後7碼" By Kin 2013/07/26
                'arySTB.Value.Item(i) = UInt64.Parse(STBID.Split(",")(i))
                'arySTB.Value.Item(aIndex) = Convert.ToUInt64("12B000000000", 16) +
                '                                        UInt64.Parse(Right("0000000" & STBID.Split(",")(aIndex), 7))
        ...
        Finally
            ...
            arySTB.Dispose()
            STBNoValue.Dispose()
            FaciKindValueLength.Dispose()
        End Try
    End Function
    Private Overloads Function CreateRepair_Data_Count(ByVal Card_SN As String, ByVal STBID As String) As Byte()
        Return CreateRepair_Data_Count(Card_SN, STBID, "12B00", 0)
        'Dim byteData As New ThreadLocal(Of List(Of Byte))
        'byteData.Value = New List(Of Byte)
        'Dim arySTB As New ThreadLocal(Of List(Of UInt64))
        'arySTB.Value = New List(Of UInt64)
