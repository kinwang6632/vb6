...
Sub AttachMessage(iwp As clsISubClass, ByVal hwnd As Long, ByVal iMsg As Long)
Dim procOld As Long
...
Private Function WindowProc(ByVal hwnd As Long, ByVal iMsg As Long, ByVal wParam As Long, ByVal lParam As Long) As Long
    ...
    Dim iwp As clsISubClass
    Dim iwpT As clsISubClass
    Dim iIndex As Long
...
Private Sub pClearUp(ByVal hwnd As Long)
    Dim msgCount As Long
    Dim procOld As Long
   ' this is only called if you haven't explicitly cleared up your subclass from the caller.  You will get a minor resource leak as it does not clear up any message
   ' specific properties.
   msgCount = MessageCount(hwnd)
    If (msgCount > 0) Then
        procOld = OldWindowProc(hwnd) ' remove the subclass:
        If Not (procOld = 0) Then
        Call SetWindowLong(hwnd, GWL_WNDPROC, procOld) ' Unsubclass by reassigning old window procedure
    End If
    OldWindowProc(hwnd) = 0 ' remove the old window proc:
    MessageCount(hwnd) = 0
    End If
End Sub
