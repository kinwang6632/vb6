...
Private Function subChoose() As Boolean
  On Error GoTo ChkErr
    ...
    Else
        ...
        End If
    End If
    '#6535 增加VODACCOUNTID 與 FACISNO 條件 By Kin 2013/06/27
...
Private Function GetSQL(ByVal strWhere As String)
  On Error GoTo ChkErr
    ...
    aExcelWhere1 = Empty
    aExcelWhere2 = Empty
    'MustPayCredit=A.NOPAYCREDIT - A.PREPAY + A.PRESENT(必須付費點數=未付點數-儲值點數+贈點 :100-80+20)
...
'                adUseClient, adOpenKeyset, adLockOptimistic) Then Exit Function
    ...
    Do While Not rs182.EOF
        ...
        If Val(rs182("Flag")) = 0 Then
            ...
            '#6388 取消strUPD182 By Kin 2013/01/14
            '#又要還原回來 For Jacky By Kin 2013/05/21
            '#還原回來要改變語法
...
'                        " AND CLOSEBILLNO IS NULL "
            ...
            '#6388 取消strUPD033Vod By Kin 2013/01/14
            '#又要還原回來 For Jacky By Kin 2013/05/21
...
'                            " And CloseBillNo IS NULL "
            ...
            '#6388 取消更新SO182與SO033VOD By Kin 2013/01/14
            '#又要還原回來 For Jacky By Kin 2013/05/21
        ...
        Else
            '如果是未執行,要將次數加1
            Dim strQry As String
            '#6388 取消更新SO182 By Kin 2013/01/14
            '又要還原回來FOR jacky By Kin 2013/05/23
        ...
        End If
        If blnNOUpdate Or blnAutoEXE Then
        '#6388 取消回傳已更新的RecordSet By Kin 2013/01/14
        '又要還原回來 For Jacky By Kin 2013/05/23
        '#6778 多判斷State狀態，否則如果第一筆資料Flag=0 會出錯 By Kin 2014/04/25
