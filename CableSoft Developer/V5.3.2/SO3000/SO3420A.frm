...
Begin VB.Form frmSO3420A 
   ...
   Begin TabDlg.SSTab stData 
      ...
      Tab(0).Control(2)=   "ggrData"
      Tab(0).Control(3)=   "txtBillNo"
      Tab(0).Control(4)=   "lbl6"
      Tab(0).Control(5)=   "lblClassName"
      Tab(0).Control(6)=   "lblCount"
      Tab(0).Control(7)=   "Label1"
      Tab(0).Control(8)=   "lbl1"
      Tab(0).Control(9)=   "lblBillNo"
    ...
    Dim blnShowView As Boolean
        ...
        If stData.Tab = 0 Then
            If Not GetRS(rsData, "Select Distinct CustID, BillNo,CustName,PrtSNo, " & _
                ...
                "ClassName1,WipName1,WipName2,WipName3,ServiceType,Balance,StartDateA as RealStartDate,StopDateA as RealStopDate,SeqNo,CitemCode,CitemName,MediaBillNo From So3420A Order By CustId", cnMDB) Then Screen.MousePointer = vbDefault: Me.Enabled = True: Exit Sub
        ElseIf stData.Tab = 1 Then
            ...
            ElseIf cboMduRange.ListIndex = 1 Then
                ...
                strSQL = "Select Distinct A.Custid,B.ServiceType,B.CitemCode,B.CitemName,B.ShouldDate,B.RealStartDate," & _
                        "B.RealStopDate,B.SeqNo,B.BillNo,B.MediaBillNo,B.PrtSNo,B.GUINo,B.UCCode,B.UCName,B.ShouldAmt,B.RealAmt,B.RealDate " & _
        ...
        ElseIf stData.Tab = 2 Then
            ...
            strSQL = "Select Distinct A.Custid,A.ServiceType,A.CitemCode,A.CitemName," & _
                     ...
                     "A.MediaBillNo,A.PrtSNo,A.GUINo,A.UCCode,A.UCName,A.RealAmt,A.ShouldAmt,A.RealDate " & strSQL & " Order By A.CustId"
...
'        Screen.MousePointer = vbHourglass
        ...
        If (stData.Tab = 1 And InStr(1, cboMduRange.Text, "只有待收戶") > 0) Or (stData.Tab) = 2 Then
            ...
            blnShowView = True
        End If
        If Not blnViewOk And blnShowView Then MsgBox "作業取消", vbExclamation, "訊息": GoTo Finall
        '****************************************************
        lngClsWorkCount = 0
        lngCount = 0
        lngError = 0
                    ...
                    If blnFlag Then
                        ...
                        If InStr("126", rsSO001("CustStatusCode")) > 0 Then
                            ...
                            If rsSO001("CustStatusCode") <> 6 Then
                                If Not InsertToSO00x(giEditModeInsert, rsData("CustId"), gilPRCode.GetCodeNo, rsSO009) Then GoTo ErrTrans
                                '#3254 如果"不新增至服務申告"，則維持原做法，傳進一個Nothing的Record，反之傳進一個SO006的Recordc By Kin 2007/07/18
                                Call ReturnSo006(rsSO006)
                                If Not clsAlterWip.Action(lngEditMode, gcnGi, rsSO009, rsSO004, rsSO033, , , , , False, rsSO006) Then GoTo ErrTrans
                                blnUpdateFlag = True
                            End If
                            If chkClsWork.Value = 1 Then
                                '正常、停機:會產生拆機派工單，且將拆機派工單結案。
                                '拆機中:不會產生拆機派工單，且將原來拆機中派工單結案
                                If Not InsertToSO00x(giEditModeEdit, rsData("CustId"), gilPRCode.GetCodeNo, rsSO009) Then GoTo ErrTrans
                                '********************************************************************************************************
                                ...
                                End If
                                '********************************************************************************************************
                                If Not clsAlterWip.Action(lngEditMode, gcnGi, rsSO009, rsSO004, rsSO033, , , , , False) Then GoTo ErrTrans
                            ...
                            If InStr("12", rsSO001("CustStatusCode")) > 0 Then
                                ...
                                If optClsBill Or optUCCode Then
                                    If Not subCloseBill(rsData("CustId"), rsData("ServiceType") & "", strChoose033) Then GoTo ErrTrans
                                    blnUpdateFlag = True
                                End If
                            End If
...
Private Sub txtBillNo_Validate(Cancel As Boolean)
    ...
    Dim strBillField As String
        ...
        strSQL = "Select ServiceType,RealStartDate,RealStopDate,CitemCode,CitemName,CustId,BillNo,Item,PrtSNo,MediaBillNo,SeqNo,ClassCode,GUINO "
...
Private Function OpenData() As Boolean
    ...
    Dim rsTmp As New ADODB.Recordset
        ...
        strSQL = "Select 1 RowNo,B.PrtSNo,A.CustId,D.WipName1,D.WipName2,A.ClassName1,A.Balance,D.wipName3,A.ServCode,A.Tel1,A.Tel2,A.Tel3,A.CompCode,A.InstAddrNo,A.InstAddress,D.CustStatusCode,D.CustStatusName,A.CustName,A.ServiceType," & _
            "B.BillNo,B.RealStartDate StartDateA,B.RealStopDate StopDateA,C.StartDate StartDateB,C.StopDate StopDateB,C.ClctDate,B.MediaBillNo,B.CitemCode,B.CitemName,B.SeqNo,0 as Type,B.RealStartDate,B.RealStopDate From " & GetOwner & "So001 A," & GetOwner & "SO033 B," & GetOwner & "So003 C," & GetOwner & "So002 D " & _
...
Private Function subCloseBill(lngCustId As Long, strServiceType As String, _
    strChoose033 As String) As Boolean
    On Error GoTo ChkErr
    Dim rsTmp As New ADODB.Recordset
    Dim strSQL As String, strWhere As String
        ...
        If optClsBill Then
            ...
            End If
        Else
            strSQL = strSQL & ",UCCode=" & gilUCCode.GetCodeNo & ",UCName='" & gilUCCode.GetDescription & "'"
        End If
        ...
        If stData.Tab = 0 Then
            ...
            Loop
        Else
            gcnGi.Execute strSQL & " Where CustId = " & lngCustId & " And UCCode Is Not Null And ServiceType = '" & strServiceType & "'" & strChoose033
        End If
        subCloseBill = True
    Exit Function
ChkErr:
...
Private Function InsertToSO00x(lngMode As giEditModeEnu, lngCustId As Long, _
        intPRCode As Integer, ByRef rsSO00x As ADODB.Recordset) As Boolean
    On Error GoTo ChkErr
        lngEditMode = lngMode
        With rsSO00x
    ...
    On Error Resume Next
        If strMsg = "" Then
            Select Case intErrorCustStatus
                ...
                Case csGUINo
                    strMsg = "該客戶待收資料已有發票號碼, 客戶編號:" & rsData("CustId") & ", 單據編號:" & rsData("BillNo")
                '********************************************************************************************************************
                Case csOtherError
