...
Public Class ReadCommon
    ...
    ''' <param name="sParaFile">虛擬目錄下的加密連線檔案(需包含路徑)</param>
    ''' <param name="sIniFile">虛擬目錄下的Ini檔案(需包含路徑)</param>
    ''' <param name="stm">除錯模式(StreamWriter)</param>
    ''' <remarks>會將錯誤訊息寫入文字檔</remarks>
    Sub New(ByVal strXml As String, ByVal sParaFile As String, ByVal sIniFile As String, ByRef stm As StreamWriter)
        stmDug = stm
        strSysPara = sParaFile
        strIniFile = sIniFile
        LoadXmlFile(strXml)
    ...
    Private Function GetXmlPara() As Boolean
        Try
            ...
            If strCmd = String.Empty OrElse strTel = String.Empty OrElse intCompCode = 0 Then
                ...
                If Not stmDug Is Nothing Then
                    stmDug.WriteLine("[GetXmlPara] 失敗")
                    stmDug.WriteLine("[GetXmlPara] strCmd:" & strCmd & "  strTel:" & strTel & "  intCompCode:" & intCompCode.ToString)
                End If
                Return False
                Exit Function
            End If
            If strCn = String.Empty Then
                If Not stmDug Is Nothing Then
                    stmDug.WriteLine("[GetXmlPara] 失敗")
                    stmDug.WriteLine("[GetXmlPara] strCn是空白")
                End If
                blnTestXML = False
                Return False
            Else
        ...
        Catch ex As Exception
            blnTestXML = False
            If Not stmDug Is Nothing Then
                stmDug.WriteLine("[GetXmlPara] 錯誤")
            End If
            Return False
        End Try
    End Function
    ...
    Private Function OpenConnect(ByVal strFile As String, ByVal intComp As Integer) As Boolean
        Try
            Using stmRead As New StreamReader(strFile, Encoding.Default)
                ...
                Else
                    ...
                    strCn = String.Empty
                    If Not stmDug Is Nothing Then
                        stmDug.WriteLine("[OpenConnect]  失敗")
                        stmDug.WriteLine("[OpenConnect]  strFile:" & strFile & "  intComp:" & intComp)
                        stmDug.WriteLine("[OpenConnect]  strCn:" & strCn & "  intComp:" & intCompCode)
                    End If
                    Return False
                End If
            End Using
        Catch ex As Exception
            ...
            intCompCode = 0
            If Not stmDug Is Nothing Then
                stmDug.WriteLine("[OpenConnect] 錯誤")
            End If
            Return False
        End Try
    End Function
    Private Sub LoadXmlFile(ByVal strXml As String)
        If (Not strXml Is Nothing) AndAlso (strXml <> String.Empty) Then
            ...
            Catch ex As Exception
                ...
                blnIsXML = False
                blnTestXML = False
                stmDug.WriteLine("LoadXmlFile 錯誤")
            End Try
        End If
    End Sub
    ...
    Private Function GetOwner() As String
        ...
        Try
            If strCn <> String.Empty Then
                ...
                Catch ex As Exception
                    If Not stmDug Is Nothing Then
                        stmDug.WriteLine("[GetOwner] 錯誤")
                    End If
                    Return String.Empty
                End Try
            Else
                If Not stmDug Is Nothing Then
                    stmDug.WriteLine("[GetOwner] 錯誤 連接串字為空值")
                End If
                Return String.Empty
