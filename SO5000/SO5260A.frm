    ...
    Screen.MousePointer = vbHourglass
      ...
      Me.Enabled = False
        ...
        Else
           InsertDataSO5260A4
'           '各期統計彙總
'           strsql = "Insert into SO5260A5  Select Period,sum((Month01+Month02+Month03+Month04+ " & _
'                     "Month05+Month06+Month07+Month08+Month09+Month10+Month11+Month12)) " & _
'                     "as Total from SO5260A4 GROUP BY Period"
'           Set rsTmp = cnn.Execute(strsql)
           ...
           '各期統計彙總
           strSubQry2 = "Insert into SO5260A5  Select Period,sum((Month01+Month02+Month03+Month04+ " & _
                     "Month05+Month06+Month07+Month08+Month09+Month10+Month11+Month12)) " & _
                     "as Total from SO5260A4 GROUP BY Period"
            Set rsTmp = cnn.Execute(strSubQry2)
           '#3108 將子報表二指定為strSubQry2 by Kin 2007/3/9
           strSubQry2 = "Select * from SO5260A5"
           If blnExcel Then
               Call toExcel
           Else
              If Not optPeriod Then
              ...
              Else
                '#3108 如果INI改變路徑Report會出錯,少傳strSubQry2 by Kin 2007/3/9
                   Call PrintRpt(GetPrinterName(5), RptName("SO5260", 2), , Me.Caption, strSQL, strChooseString, , True, "Tmp0000.Mdb", , strGroupName, GiPaperLandscape, , strSubQry1, strSubQry2)
              End If
           End If
  ...
  '串SO014
    ...
    If gymBehind1.GetValue <> "" Then
       strBehindWhere = " And " & strChoose & " And SO003.ClctDate>=To_Date('" & Format(gymBehind1.GetValue(True) & "/01", "YYYYMMDD") & "','YYYYMMDD')" & _
                        " And SO003.ClctDate<To_Date('" & gdaClctDate1.GetValue & "','YYYYMMDD') And SO003.Period > 0 "
    ...
    strChooseString = "收費日預估範圍: " & subSpace(gdaClctDate1.GetValue(True)) & "~" & subSpace(gdaClctDate2.GetValue(True)) & ";" & _
...
'問題集 2801 將回塑年的資料加到SO5260A3 MDB
        strTemp = "SELECT " & GroupField & ",SO003.ClctDate,Nvl(SO003.Period,0) Period,Nvl(SO003.Amount,0) Amount,Nvl(Sum(SO003.Amount),0) as SumAmount" & _
                 " From " & strTables & " Where " & strWhere & strBehindWhere & " Group By " & strGroupBy & ",SO003.ClctDate,SO003.Period,SO003.Amount"
        SendSQL strTemp, True
...
Sub CreateTable()
  On Error Resume Next
    ...
    cnn.Execute "Drop Table SO5260A5"
  ...
  'SO5260A4 各期統計
    cnn.Execute "Create Table SO5260A4 (" & _
                ...
                "CountCust12 Long default 0)"
  'SO5260A5 各期統計彙總
    cnn.Execute "Create Table SO5260A5 (Period Long Default 0,Total Long default 0)"
   Exit Sub
ChkErr:
...
Private Sub toExcel(Optional ByVal strSQL As String = "", Optional ByVal strSubQry1 As String = "")
  On Error GoTo ChkErr
    ...
    Call UseProperty(rsExcel, "預收資料一覽表", "第一頁", strShowField, , rsSubExcel1, _
                    , , , , , , , , , , , , , , , , , , , , , , , , , , , , , , , , , strChooseString)
