...
Private Sub subChoose()
  ...
  '網路編號
    ...
    If strChooseSO003 <> "" Then
        ...
        '多增加此行，因為使用到SO003，但沒有使用到SO003任何條件，導致速度整個被Down下來，此行可過濾SO003條件
        strChooseNot = strChooseNot & " And SO002.CustId = SO003.CustId And SO002.CompCode = SO003.CompCode And SO002.ServiceType = SO003.ServiceType And " & strChooseSO003
    End If
    ...
    strChooseString = "公司別　  :" & subSpace(gilCompCode.GetDescription) & ";" & _
                      ...
                      "下次收費日:" & subSpace(gdaClctDate1.GetValue(True)) & "~" & subSpace(gdaClctDate2.GetValue(True)) & ";" & _
  ...
  Call CreateTable
    ...
    If chkYes.Value = 1 Then
        strMDB = "Select A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid,Count(Distinct A.CustId) As intCount  From(" & _
                 ...
                 "Select SO003.CMCode TypeCode,SO003.CMName TypeName," & strField & strMduid & " ,SO001.CustId From " & strCMFrom & strChooseNot & strTitleChoose & strCMSQL & " And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3)) A " & _
    ...
    If chkYes.Value = 1 Then
        strMDB = "Select A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid,Count(Distinct A.CustId) As intCount  From(" & _
                 ...
                 "Select SO001.ClassCode1 TypeCode,SO001.ClassName1 TypeName," & strField & strMduid & " ,SO001.CustId From  " & strChooseNot & strTitleChoose & " And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3)) A " & _
                 " Group by A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid "
...
'設備項目
    If blnSO004 Then
        ...
        strFaciWhere2 = strChooseNot & strTitleChoose & " And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3)) A "
    Else
        ...
        strFaciWhere2 = "SO004, " & strChooseNot & strTitleChoose & " And SO004.CustId = SO001.CustId " & IIf(gilServiceType.GetCodeNo = "", "", " And SO004.ServiceType ='" & gilServiceType.GetCodeNo & "' ") & " And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3) And SO004.InstDate is Not Null And SO004.PRDate is Null ) A "
    End If
    If chkYes.Value = 1 Then
        strMDB = "Select A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid,Count(A.TypeCode) As intCount  From (" & _
  ...
  'NodeNo
    ...
    Else
        ...
        strFaciWhere2 = "SO014, " & strChooseNot & strTitleChoose & " And SO001.InstAddrNo = SO014.AddrNo And SO001.CompCode = SO014.CompCode And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3)) A "
    End If
    If chkYes.Value = 1 Then
        strMDB = "Select A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid,Count(Distinct A.CustId) As intCount  From (" & _
  ...
  '工期類別
    ...
    If chkYes.Value = 1 Then
        strMDB = "Select A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid,Count(Distinct A.CustId) As intCount  From(" & _
                 ...
                 "Select SO003.CitemCode TypeCode,SO003.CitemName TypeName," & strField & strMduid & " ,SO001.CustId From " & Left(strChoose, InStr(strChoose, "W") - 1) & IIf(InStr(strChoose, "SO003") = 0, ",SO003 ", "") & _
                 Right(strChoose, Len(strChoose) + 1 - InStr(strChoose, "W")) & " AND SO001.CustId=SO003.CustId  And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3)) A " & _
                 " Group by A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid"
    Else
    ...
    If chkYes.Value = 1 Then
        strMDB = "Select A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid,Count(Distinct A.CustId) As intCount  From(" & _
                 "Select SO001.ClassCode2 TypeCode,SO001.ClassName2 TypeName," & strField & strMduid & " ,SO001.CustId From  " & strChoose & " And SO001.ChargeType in (1,2) And SO001.ClassCode2 is not null Union All " & _
                 "Select SO001.ClassCode2 TypeCode,SO001.ClassName2 TypeName," & strField & strMduid & " ,SO001.CustId From  " & strChooseNot & strTitleChoose & " And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3 And SO001.ClassCode2 is not null)) A " & _
                 " Group by A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid "

    Else
        strMDB = "Select SO001.ClassCode2 TypeCode,SO001.ClassName2 TypeName," & strField & _
                  strMduid & " ,Count(Distinct SO001.CustId) As intCount  From  " & strChoose & _
                  " And SO001.ClassCode2 is not null Group by SO001.ClassCode2,SO001.ClassName2, " & strGroupField & strGroup
    End If
    ...
    If chkYes.Value = 1 Then
        strMDB = "Select A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid,Count(Distinct A.CustId) As intCount  From(" & _
                 "Select SO001.ClassCode3 TypeCode,SO001.ClassName3 TypeName," & strField & strMduid & " ,SO001.CustId From  " & strChoose & " And SO001.ChargeType in (1,2) And SO001.ClassCode3 is not null Union All " & _
                 "Select SO001.ClassCode3 TypeCode,SO001.ClassName3 TypeName," & strField & strMduid & " ,SO001.CustId From  " & strChooseNot & strTitleChoose & " And SO001.MduId in (Select Distinct(SO001.MduId) as MduId From " & strChoose & " And SO001.ChargeType=3 And SO001.ClassCode3 is null)) A " & _
                 " Group by A.TypeCode,A.TypeName,A.ASCode,A.ASName,A.Mduid "

    Else
        strMDB = "Select SO001.ClassCode3 TypeCode,SO001.ClassName3 TypeName," & strField & _
                  strMduid & " ,Count(Distinct SO001.CustId) As intCount  From  " & strChoose & _
                  " And SO001.ClassCode3 is not null Group by SO001.ClassCode3,SO001.ClassName3, " & strGroupField & strGroup
    End If
