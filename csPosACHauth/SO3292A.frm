                  ...
                  'strDate 為取匯入檔案首錄資料的交易並成西元年,更新so106的核準日期;問題集1957  --------for Crystal 95/02/22
                    ...
                    If Mid(strData, 107, 1) = "R" Then
                        ...
                        strSQL = "Select RowId,CUSTID,MasterId From " & GetOwner & "SO106 A Where ACHCUSTID='" & GetACHCustID(strData) & _
                                ...
                                IIf(UCase(Left(CmbApplyType.Text, 1)) = "D", "", " And StopFlag<>1")
                        
                       
                        
                        If Not GetRS(rsCustid, strSQL, gcnGi, adUseClient, adOpenKeyset, adLockOptimistic, , , True) Then Exit Sub
                        If Not rsCustid.EOF Or Not rsCustid.BOF Then    '在SO106中沒有ACHCUSTID時會出LOG
                            ...
                            '#3946 改用MasterId By Kin 2008/05/30
                            If Not GetRS(rsSO106A, GetSO106A(UCase(Mid(strData, 71, 1)), rsClone("MasterId"), strData), gcnGi, adUseClient, adOpenKeyset, adLockOptimistic) Then Exit Sub
                            '*************************************************************************************
                            '驗證回覆訊息是否為成功
...
'                                                gcnGi.Execute strSql
                        ...
                        End If
                    Else
                        MsgBox "格式錯誤！！": gcnGi.RollbackTrans: Exit Sub
                    End If
                End If
            End With
    ...
    Do While Not rs.EOF
        ...
        Do While Not rsSO106A.EOF
            ...
            If rsSO106A("CitemCodeStr") & "" <> "" And ChkInSO106ACitem(gimCitemCode.GetQueryCode, rsSO106A("CitemCodeStr") & "", IIf(gimCitemCode.GetQueryCode = Empty, True, False)) Then
                ...
                strData = "1"
                '委託機構代號
                strData = strData & GetString(rs("BankCode"), 3, giLeft, False) & ""
                '保留欄
...
'    End Select
    ...
    If Len(gilBank.GetCodeNo & "") > 0 Then
        subAnd ("BankCode = " & gilBank.GetCodeNo)
    End If
...
Private Function InsertFinal(rs As ADODB.Recordset) As Boolean
  On Error GoTo ChkErr
    ...
    strData = "2"
    '委託機構代號
    strData = strData & GetString(gilBank.GetCodeNo & "", 3, giLeft, False)
    '保留欄
    strData = strData & Space(4)
    '媒體產生日期
    strData = strData & GetString(GetRealDateTran(gdaSendDate.GetValue), 8, giRight, True)
