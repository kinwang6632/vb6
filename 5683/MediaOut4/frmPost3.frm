...
Private Function BeginTran() As Boolean
  ...
  On Error GoTo ChkErr
    ...
    If intRefNo <> 2 Then
        ...
        Else
            ...
            strSQL = "SELECT " & GetUseIndexStr("SO033", "ShouldDate") & _
                    ...
                    " A.AccountNo,TO_CHAR(A.ShouldDate,'YYYYMM') as ShouldDate, " & _
                    " Max(Nvl(A.PayKind,0)) PayKind," & strMinRealStopDate & _
                    " From " & strGetOwner & "SO033 A " & StrTableName & _
                    " Where " & strWhere & _
    ...
    Else
        ...
        End If
        strSQL = "SELECT " & GetUseIndexStr("SO033", "ShouldDate") & _
                " A.CitibankATM BillNo,Sum(A.ShouldAmt) ShouldAmt,A.AccountNo," & _
                " TO_CHAR(A.ShouldDate,'YYYYMM') as ShouldDate,A.BillNo OldBillNo, " & _
                " Max(Nvl(A.PayKind,0)) PayKind," & strMinRealStopDate & _
               "From " & strGetOwner & "SO033 A " & StrTableName & _
               " Where " & strWhere & _
               IIf(Len(strWhere) = 0, "", " And ") & strChoose & " Group By A.AccountNo,A.CitibankATM,A.ShouldDate,BillNo Order by BillNo"
...
'              IIf(Len(strWhere) = 0, "", " And ") & strChoose
    strUPDUCCode = GetUpdateSQL(IIf(intRefNo = 2, 3, Val(strMedia)), StrTableName, strWhere, strChoose)
    '**************************************************************************************************
...
'    If Not GetRS(rsTmp, strSQL, gcnGi) Then Exit Function
    ...
    Else
        ...
        While Not rsTmp.EOF
            ...
            If Not IsPayKindOK(rsTmp, gdaTransfer.GetValue & "", giAll, IIf(intRefNo = 2, 3, Val(strMedia))) Then
            ...
            '計算扣帳總金額
             lngTotAmt = lngTotAmt + lngShouldAmt
              ...
              With rsDefTmp
                   ...
                   '繳費帳號 20-33
                   strAccNameID = GetRsValue("Select AccountNameID From " & strGetOwner & "SO106 Where AccountID='" & Mid(GetFieldValue(rsTmp, "AccountNo") & "", 1, 14) & "' And StopFlag<>1 ", gcnGi) & ""
...
lNextRcd:
        ...
        If blnUpdUcCode Then
            ...
            Do While Not rsUpdUCCode.EOF
                ...
                If IsPayKindOK(rsUpdUCCode, gdaTransfer.GetValue & "", giSingle, IIf(intRefNo = 2, 3, Val(strMedia))) Then
