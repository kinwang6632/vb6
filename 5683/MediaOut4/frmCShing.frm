...
Private Function BeginTran() As Boolean
    ...
    '************************************************************************
    '#5683 增加挑選RealStopDate與PayKind By Kin 2010/08/04
    ...
    If strMedia = 0 Then
        strSQL = "SELECT " & GetUseIndexStr("SO033", "ShouldDate") & _
                ...
                " A.CustId,A.MediaBillNo,C.ID,Max(Nvl(A.PayKind,0)) PayKind, " & _
...
'                IIf(Len(strWhere) = 0, "", " And ") & strChoose
    ...
    While Not rsTmp.EOF
        '#5683 如果RealStopDate<畫面條件,則不允許出帳 By Kin 2010/08/04
...
lNextRcd:
    ...
    If blnUpdUcCode Then
        ...
        Do While Not rsUpdUCCode.EOF
            '#5683 RealStopDate> 畫面條件才要Update
