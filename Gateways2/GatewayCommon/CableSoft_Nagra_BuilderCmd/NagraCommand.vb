...
Public Class NagraCommand
    ...
    Private _ConnectErrCnt As Int32 = 0
    Private _SndConnectErrMail As Boolean = False
    Public Property ConnectErrCnt As Int32
        Get
            Return _ConnectErrCnt
        End Get
        Set(ByVal value As Int32)
            _ConnectErrCnt = value
        End Set
    End Property
    Public Property SndConnectErrMail As Boolean
        Get
            Return _SndConnectErrMail
        End Get
        Set(ByVal value As Boolean)
            _SndConnectErrMail = value
        End Set
    End Property
    ...
    Public Sub New(ByVal aSourceId As String, ByVal aDest_Id As String, ByVal aMOP_PPID As String)
        _SourceId = Right("0000" & aSourceId, 4)
        _Dest_Id = Right("0000" & aDest_Id, 4)
        _MOP_PPID = Right("00000" & aMOP_PPID, 5)
    End Sub
    ...
    Public Overloads Function NagraStatus(ByVal aByte() As Byte, ByVal aType As CmdType) As Object
        Try
            ...
            Select Case aType
                ...
                Case CmdType.Cmd0851, CmdType.cmd0126
    ...
    Public Overloads Function GetCMDStatus(ByVal aString As String, ByVal aLen As Int32, ByVal aType As CmdType) As NagraCmdStatus_Type
        ...
        obj.Value.ErrorCode = CustomerNagraError.CableSoft_Initial
        obj.Value.ErrorName = [Enum].GetName(GetType(CustomerNagraError), CustomerNagraError.CableSoft_Initial)
        ...
        Try
            ...
            If aString.Length <> aLen Then
                obj.Value.ErrorCode = CustomerNagraError.CableSoft_Socket_Length_Error
                obj.Value.ErrorName = [Enum].GetName(GetType(CustomerNagraError), CustomerNagraError.CableSoft_Socket_Length_Error)
            Else
                obj.Value.Ack = aString.Substring(FixCmd, 4)

                Select Case aType
                    Case CmdType.Cmd0851
                        obj.Value.LowCmd = "0851"
                        If aString.Substring(FixCmd, 4) = "2005" Then
                            ...
                            End If
                            obj.Value.RcvTranNum = aString.Substring(FixCmd + 4, 9)    '送給Nagra的TranNum
                        Else
                            ...
                            Else
                                ...
                                obj.Value.Nack_Status = aString.Substring(FixCmd + 4 + 9, 1)
                                obj.Value.RcvTranNum = aString.Substring(FixCmd + 4, 9) '送給Nagra的TranNum
                            End If
                            If _ErrCodeLst IsNot Nothing AndAlso _ErrCodeLst.Count > 0 Then
                                If _ErrCodeLst.ContainsKey(obj.Value.ErrorCode) Then
                                    obj.Value.ErrorName = _ErrCodeLst.Item(obj.Value.ErrorCode)
                                End If
                            End If

                        End If
                    Case CmdType.cmd0126
                        obj.Value.LowCmd = "0126"
                        If aString.Substring(FixCmd, 4) = "1003" Then
                            obj.Value.ErrorCode = String.Empty
                            obj.Value.ErrorName = String.Empty
                            obj.Value.Status = AckCode.Ack
                            aObjNagraResult.Value = New Dictionary(Of String, String)
                            aObjNagraResult.Value = GetNagraResult(aString, aType)
                            If aObjNagraResult.Value IsNot Nothing AndAlso aObjNagraResult.Value.Count > 0 Then
                                obj.Value.NUID = aObjNagraResult.Value.Item("NUID")
                                obj.Value.VUA = aObjNagraResult.Value.Item("VUA")
                            End If
                            obj.Value.RcvTranNum = aString.Substring(FixCmd + 4, 9)    '送給Nagra的TranNum
                        Else
                            If aString.Substring(FixCmd, 4) <> "1003" Then
        ...
        Try
            ...
            Select Case aType
                Case CmdType.Cmd0851
                    ...
                    End If
                Case CmdType.cmd0126
                    obj.Value.Add("NUID", String.Empty)
                    obj.Value.Add("VUA", String.Empty)
                    If aString.Length >= FixCmd + 4 + 9 + 10 + 10 Then
                        obj.Value.Item("VUA") = aString.Substring(FixCmd + 4 + 9, 10)
                        obj.Value.Item("NUID") = aString.Substring(FixCmd + 4 + 9 + 10, 10)
                    End If
            End Select
            Return obj.Value
        Catch ex As Exception
    ...
    Private Function GetCmd1002Status(ByVal aByte() As Byte) As NagraCmdStatus_Type
        ...
        objRet.ErrorName = "CableSoft_Initial"
        ...
        Try
            ...
            If aString.Length <> aLen Then
                ...
                objRet.ErrorCode = -9994
                objRet.ErrorName = "CableSoft_Socket_Length_Error"
            Else
    ...
    Private Function ConfirmConnect() As List(Of Byte())
        ...
        End Try
    End Function
    Private Function GetCMD0126(ByVal aValueLst As Dictionary(Of String, String)) As List(Of Byte())
        'Dim aRet As New ThreadLocal(Of String)
        ...
        Try
            ...
            'aBodyValue.Value = GetBodyCMD0126(aValueLst)
            aValue.Value = GetHeadString(CmdType.cmd0126) & GetSectionString(CmdType.cmd0126, aValueLst) & _
                GetBodyCMD0126(aValueLst)
            ...
            Else
                Select Case aType
                    ...
                    Case CmdType.cmd0126
                        ...
                        'Right("0000000000" & aValueLst.Item("ICC_NO"), 10)
                    Case Else
                        Return String.Empty
                End Select
            End If
    ...
    Private Shared Function StringToUTCDate(ByVal aSourceDate As String) As String
        ...
        Try
            aSourceDate = aSourceDate.Replace("/", "").Replace(":", "").Trim()
        ...
        End Try
    End Function
    Private Shared Function GetBodyCMD0126(ByVal aValueLst As Dictionary(Of String, String)) As String
        
        Using aValue As New ThreadLocal(Of String)
            Try
                aValue.Value = "0126" & Right("0000000000" & aValueLst.Item("NOTES"), 10)
            Catch ex As Exception
                WriteErrTxtLog.WriteTxtError(ex, Nothing, Nothing)
                Return Nothing
...
'End Class
Public Enum CustomerNagraError
