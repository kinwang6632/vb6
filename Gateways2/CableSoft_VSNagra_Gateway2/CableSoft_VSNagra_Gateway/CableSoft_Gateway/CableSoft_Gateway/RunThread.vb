...
Module RunThread
    ...
    Private Const FSQLCMD As String = "SELECT * FROM SEND_NAGRA A WHERE A.CMD_STATUS='W' " & _
        " AND ROWNUM <= {0} AND  A.GATEWAYTYPE=1 AND COMPCODE = {1} " & _
        " ORDER BY A.SEQNO "
    ...
    Private Sub ThreadCmd(ByVal obj As SOInfoClass)
        ...
        Try
            ...
            SyncLock obj
                ...
                aId.Value = Guid.NewGuid.ToString   'Format(Date.Now, "yyyyMMddHHmmssff")
        ...
        End Try
    End Sub
    Private Sub ReadOK(ByVal obj As SOCMDInfo)

        Dim aThreadPoolOK As New ThreadLocal(Of Boolean)
        Dim objCollect As New ThreadLocal(Of List(Of Object))
        objCollect.Value = New List(Of Object)
        aThreadPoolOK.Value = False
        Try
            Interlocked.Increment(FThreadsTotal)    '使用的Thread加1,UPDUIANDDB會扣回來
            If (FNagraHaveProcing < FNagraMaxProcing) Then
                '判斷Socket有連線就送資料
                If (fSocket(fSocketNum) IsNot Nothing) AndAlso (fSocket(fSocketNum).Connected) Then
                    aThreadPoolOK.Value = True    '其中一個成功就設成功,後面的命令可以繼續送

                    objCollect.Value.Add(fSocketNum)
                    objCollect.Value.Add(obj)
                    '將處理的資料丟至ThreadPool
                    ThreadPool.QueueUserWorkItem(AddressOf SendNagraCmd, objCollect.Value)


                    'evn.Set()
                End If

                Interlocked.Increment(fSocketNum)
                If fSocketNum >= fSocket.Length Then
                    Interlocked.Exchange(fSocketNum, 0)
                    '沒有任何Socket可以連線,就將資料設成失敗

                    If Not aThreadPoolOK.Value Then
                        Try
                            obj.AllFieldsValue("Cmd_Status".ToUpper) = "E"
                            obj.AllFieldsValue("Err_Code".ToUpper) = "-9997"
                            obj.AllFieldsValue("Err_Msg".ToUpper) = "CableSoft_Socket_All_Disconnect"
        ...
        Try
            ...
            If (obj.OracleCn.State = ConnectionState.Closed) Then
                ...
                Using cmd As OracleCommand = obj.OracleCn.CreateCommand
                    cmd.CommandText = String.Format(FSQLCMD, FProcessNumber, obj.CompID)
                    Using r As OracleDataReader = cmd.ExecuteReader(CommandBehavior.CloseConnection)
                        ...
                        While r.Read
                            Try
                                ...
                                ReadOK(SendCmdInfo.SetSOCMDInfo(obj.CompID, obj.CompName, r("SeqNo"), r("HIGH_LEVEL_CMD_ID"), obj.OraConnectString, r))
    ...
    Private Sub SendNagraCmd(ByVal aInfo As List(Of Object))
        ...
        Try
            ...
            SyncLock FSOIndex(aSoCmdInfo.Value.CompId)
               
                If FSOIndex(aSoCmdInfo.Value.CompId).ProcessingNumber > 0 Then
                    FSOIndex(aSoCmdInfo.Value.CompId).ProcessingNumber -= 1
                End If
            End SyncLock
    ...
    Private Function UpdSend_Nagra(ByRef objSOCMDInfo As SOCMDInfo) As Boolean
        ...
        Finally
            ...
            SyncLock FSOIndex(objSOCMDInfo.CompId)
                If FSOIndex(objSOCMDInfo.CompId).WaitProcessNumber > 0 Then
                    FSOIndex(objSOCMDInfo.CompId).WaitProcessNumber -= 1
                End If
                If FSOIndex(objSOCMDInfo.CompId).ProcessingNumber > 0 Then
                    FSOIndex(objSOCMDInfo.CompId).ProcessingNumber -= 1
                End If
            End SyncLock
...
Public Class SendCmdInfo
    ...
    Public Shared Function SetSOCMDInfo(ByVal aCompID As String, ByVal aCompName As String, _
        ...
        Dim obj As New ThreadLocal(Of SOCMDInfo)
        Dim objList As New ThreadLocal(Of Dictionary(Of String, String))
        obj.Value = New SOCMDInfo()
        objList.Value = New Dictionary(Of String, String)
        Try
            'rwl.EnterWriteLock()
            ...
            SyncLock aDataReader
                ...
                For i As Int32 = 0 To aDataReader.FieldCount - 1
                    If aDataReader.IsDBNull(i) Then
                        objList.Value.Add(aDataReader.GetName(i).ToUpper, String.Empty)
                    Else
                        objList.Value.Add(aDataReader.GetName(i).ToUpper, aDataReader.Item(i))
                    End If
                Next
                objList.Value.Add("SOCKETSNDMSG", String.Empty)
                objList.Value.Add("SOCKETRCVMSG", String.Empty)
                objList.Value.Add("GUIDNO", String.Empty)
                objList.Value.Add("TranNum", String.Empty)
                If Not objList.Value.ContainsKey("Returnlicenses".ToUpper) Then
                    objList.Value.Add("Returnlicenses".ToUpper, String.Empty)
                End If
                objList.Value.Item("Cmd_Status".ToUpper) = "P"
                obj.Value.NagraSendAndRecvMsg = New Dictionary(Of String, String)
                obj.Value.CompId = aCompID
                obj.Value.SEQNO = aSEQNO
                obj.Value.CMD = aCMDName
                obj.Value.CompName = aCompName
                obj.Value.OraConnectString = aConnectString
                obj.Value.CmdPriority = 9
                obj.Value.AllFieldsValue = objList.Value
                obj.Value.Returnlicenses = New Dictionary(Of String, String)
                obj.Value.NagraCommandType = New NagraCmdStatus_Type
                obj.Value.NagraCommandType.Status = AckCode.Other
                obj.Value.NagraCommandType.ErrorCode = CustomerNagraError.CableSoft_Initial
                obj.Value.NagraCommandType.ErrorName = [Enum].GetName(GetType(CustomerNagraError), CustomerNagraError.CableSoft_Initial)
