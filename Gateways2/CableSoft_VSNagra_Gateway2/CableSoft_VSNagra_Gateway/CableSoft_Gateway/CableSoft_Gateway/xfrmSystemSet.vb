...
Public Class xfrmSystemSet
    ...
    Private Sub btnTestConn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnTestConn.Click
        Try
            ...
            Dim aSID As String = TreLstDB.FocusedNode.GetValue("DBSid")
            Dim aUserId As String = TreLstDB.FocusedNode.GetValue("DBUser")
            Dim aPassword As String = TreLstDB.FocusedNode.GetValue("DBPassword")
            lblMsg.Text = String.Empty
            If String.IsNullOrEmpty(aSID) Then
                XtraMessageBox.Show("SID未設定", "訊息", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
            If String.IsNullOrEmpty(aUserId) Then
                XtraMessageBox.Show("UserId未設定", "訊息", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
            If String.IsNullOrEmpty(aPassword) Then
                XtraMessageBox.Show("Password未設定", "訊息", MessageBoxButtons.OK, MessageBoxIcon.Information)
                Exit Sub
            End If
