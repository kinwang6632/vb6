...
Begin VB.Form frmSO3320A 
   ...
   Begin Gi_Date.GiDate gdaClctDate2 
      ...
      EndProperty
   End
   Begin CS_Multi.CSmulti gimAccIdP1 
      Height          =   225
      Left            =   5250
      TabIndex        =   109
      Top             =   1650
    ...
    Dim strHead As String
        ...
        If lngAffected > 0 Then
            strHead = "BillNo as 單據編號,CustId as 客戶編號,CustName as 客戶姓名,Tel1 as 電話1,Tel3 as 電話3,CitemCode as 收費項目代碼,CitemName as 收費項目, " & _
                            ...
                            "BpCode as 優惠組合代碼,BpName as 優惠組合名稱,PayKindName as 繳付類別,FaciSNO as 物品序號"
                            
                            
                            'iif(PayKind=0,'預付制','現付制') as 繳付類別
            ...
            '#7074 Add SQL-Contidion into SO054 Table By Kin 2015/09/15
            ExportLog Replace(UCase(Screen.ActiveForm.Name), "FRM", ""), _
                        garyGi(0), Format(NowTime, "YYYYMMDD HHMMSS"), SecToHMS(LastTime - startTime), GetQryStr, RightNow
    ...
    Dim strSO003sql As String
        ...
        While Not rsTmp.EOF
            ...
            If rsTmp("AccountNo") & "" <> "" Then
                ...
                Else
                    strCardType = GetRsValue("Select CardName From " & GetOwner & "SO002A Where CustId = " & rsTmp("CustId") & " And AccountNo = '" & rsTmp("AccountNo") & "'" & IIf(strChooseSO002A <> "", "And ", "") & strChooseSO002A) & ""
                End If
            End If
...
'                strClctDate = GetRsValue(strSO003sql)
            ...
            '#5683 增加繳付類別 By Kin 2010/08/18
            '#7603 Add the field of FaciSNo By Kin 2017/08/21
            strInstrMDB = "Insert Into SO3320A1 (CustId,BillNo,CustName,Tel1,CitemName,RealDate,RealPeriod,RealAmt," & _
                            ...
                            "OldClctEn,AreaCode,ServCode,InvNo,ClctAreaCode,ClctAreaName," & _
                            "UpdTime,MduCount,STName,ClctEnWorkClass," & _
                            "CardType,ManualNo,AccountNo,FaciSeqNo,DiscountCode," & _
                            "DiscountName,DiscountDate1,DiscountDate2,MediaBillNo," & _
                            "Tel3,ClassCode,ClassName,PTCode,PTName,BpCode,BpName,PayKind,PayKindName,FaciSNO) Values (" & _
            ...
            strInstrMDB = strInstrMDB & GetNullString(rsTmp("CustStatusName")) & "," & GetNullString(rsTmp("ClctDate"), giDateV, giAccessDb) & "," & _
                            ...
                            GetNullString(rsTmp("PayKind")) & "," & GetNullString(rsTmp("PayKindName")) & "," & GetNullString(rsTmp("FaciSNO")) & ")"
...
Private Function GetStrSQL1(ByRef strSQL As String, ByVal StrTableName As String) As Boolean
    ...
    Dim strTel1 As String
        ...
        If strChooseSO002A <> "" Then
            strWhere = "And SO033.CustId=SO002A.CustId And SO033.AccountNo=SO002A.AccountNo And " & strChooseSO002A
            strTableName2 = "," & GetOwner & "SO002A"
        End If
        ...
        strSQL2 = "Select " & strUseIndexStr & "  SO033.CustId,SO033.BillNo," & _
                 ...
                 IIf(strChooseSO002A <> "", "SO002A.CardName", "to_char(null)") & " as CardType," & _
                 ...
                 "SO033.MediaBillNo,SO001.Tel3,SO033.ClassCode," & _
                 "CD004.Description ClassName,SO033.PTCode," & _
                 "SO033.PTName,SO033.BpCode,SO033.BPName," & _
                 "Nvl(SO033.PayKind,0) PayKind,CD112.Description PayKindName,SO033.FACISNO From " & _
                 ...
                 "," & GetOwner & "CM001," & GetOwner & "CD019, " & GetOwner & "CD112 " & _
                 ...
                 " And SO033.CustId = SO003.CustId(+) And SO033.CompCode = SO003.CompCode(+) " & _
                 " And SO033.ServiceType = SO003.ServiceType(+) And SO033.CitemCode = SO003.CitemCode(+) " & _
                 " And SO033.FaciSeqNo = SO003.FaciSeqNo(+) AND NVL(SO033.PAYKIND,0)=CD112.CODENO " & strWhere & " And So033.ServCode=Cd002.CodeNo And So033.AreaCode=Cd001.CodeNo And So033.ClassCode=Cd004.CodeNo And SO033.ClctAreaCode = CD040.CodeNo(+) And " & _
                 " SO033.FaciSeqNO=SO004.SEQNO(+) And SO033.CustId=SO004.CustId(+) And " & strChoose
...
Private Sub subChoose()
    ...
    Dim strField As String
        ...
        If gimPayType.GetQryStr <> "" Then
            Call subAnd("Nvl(SO033.PayKind,0) " & gimPayType.GetQryStr)
        End If
        ...
        If gimSuite.GetQryStr <> "" Then
            subAnd "SO001.AMduId  " & gimSuite.GetQryStr
        Else
            If gimSuite.GetDispStr <> "" Then subAnd "SO001.AMduId Is Not Null"
        End If
        '#6967增加優惠組合條件 By Kin 2015/01/12
        If gimBP.GetQryStr <> "" Then
            subAnd "SO033.BPCode " & gimBP.GetQryStr
        Else
            If gimBP.GetDispStr <> "" Then subAnd "So033.BPCode Is Not Null"
        End If
        ...
        If gdtUpdTime1.GetValue <> "" Then Call subAnd("GiPackage.GetDate1(SO033.UpdTime) >= " & GetNullString(gdtUpdTime1.GetValue(True), giDateV, giOracle, True) & "")
        If gdtUpdTime2.GetValue <> "" Then Call subAnd("GiPackage.GetDate1(SO033.UpdTime) <= " & GetNullString(gdtUpdTime2.GetValue(True), giDateV, giOracle, True) & "")
...
End Function
Private Sub GetAccIdP1Citem()
  On Error GoTo ChkErr
    '#6122 報表顯示Where的條件增加會計科目所對應的收費項目 By Kin 2012/05/09
    Dim rsTmp As New ADODB.Recordset
    Dim aSQL As String
    gimAccIdP1.Clear
    If Len(gimCM001.GetDispStr) <= 0 Then
        CloseRecordset rsTmp
        Exit Sub
    End If
    aSQL = "SELECT CodeNo FROM " & GetOwner & "CD019 " & _
            "WHERE AccIdP1 " & gimCM001.GetQryStr
    If Len(gimCitemCode.GetQryStr) > 0 Then
        aSQL = aSQL & " AND  CODENO NOT IN (SELECT CODENO FROM " & GetOwner & "CD019 WHERE CODENO IN  " & gimCitemCode.GetQryStr & ")"
    End If
    If Not GetRS(rsTmp, aSQL, gcnGi, adUseClient, adOpenKeyset, adLockOptimistic) Then Exit Sub
    If Not rsTmp.EOF Then
        Dim strCode As String
        strCode = rsTmp.GetString(adClipString, , , ",")
        gimAccIdP1.SetQueryCode Mid(strCode, 1, Len(strCode) - 1)
    End If
'    Do While Not rsTmp.EOF
'
'        rsTmp.MoveNext
'    Loop
'    If Len(gimCitemCode.GetDispStr) > 0 Then
'        If Len(strRet) > 0 Then strRet = "," & strRet
'    Else
'        strRet = Mid(strRet, 2, Len(strRet) - 1)
'    End If
'
    
  Exit Sub
ChkErr:
  Call ErrSub(Me.Name, "GetAccIdP1")
End Sub
...
Private Function GetQryStr() As String
    ...
    Dim str1 As String
        ...
        strChooseString = "公司別:" & subSpace(gilCompCode.GetDescription) & ";" & _
                              ...
                              "收費項目:" & subSpace(gimCitemCode.GetDispStr) & IIf(Len(gimCitemCode.GetDispStr) > 0, ",", "") & subSpace(gimAccIdP1.GetDispStr) & ";" & _
        ...
        GetQryStr = Mid(strChooseString, 1, 500)
    Exit Function
ChkErr:
...
Private Sub subGim()
    On Error GoTo ChkErr
        ...
        SetgiMulti gimAccIdP1, "CodeNo", "Description", "CD019", "收費項目代碼", "收費項目名稱"
        ...
        '#5683 增加繳費類別 By Kin 2010/08/18
        'Call SetgiMultiAddItem(gimPayType, "0,1", "預付制,現付制", "代碼", "名稱")
        SetgiMulti gimPayType, "CodeNo", "Description", "CD112", "代碼", "名稱"
        '#6967 增加優惠組合 By Kin 2015/01/12
        SetgiMulti gimBP, "CodeNo", "Description", "CD078", "代碼", "名稱"
