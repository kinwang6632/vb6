...
Private Function GetChkPayKind() As Boolean
    ...
    Dim aRet As Boolean
    aRet = False
    aRet = Val(cn.Execute("SELECT NVL(PayNowChkStopDate,0) FROM " & TableOwnerName & "SO041")(0)) = 1
    GetChkPayKind = aRet
...
'' *****************************************************************************************************
    ...
    Else
     ...
     '找出客戶正式應收總金額
        ...
        '#7179 如果跨客編要把客編欄位先抓出來 By Kin 2016/02/22
        strsql = "SELECT A.SHOULDAMT,A.ACCOUNTNO," & _
            ...
            " WHERE A.ACCOUNTNO=C.ACCOUNTID AND C.STOPFLAG <> 1 " & _
            " AND SnactionDate IS NOT NULL AND A.CUSTID=C.CUSTID " & strCardNameSQL & _
            " GROUP BY A.SHOULDAMT,A.ACCOUNTNO,A.MEDIABILLNO " & IIf(isCrossCustCombine, ",A.CustId", "") & _
...
'                                                          " Where A.MainCustId = Custid And A.AmdUid = AmduId And Nvl(A.MEDIABILLNO,'X')= Nvl(MEDIABILLNO,'X') " & _
...
'                                                  " End)  from dual) CustIdExistFlag  from " & TableOwnerName & sqlTmpViewName & " A"
            strsql = " select A.*,(select (case " & _
                                  "                when amduid is null then 1 Else " & _
                                                  ...
                                                  " End)  from dual) CustIdExistFlag  from " & TableOwnerName & sqlTmpViewName & " A"
            strsql = "Select Decode(CustIdExistFlag,0,CustId, MainCustId ) CustId,Nvl(MEDIABILLNO,'X') MEDIABILLNO,AccountNo,Max(PayKind) PayKind,Min(RealStopDate) RealStopDate," & _
                            " Sum(ShouldAmt) ShouldAmt,AMDUID,CustIdExistFlag " & _
                    " From ( " & strsql & " ) A Group By Decode(CustIdExistFlag,0,CustId, MainCustId ),MEDIABILLNO,AccountNo,AMDUID,CustIdExistFlag "
           '將CardExpDate 找出來
            strsql = "Select distinct A.*, " & _
                        " (select CardExpDate from " & TableOwnerName & sqlTmpViewName & _
                                    " where a.MEDIABILLNO=MEDIABILLNO  and  A.AccountNo = AccountNo And rownum<=1) CardExpDate " & _
                        " From (" & strsql & ") A"
           ...
           ''2005/02/02 將整理好的資料  依照單據排序之後  將同單號正負項合併
            ...
            If isCrossCustCombine Then
                If Val(rsTmp("CustId") & "") = -1 Then
                    ...
                    strErrMessage = "單據編號： " & rsTmp(strBillNOField) & "套房找不到統收戶客戶編號，請設定統收戶客戶編號"
                    file(0).WriteLine strErrMessage
                    GoTo NextLoop
                End If
'                If Val(rsTmp("CustIdExistFlag") & "") = 0 Then
'                    lngErrCount = lngErrCount + 1
'                    strErrMessage = "單據編號：[ " & rsTmp(strBillNOField) & " ]　出帳資料沒有包含統收戶客戶編號：" & rsTmp("CustId")
'                    file(0).WriteLine strErrMessage
'                    GoTo NextLoop
'                End If
           ...
           '7179
            ...
            Call GetRS(rsCustIDErr, "SELECT  distinct SO001.CUSTNAME , SO106.CUSTID FROM " & TableOwnerName & "SO106," & TableOwnerName & "SO001  " & _
                                 ...
                                 "Select DISTINCT CUSTID From " & TableOwnerName & "SO033 Where " & strBillNOField & "='" & _
                                  rsTmp(strBillNOField) & "' And AccountNo='" & rsTmp("AccountNO") & "')" & _
                                  IIf(isCrossCustCombine, " And SO106.CustId =" & rsTmp("CustId"), ""), cn)
            ...
            '#5843 如果參數設定不檢核則強迫出帳 By Kin 2010/11/24
            If (Not IsPayKindOK(rsTmp, gdaSystemDate.GetValue & "", giAll, intPara24)) _
                And (blnChkPayKind) Then
                lngErrCount = lngErrCount + 1
                strErrMessage = "收視截止日不正確 --> 收視截止日大於扣款處理日 :單號  " & rsTmp(strBillNOField) & " 客戶姓名 : " & rsCustIDErr("CustName") & _
   ...
   lngerror = 6
         ...
         ''  strCardExpDate = GetNullString(rsTmp("CardExpDate"), giStringV, giAccessDb)
            ...
            If strCardExpDate = "NULL" Or strCardExpDate = "" Then
                lngErrCount = lngErrCount + 1
...
'           strCardExpDate = Right(strCardExpDate, 4) & "/" & Mid(strCardExpDate, 1, iCED)
          ...
          lngerror = 112
          If (theSingalAmount < 0 Or theSingalAmount = 0) And (Not objStorePara.uOutZero) Then
        ...
        If cboBillType.ListIndex = 2 Then
            strSpcNo = "Select SPCNO From CD037 WHERE CARDID IN (Select DISTINCT To_Char(CARDCODE) FROM " _
