...
Private blnExcI As Boolean
Private aNow As String
Private strBaseKey  As String
...
Private Sub cmdOK_Click()
 ...
 '''If Not ChkDTok Then Exit Sub
    ...
    aNow = GiDAccept.GetValue(False) & "080000"
...
Private Sub Form_Load()
    On Error Resume Next
      ...
      End If
     If frmCreditCardType = lngFubon Then
        If blnFubonIntegrate = True Then
            txtSpcNO.text = strSpcNo2
        Else
            txtSpcNO.text = strSpcNo
...
Private Sub ReadTxtToScr()
  On Error Resume Next
    ...
    Set LogFile = Nothing
    interr = 1
    If Dir(strPath & "\" & strPrgName & "Out.log") = "" Then
        GoTo 88
        'Exit Sub
    End If
    interr = 2
    ...
    With LogFile
        ...
        If Not .AtEndOfStream Then txtAuthorizationBatch.text = .ReadLine
        'If Not .AtEndOfStream Then cboMonthDay.Text = .ReadLine & "":    interr = 8
    End With
88:
    '#5919 測試不OK,如果沒有LOG檔會沒有效果 By Kin 2011/08/05
    '#6130 SPCNO開放至15碼,不然PM會認定是BUG By Kin 2011/09/21
    Select Case frmCreditCardType
        Case lngChinatrust
            txtMerchantName.MaxLength = 15
            txtMerchantName.text = strDefSpcNo
        Case Else
            txtSpcNO.MaxLength = 15
            txtSpcNO.text = strDefSpcNo
...
'        End If
    ...
    '#5919 增加預設特店代碼 By Kin 2011/06/03
    '#5919 測試不OK 中國信託(新)預設值位置不一樣 By Kin 2011/07/18
'    If frmCreditCardType = lngCtrust2 Then
'        txtSpcNO.Text = strDefSpcNo
'    Else
'        txtMerchantName.Text = strDefSpcNo
...
'  Set o = CreateObject("CreditCardOut.clsInterface")
    ...
    End With
    '#7738 2018/04/10 By Kin
    If blnExcI Then
        strChoose = strChoose & " And  substr(A.billno,7,1) || A.BillMark <> 'I' "
    End If
...
End Sub
Private Sub DefRs_7(ByRef rsDef As Recordset)
  On Error GoTo ChkErr
    With rsDef
        .Fields.Append "F1-1", adBSTR, 1, adFldIsNullable
        .Fields.Append "F2-17", adBSTR, 16, adFldIsNullable
        .Fields.Append "F18-42", adBSTR, 25, adFldIsNullable
        .Fields.Append "F43-44", adBSTR, 2, adFldIsNullable
        .Fields.Append "F45-45", adBSTR, 1, adFldIsNullable
        .Fields.Append "F45-56", adBSTR, 11, adFldIsNullable
        .Fields.Append "F57-92", adBSTR, 36, adFldIsNullable
        .Fields.Append "F93-94", adBSTR, 2, adFldIsNullable
        .Fields.Append "F95-134", adBSTR, 40, adFldIsNullable
        .Fields.Append "F135-144", adBSTR, 10, adFldIsNullable
        .Fields.Append "F145-184", adBSTR, 40, adFldIsNullable
        .Fields.Append "F185-192", adBSTR, 8, adFldIsNullable
        '8575 stop checking the bank by kin 2020/02/21
        If blnFubonIntegrate And 1 = 0 Then
            .Fields.Append "F193-205", adBSTR, 13, adFldIsNullable
            .Fields.Append "F206-221", adBSTR, 16, adFldIsNullable
            .Fields.Append "F222-300", adBSTR, 79, adFldIsNullable
        Else
            .Fields.Append "F193-300", adBSTR, 108, adFldIsNullable
        End If
    ...
    Exit Sub
ChkErr:
  Call ErrSub(Me.Name, "DefRs_7")
End Sub
Private Function GetTmpViewName(cn As ADODB.Connection) As String
  On Error Resume Next
    Dim strTmpViewName As String
    Dim strGetOwner As String
    
    strGetOwner = TableOwnerName
    strTmpViewName = "TMP_" & GetRsValue("SELECT trim(To_Char(" & TableOwnerName & "S_TMPRPT_ViewName.NextVal, '0999999')) FROM Dual", cn)
...
Private Function BeginTran() As Boolean
  On Error GoTo ChkErr
    ...
    Dim strPrepareEncrypt As String
    begintrnsErrCode = 0
    lngShouldAmt = 0
    '#5843 增加檢核參數 By Kin 2010/11/24
    blnChkPayKindOK = GetChkPayKind
    ...
    intPara24 = 1
    '#7179
    isCrossCustCombine = Val(GetRsValue("Select Nvl(CrossCustCombine,0) From " & TableOwnerName & "SO041", cn) & "")
     If Len(sqlTmpViewName) > 0 Then
        cn.Execute "Drop View " & TableOwnerName & sqlTmpViewName
    ...
    '' 20050203 移除And A.ShouldAmt > 0的條件限制
        ...
        strsql = "SELECT A." & strMediaBillNOField & " BILLNO ,ShouldAmt," & _
         ...
         If isCrossCustCombine Then
            ...
            sqlTmpViewName = GetTmpViewName(cn)
            ...
            strsql = "Select A.*," & _
                        ...
                        " (Case " & intCrossCustCombine & " When 1 Then " & _
                                       " ( Case  When AMDUID Is Null Then A.CustId Else Nvl((Select MainCustId From " & TableOwnerName & "SO202 Where SO001.AMduId = SO202.MduId  ),-1)  End ) " & _
                        "  Else A.CustId  End ) MainCustId " & _
                        " From (" & strsql & ") A," & TableOwnerName & "SO001 " & _
                " Where A.CustId=SO001.CustId"
            strsql = "Create View " & TableOwnerName & sqlTmpViewName & " As (" & strsql & ")"
            cn.Execute strsql
...
'                                                          " ( select  count(1) from " & TableOwnerName & sqlTmpViewName & _
'                                                          " Where A.MainCustId = Custid And A.AmdUid = AmduId And Nvl(A.BillNo,'X')= Nvl(BillNo,'X') " & _
'                                                           " And A.AccountNo = AccountNo ) " & _
'                                                  " End)  from dual) CustIdExistFlag  from " & TableOwnerName & sqlTmpViewName & " A"
            strsql = " select A.*,(select (case " & _
                                  "                when amduid is null then 1 Else " & _
                                                          " ( select  count(1) from " & TableOwnerName & sqlTmpViewName & _
            ...
            '將TCBbudget、PTCode、ServiceType、CVC2、CardExpDate找出來
'            strsql = "Select distinct A.*, " & _
'                        " (select TCBbudget from " & TableOwnerName & sqlTmpViewName & _
'                                    " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) TCBbudget ," & _
'                        " (select PTCode from " & TableOwnerName & sqlTmpViewName & _
'                                    " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) PTCode ," & _
'                        " (select ServiceType from " & TableOwnerName & sqlTmpViewName & _
'                                    " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) ServiceType ," & _
'                         " (select CVC2 from " & TableOwnerName & sqlTmpViewName & _
'                                    " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) CVC2 ," & _
'                        " (select CardExpDate from " & TableOwnerName & sqlTmpViewName & _
'                                    " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) CardExpDate " & _
'                        " From (" & strsql & ") A"
            strsql = "Select distinct A.*, " & _
                        " (select TCBbudget from " & TableOwnerName & sqlTmpViewName & _
                                    " where a.billno=billno and  A.AccountNo = AccountNo And rownum<=1) TCBbudget ," & _
                        " (select PTCode from " & TableOwnerName & sqlTmpViewName & _
                                    " where a.billno=billno and  A.AccountNo = AccountNo And rownum<=1) PTCode ," & _
                        " (select ServiceType from " & TableOwnerName & sqlTmpViewName & _
                                    " where a.billno=billno and  A.AccountNo = AccountNo And rownum<=1) ServiceType ," & _
                         " (select CVC2 from " & TableOwnerName & sqlTmpViewName & _
                                    " where a.billno=billno and  A.AccountNo = AccountNo And rownum<=1) CVC2 ," & _
                        " (select CardExpDate from " & TableOwnerName & sqlTmpViewName & _
                                    " where a.billno=billno  and  A.AccountNo = AccountNo And rownum<=1) CardExpDate " & _
                        " From (" & strsql & ") A"
...
'                                                  " End)  from dual) CustIdExistFlag  from " & TableOwnerName & sqlTmpViewName & " A"
            ...
            strsql = "Select Decode(CustIdExistFlag,0,Custid,MainCustId) CustId,Nvl(BillNo,'X') BillNo,AccountNo,Max(PayKind) PayKind,Min(RealStopDate) RealStopDate," & _
                            " Sum(ShouldAmt) ShouldAmt,AMDUID,CustIdExistFlag " & _
                    " From ( " & strsql & " ) A Group By Decode(CustIdExistFlag,0,Custid,MainCustId),BillNo,AccountNo,AMDUID,CustIdExistFlag "
            '將TCBbudget、PTCode、ServiceType、CVC2、CardExpDate、CustName、CitemCode找出來
'            strsql = "Select distinct A.*, " & _
'                       " (select TCBbudget from " & TableOwnerName & sqlTmpViewName & _
'                                   " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) TCBbudget ," & _
'                       " (select PTCode from " & TableOwnerName & sqlTmpViewName & _
...
'                        " (select CVC2 from " & TableOwnerName & sqlTmpViewName & _
'                                   " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) CVC2 ," & _
'                        " (select CustName from " & TableOwnerName & sqlTmpViewName & _
'                                   " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) CustName ," & _
'                        " (select CitemCode from " & TableOwnerName & sqlTmpViewName & _
'                                   " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) CitemCode ," & _
'                       " (select CardExpDate from " & TableOwnerName & sqlTmpViewName & _
'                                   " where a.billno=billno and custid=maincustid and  A.AccountNo = AccountNo And rownum<=1) CardExpDate " & _
'                       " From (" & strsql & ") A"
            strsql = "Select distinct A.*, " & _
                          ...
                          " (select ServiceType from " & TableOwnerName & sqlTmpViewName & _
                           ...
                           " (select CVC2 from " & TableOwnerName & sqlTmpViewName & _
                                      " where a.billno=billno and  A.AccountNo = AccountNo And rownum<=1) CVC2 ," & _
                           " (select CustName from " & TableOwnerName & sqlTmpViewName & _
                                      " where a.billno=billno  and  A.AccountNo = AccountNo And rownum<=1) CustName ," & _
                           " (select CitemCode from " & TableOwnerName & sqlTmpViewName & _
                                      " where a.billno=billno  and  A.AccountNo = AccountNo And rownum<=1) CitemCode ," & _
                          " (select CardExpDate from " & TableOwnerName & sqlTmpViewName & _
                                      " where a.billno=billno and  A.AccountNo = AccountNo And rownum<=1) CardExpDate " & _
                          " From (" & strsql & ") A"
 ...
 '' errcode**********
        begintrnsErrCode = 9
           ''開始逐筆收集單資料
           ''**********************************************************************************
        Dim strPayKindName As String
        strPayKindName = GetRsValue("SELECT Description FROM " & TableOwnerName & "CD112 WHERE CODENO=1", cn) & ""
        If strPayKindName & "" = "" Then
            strPayKindName = "現付制"
        End If
         If frmCreditCardType = lngFubon Then
...
'' errcode**********
            ...
            If isCrossCustCombine Then
                If Val(rsTmp("CustId") & "") = -1 Then
                    ...
                    strErrMessage = "單據編號： " & rsTmp("BillNo") & "套房找不到統收戶客戶編號，請設定統收戶客戶編號"
...
'                    lngErrCount = lngErrCount + 1
'                    strErrMessage = "單據編號：[ " & rsTmp("BillNo") & " ]　出帳資料沒有包含統收戶客戶編號：" & rsTmp("CustId")
'                    file(1).WriteLine strErrMessage
'                    GoTo NextLoop
'                End If
    ...
    ' Jacky 94/10/05
            ...
            If (Not IsPayKindOK(rsTmp, GiDAccept.GetValue & "", giAll, 1)) _
               And (blnChkPayKindOK) Then
                lngErrCount = lngErrCount + 1
                strErrMessage = "收視截止日不正確 --> 收視截止日大於扣款處理日：單號　" & rsTmp("BillNo") & " 客編：" & rsCustIDErr("CUSTID") & _
                            " 客戶姓名　" & rsCustIDErr("custname") & _
                            " 收視截止日：" & rsTmp("RealStopDate") & " 繳付類別：" & strPayKindName
                file(1).WriteLine strErrMessage
                GoTo NextLoop
...
'' errcode**********
            ...
            '#6441 單張帳單合計總額<=0是否產生 By Kin 2013/05/13
            If rsTmp("ShouldAmt") <= 0 And (Not objStorePara.uOutZero) Then
            ...
            With rsDefTmp
                ...
                Case 0
                    If IsTCB = False Then
                        ...
                        '5902 民國100年要使用00 By Kin 2011/01/10
                        '.Fields("F5661") = Format(GiDAccept.Text, "EEMMDD")
                        .Fields("F5661") = Right(Format(GiDAccept.text, "EEMMDD"), 6)
...
'                    .Fields("STOPYM") = Right(rsTmp("CardExpDate"), 2) & IIf(Len(rsTmp("CardExpDate")) = 6, Left(rsTmp("CardExpDate"), 2), "0" & Left(rsTmp("CardExpDate"), 1))
                    ...
                    .Fields("ACCEPTDATE") = Right(Format(GiDAccept.text, "EEMMDD"), 6)
...
'                        End With
                   ...
                   ' a = EncodeBase64(xorEncrpty(StrConv("5430450000000014   1012   ", vbFromUnicode), "200804141048000001CIARATES"))
                        
                    'b = xorTest(decodeBase64(a), "200804141048000001CIARATES")
     
                    strBaseKey = aNow & GetString(txtAuthorizationBatch.text, 4, GIRIGHT, True) & Mid(rsTmp("BillNo") & "", 1, 8)
                    .Fields("F1-1") = "D"
                    .Fields("F2-17") = GetString(txtMerchantName.text, 16, giLeft)
                    .Fields("F18-42") = GetString(rsTmp("BillNo"), 25, giLeft)
                    .Fields("F43-44") = "00"
                    .Fields("F45-45") = "0"
                    .Fields("F45-56") = GetString(rsTmp("ShouldAmt"), 11, GIRIGHT, True)
                    lngtotal = lngtotal + rsTmp("ShouldAmt")
'                    strPrepareEncrypt = GetString(GetString(rsTmp("AccountNO"), 19, giLeft) & _
'                                                            Right(rsTmp("CardExpDate") & "", 2) & Right(Left(rsTmp("CardExpDate") & "", 4), 2) & _
'                                                            Space(3), 36, giLeft)
                    strPrepareEncrypt = GetString(rsTmp("AccountNO"), 19, giLeft) & _
                                                            Right(rsTmp("CardExpDate") & "", 2) & Right(Left(rsTmp("CardExpDate") & "", 4), 2) & _
                                                            Space(3)
                    .Fields("F57-92") = EncodeBase64(xorEncrpty(StrConv(strPrepareEncrypt, vbFromUnicode), strBaseKey))
                    .Fields("F93-94") = Space(2)
                    .Fields("F95-134") = Space(40)
                    .Fields("F135-144") = Space(10)
                    .Fields("F145-184") = GetString(txtBillMemo.text, 40, giLeft, False)
                    .Fields("F185-192") = Space(8)
                    '8575 stop checking the bank by kin 2020/02/21
                    If frmCreditCardType = lngFubon And blnFubonIntegrate And 1 = 0 Then
                        .Fields("F193-205") = Space(13)
                        .Fields("F206-221") = Left(Format(Now, "mmdd") & Right("000" & txtAuthorizationBatch.text, 1) & GetString(rsTmp("BillNo"), 16, giLeft, True), 16)
                        .Fields("F222-300") = Space(79)
                    Else
                        .Fields("F193-300") = Space(108)
                    End If
...
'            strHeader = strHeader & Space(106)
        ...
        Case 1
            ...
            '#5958 送件日期原本是抓現在日期,現在改為抓畫面日期 By Kin 2011/03/24
            strHeader = ""
            strHeader = GetString(txtSpcNO.text, 6, giLeft, False) & Right(GiDAccept.GetValue, 6) & _
        ...
        Case 6
            strHeader = ""
            strHeader = "HEADER" & Right(Format(GiDAccept.text, "EEMMDD"), 6) & _
        ...
        Case 7
            strHeader = ""
            strHeader = "HBA" & GetString(Left(txtSpcNO.text, 16), 16, giLeft, False) & _
                        aNow & GetString(txtAuthorizationBatch.text, 4, GIRIGHT, True) & _
                        Space(263)
        ...
        If IsTCB = True Then
            ...
            End If
        End If
        '#7765 Trailer Record By Kin 2018/05/09
         If frmCreditCardType = lngFubon Then
            strEndLine = ""
            strEndLine = "T"
            strEndLine = strEndLine & GetString(lngCount, 8, GIRIGHT, True) & _
                              GetString(lngCount, 8, GIRIGHT, True) & _
                              GetString("", 8, GIRIGHT, True) & _
                              GetString(lngtotal, 13, GIRIGHT, True) & _
                              GetString(lngtotal, 13, GIRIGHT, True) & _
                              GetString("", 13, GIRIGHT, True) & _
                              GetString("", 8, GIRIGHT, True) & _
                              GetString("", 8, GIRIGHT, True) & _
                              GetString("", 8, GIRIGHT, True) & _
                              GetString("", 8, GIRIGHT, True) & _
...
Private Function xorEncrpty(text() As Byte, Key As String) As Byte()
  On Error GoTo ChkErr:
       ' Dim res As Byte() = New Byte(text.Length - 1) {}
        Dim res() As Byte
        Dim c As Integer
       ' Dim keyBytes = System.Text.Encoding.UTF8.GetBytes(key)
        Dim keybytes() As Byte
        ReDim Preserve res(UBound(text))
    ...
    Exit Function
ChkErr:
 Call ErrSub(Me.Name, "xorEncrpty")
End Function
...
On Error Resume Next
     ...
     'Set objXML = Nothing
ChkErr:
    Call ErrSub(Me.Name, "EncodeBase64")
End Function
  ...
  On Error GoTo ChkErr
    ...
    With rsDefTmp
        If .BOF Or .EOF Then Exit Function
            ...
            If IsTCB = True Or frmCreditCardType = lngCtrust2 Or frmCreditCardType = lngFubon Then
...
Private Sub ScrToRcd()
    ...
    With LogFile
        ...
        If txtAuthorizationBatch.Visible Then
            .WriteLine (txtAuthorizationBatch.text)
        End If
    End With
...
End Sub
Private Function chkAuthorizationBatch(ByVal str As String) As Boolean
    Dim strerrmsg As String
    If Len(str) > 4 Then
        strerrmsg = "授權批次長度需小於4"
        GoTo Warning
    End If
    If Val(str) > 7999 Then
        strerrmsg = "授權批次需介於0000~7999"
        GoTo Warning
    End If
    chkAuthorizationBatch = True
Exit Function
Warning:
    MsgBox strerrmsg, vbOKOnly + vbInformation, "訊息"
    Exit Function
End Function
Private Function IsDataOk() As Boolean
    ...
    '#7765 By Kin 2018/05/08
    If frmCreditCardType = lngFubon Then
        If ChkFields(True, True, False, False, True, True) = False Then Exit Function
        If chkAuthorizationBatch(txtAuthorizationBatch.text) = False Then Exit Function
    End If
...
Private Function ChkFields(ByVal blnSpcNO As Boolean, ByVal blnMerchantName As Boolean, _
    ...
    If blnMerchantName = True Then
        If Len(txtMerchantName.text & "") = 0 Then strerrmsg = Label8.Caption: txtMerchantName.SetFocus: GoTo Warning
    End If
    If blnSet = True Then
...
Private Sub GiDAccept_GotFocus()
    ...
    If Len(GiDAccept.text) = 0 Then GiDAccept.text = Date
End Sub
