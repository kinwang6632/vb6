...
'    gilCompCode.ListIndex = 1
    strCn = InputBox("連線字串設定", "設定", "Provider=MSDAORA.1;Password=EMCNCC;User ID=EMCNCC;Data Source=RDKNET")
    If strCn <> "" Then
        txtConnect = strCn
    Else
        GoTo ChkErr
    ...
    Exit Sub
ChkErr:
  MsgBox "Form_Load失敗，請重新設定連線字串", vbCritical, "訊息"
End Sub
Private Sub DBConnect()
  On Error Resume Next
    ...
    SetgiList gilCompCode, "CodeNo", "Description", "CD039", , , , , 3, 12
    gilCompCode.ListIndex = 1
    intCompCode = gilCompCode.GetCodeNo
    Call Getowner
    strQry = "Select RowId,A.* From " & strOwnerName & "SO106 A"
    strUpdTime = GetDTString(RightNow)
    strCreateTime = Format(RightNow, "yyyy/mm/dd hh:mm:ss")
