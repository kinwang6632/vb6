...
Public Const FormName = "csCNScontrol"
Public Const strCrlf = vbCrLf & vbCrLf
Public strCPno As String
Public blnAccChk As Boolean
'#4276 判斷是否使用Fttx By Kin 2008/12/23
Public blnUseFttx As Boolean
...
Public Function GetPremission(strMID As String) As Boolean
    ...
    Dim rs As New ADODB.Recordset
        ...
        If gstrUserPremission = "" Then
            If Not GetRS(rs, "Select Mid From " & GetOwner & "So029 Where Link like 'SO1173%' And Group" & GaryGi(4) & " = 1") Then Exit Function
...
End Function
Public Function Get_SO004F_Seq_No(ByRef cnObj As Object) As String
  On Error GoTo ChkErr
    Get_SO004F_Seq_No = RPxx(cnObj.Execute("SELECT " & strOwner & "S_SO004F_SeqNo.NEXTVAL FROM DUAL").GetString & "")
  Exit Function
ChkErr:
    ErrSub "mod_SysLib", "Get_SO004F_Seq_No"
End Function
Public Function Get_Cmd_Seq_No(ByRef cnObj As Object) As String
  On Error GoTo ChkErr
    Get_Cmd_Seq_No = RPxx(cnObj.Execute("SELECT " & strOwner & "s_so314_cmdseqno.NEXTVAL FROM DUAL").GetString & "")
    Get_Cmd_Seq_No = Format(Get_Cmd_Seq_No, "00000000")
  Exit Function
ChkErr:
    ErrSub "mod_SysLib", "Get_Cmd_Seq_No"
End Function
