...
Private Sub cmdPrevRpt_Click()
  On Error GoTo ChkErr
    Screen.MousePointer = vbHourglass
      Call PreviousRpt(GetPrinterName(5), RptName("SO1193"), Me.Caption)
...
Private Sub subPrint(ByVal aEXETYPE As Integer)
  On Error GoTo ChkErr
    ...
    Else
        ...
        Call PrintRpt(GetPrinterName(5), "SO1193A.rpt", , "預計結算明細報表 [SO1193A]", strPrintSQL, strChooseString, , True, , , strFormula)
        SendSQL , , True
    End If
    Me.Enabled = True
    Screen.MousePointer = vbDefault
...
Private Function subCreateView(ByVal aEXETYPE As Integer) As Boolean
    ...
    strCrtSQL = "SELECT DISTINCT CUSTID,VODACCOUNTID," & _
                ...
                "ABS(SUM(MINUSCREDIT)-SUM(ADDCREDITIT)) TOTAL  FROM (" & _
...
'                strWhere3 & " GROUP BY B.VODACCOUNTID "
    ...
    strFields = "SELECT A.CUSTID,A.DeclarantName,B.VodAccountid,B.FaciSeqNo," & _
                ...
                " NVL(B.AddCredit,0) ADDCREDITIT,B.RecTime,A.FACISNO,B.ROWID ID,C.MAXNOCLOSETIME "
...
'                " AND ( B.NOCLOSETIME > " & intPara36 & " OR C.TOTAL > " & intPara35 & ")"
    ...
    Select Case aType
        ...
        Case 3
            strRetSQL = strFields & ",0 EXETYPE " & strRunSQL & _
                        " AND ( Nvl(C.MAXNOCLOSETIME,0) >= " & intPara36 & " OR ABS(Nvl(C.MINUSTOTAL,0)-Nvl(ADDTOTAL,0)) >= " & intPara35 & ")" & _
                        " UNION " & _
                        strFields & ",1 EXETYPE " & strRunSQL & _
                        " AND ( Nvl(C.MAXNOCLOSETIME,0) < " & intPara36 & " AND ABS(Nvl(C.MINUSTOTAL,0)-Nvl(C.ADDTOTAL,0)) < " & intPara35 & ")"
  ...
  On Error GoTo ChkErr
    ...
    Do While Not rs182.EOF
        ...
        If blnNOUpdate Or blnAutoEXE Then
            If Not InsCloneRs(rs182("ID") & "", "SO182", rs182Clone) Then Exit Function
            If Not InsCloneRs(rs182("ID") & "", "SO033VOD", rs033VodClone) Then Exit Function
        End If
...
End Function
Private Function InsCloneRs(ByVal aRowId As String, ByVal aTableName As String, ByRef rsClone As ADODB.Recordset) As Boolean
  On Error GoTo ChkErr
    Dim strQry As String
    Dim rsTmp As New ADODB.Recordset
    Dim i As Long
    If UCase(aTableName) = "SO033VOD" Then
        strQry = "SELECT A.ROWID,A.* FROM " & GetOwner & aTableName & " A " & _
                " WHERE A.SEQNO IN (SELECT CREDITSEQNO FROM " & GetOwner & "SO182 " & _
                                    " WHERE ROWID='" & aRowId & "' AND ADDCREDIT IS NULL )"
    Else
        strQry = "SELECT A.ROWID,A.* FROM " & GetOwner & aTableName & " A WHERE ROWID='" & aRowId & "'"
    End If
...
Private Function Insert033(ByRef aBillNo As String, ByRef rsSO182 As Recordset) As Boolean
  On Error GoTo ChkErr
    ...
    With rs033
        ...
        .Fields("OldAmt").Value = Abs(Val(rsSO182.Fields("MINUSTotal") & "") - Val(rsSO182.Fields("ADDTOTAL") & ""))
        .Fields("ShouldAmt").Value = Abs(Val(rsSO182.Fields("MINUSTotal") & "") - Val(rsSO182.Fields("ADDTOTAL") & ""))
...
Private Sub DefaultVal()
  On Error GoTo ChkErr
    ...
    If Not blnAutoEXE Then
        chkRun.Enabled = GetUserPriv("SO1193", "(SO11931)")
    End If
