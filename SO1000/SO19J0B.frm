...
Begin VB.Form frmSO19J0B 
   BorderStyle     =   1  '單線固定
   Caption         =   "CP 帳務資料拋轉作業（for 亞太）[SO19J0B]"
   ClientHeight    =   8235
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   11880
    ...
    Select Case Left(vElement, 1)
      ...
      Case "M"
        ...
        Else
          ...
          strBillNo = ""
          '*******************************************************************************************************
          '#3526 增加判斷收集年月是否符合日期格式 By Kin 2007/09/17
...
'          End If
      ...
      Case "D"
        ...
        Else
          ...
          If rsTemp.EOF Then
            blnGoOn = False
            Call LogError1(lngDetailErrorCount, "客編：" & strCustId & "　APBT帳單項目代碼不存在：" & strCpCItemCode, "", lngLineNumber, vElement)
          Else
...
'              blnGoOn = False
'              Call LogError1(lngDetailErrorCount, "客編：" & strCustId & "　APBT帳單項目名稱不存在：" & strCpCItemName, "", lngLineNumber, vElement)
'            End If
          End If
          Set rsTemp = Nothing
...
'          End If
          ...
          If blnGoOn Then
            ...
            Else
              '#4035 找不到資料就出Log,不要填成0
              Call LogError1(lngDetailErrorCount, "客編：" & strCustId & " 找不到對應的收費項目", "", lngLineNumber, vElement)
...
'            lngTempLong = rsTemp("DataCount")
            ...
            Else
              ...
              If rsTemp2.EOF Then
                '#4035 找不到資就出Log,不要填成0 By Kin 2008/08/14
                'If Not blnInDetail Then
...
'              rsSO033Cp("NodeNo") = "0"
      ...
      Case "S"
        ...
        Else
          ...
          If blnGoOn Then
          '問題2722 第四項 ----For Taos 負項金額對應負項的收費項目
            If strSign = "-" Then
              strString1 = "CItemCode2 As CItemCode"
            Else
              strString1 = "CItemCode1 As CItemCode"
...
'            Set rsTemp = gcnGi.Execute(strString1)
          ...
          If blnGoOn Then
            ...
            Else
              ...
              Else
                '#4035 找不到資料就出Log,不要填成0
                Call LogError1(lngSumErrorCount, "客編：" & strCustId & " 找不到對應的收費項目", "", lngLineNumber, vElement)
'                rsSO033("CitemCode") = 0
'                rsSO033("CitemName") = " "
...
'                rsSO033("CPGroupCode") = 0
            ...
            'If rsTemp.EOF Then
            '  If Not blnInDetail Then
            '    Call LogError1(lngSumErrorCount, "客編：" & strCustid & "　未收原因代碼未定義", "", lngLineNumber, vElement)
            '  End If
            '  rsSO033("UcCode") = "0"
            'Else
            ...
            'If rsTemp.EOF Then
            '  Call LogError1(lngSumErrorCount, "客編：" & strCustid & "　未繳費原因代碼不存在：" & rsSO033("UcCode"), "", lngLineNumber, vElement)
            '  rsSO033("UcName") = "0"
            'Else
...
'            Set rsTemp = Nothing
              '**************************************************************************
              '#3871 SO033也要一併更新InvSeqNO By Kin 2008/04/15
              strString1 = "Select FaciSeqNo, CmCode, CmName, PtCode, " & _
...
'              Set rsTemp = Nothing
            Else
              ...
              Set rsTemp2 = gcnGi.Execute(strString1)
              If rsTemp2.EOF Then
                '#4035 只要有錯誤就出Log,不要填成0 By Kin 2008/08/14
                'If Not blnInDetail Then
...
'                rsSO033("BankName") = "0"
            ...
            lngTempLong = 0
            '依裝機地址或收費地址，將地址填入到 SO033 的地址資料中
            '#4035 如果以收費地址為選項，要以SO138的收費地址為主 By Kin 2008/08/14
            If optEmpNo.Value Then
              '裝機地址
              strString1 = "Select B.ClctEn, B.ClctName, B.AddrNo, B.StrtCode, " & _
                           "       A.MduId, A.ServCode, A.ClctAreaCode, " & _
...
'                             "And (B.CustId=A.CustId) "
            ...
            '填入到 SO033 的地址資料中
            '找不到資料就出Log，不要填成0 By Kin 2008/08/14
            If rsTemp.EOF Then
              'If Not blnInDetail Then
...
'    rsTmp012.Open "Select * From " & GetOwner & "Tmp012 Where 1=0", gcnGi, adOpenKeyset, adLockOptimistic
    ...
    '＊＊＊＊＊＊ SO19J0B 亞太格式以 Asscess TMP012 暫存表格匯入比對　＊＊＊＊＊＊＊＊＊
    '＊＊＊＊＊＊ SO19J0B 亞太格式以 Asscess TMP012 暫存表格匯入比對　＊＊＊＊＊＊＊＊＊
    '建立暫存檔
    CreateTable
    '新建一個 LOG 檔
    ...
    strmTextFile1.Write vbCrLf & "行號      MIS客編   APBT門號             訊息                                              APBT                 MIS               來源單號            安裝時間            拆除時間"
...
Private Sub ScrToRcdtxt()
    ...
    '問題2727 ----Taos提  1.需改SO19J0A之前次執行參數作儲存，而非在程式寫固定 將所有拋轉檔名存成log
        Set LogFile = Fso.CreateTextFile(strErrPath & "\SO19JOB.log", True)
        With LogFile
                .WriteLine (txtbillYM.Text)            '通信費出帳年月
                .WriteLine (txtAreaCode.Text)          '發話方區域號碼
                .WriteLine (txtNumCount.Text)          '發話方電話號碼數
                .WriteLine (txtfilter.Text)            '排除不比對之字串
                .WriteLine (txtExportFile1.Text)       'CP通信費銷帳資料匯出 拋轉檔名
                .WriteLine (txtExportFile2.Text)       'CP月租費資料匯出 拋轉檔名
                .WriteLine (txtExportFile3.Text)       'CP退租資料匯出 拋轉檔名
                .WriteLine (txtExportFile4.Text)       'CMCP強拆資料匯出 拋轉檔名
                .WriteLine (txtExportFile5.Text)       'CP通信退費資料匯出 拋轉檔名
        End With
    Exit Sub
ChkErr:
...
On Error GoTo ChkErr
  ...
  strsql = "Select CItemCode1, CItemName1, TaxCode1, TaxName1, CItemCode2, CItemName2, TaxCode2, " & _
           ...
           "From " & GetOwner & "So165"
  Call ReadyGoPrint
  Call PrintRpt2(GetPrinterName(5), RptName("SO19J0"), , "CP 帳務資料拋轉作業（for 亞太網 APBT）[SO19J0B]", strsql, "全部", "", True, , False, , GiPaperLandscape)
  Exit Sub
ChkErr:
...
'********************************************************************
'#3195 原本從文字檔的內容寫到元件現在改為Ini檔 By Kin 2007/07/13
'  Call RcdToScrtxt
'  If txtExportFile1 = "" Then txtExportFile1 = "NVW_PIH_U"
'  If txtExportFile2 = "" Then txtExportFile2 = "NVW_PIH_R"
'  If txtExportFile3 = "" Then txtExportFile3 = "NVW_REMOVE_R"
...
'*********************************************************************
  '#3195 將元件的預設改由Ini讀入 By Kin 2007/07/13
...
On Error GoTo ChkErr
  ...
  '行號
  strString1 = ""
  '#3703測試不OK,要多增加"電子檔中第"字樣 By Kin 2008/04/24
        ...
        If rsTemp("ShouldAmt") <> rsTemp("RealAmt") Then
         ...
         If rsTemp("CpExport") <> 0 Then
           strString1 = "客編(" & rsTemp("CustId") & ")，" & _
                        ...
                        "APBT服務代碼(" & rsTemp("TfnServiceId") & ")，" & _
                        "單據編號(" & rsTemp("BillNo") & ")，" & _
                        "APBT帳單編號(" & rsTemp("TfnBillNo") & ")，" & _
      ...
      Case "B"
        ...
        If chkTFNSID.Value = 1 Then
            If Trim(UBound(vDataArray2)) <> 11 Then
              Call LogError1(lngBodyErrorCount, "資料欄位數目錯誤，欄位必須是十二欄", "", lngLineNumber, vElement)
              blnGoOn = False
            End If
        Else
            If Trim(UBound(vDataArray2)) <> 10 Then
              Call LogError1(lngBodyErrorCount, "資料欄位數目錯誤，欄位必須是十一欄", "", lngLineNumber, vElement)
              blnGoOn = False
            End If
        ...
        If blnGoOn Then
          ...
          strString1 = "Insert Into Tmp012 " & _
                       "Values ( "
          For lngIndex = 1 To UBound(vDataArray2)  '因為文字檔的最後一欄不匯入,所以要減 1
...
'      strString1 = "Select SeqNo, InstDate, PrDate " & _
      ...
      '問題集2722 ----for Taos
      '以 1.開通合約編號，2.CP門號 二條件來判斷
      '有值,即 Update TfnServiceId 並判斷其1.客編,2.身份證字號,3.申裝人姓名,4.設備安裝日期 等
      '無值,即出log
...
'      是，已存在，要 Update 相關欄位
      ...
      If Not rsTemp2.EOF Then
        ...
        If Trim(rsTemp("CustId") & "") <> rsTemp2("CustId") & "" Then
           Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "客戶編號不合!", rsTemp("CustId") & "", rsTemp2("CustId") & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
        End If
        ...
        If (UCase(rsTemp2("Id")) & "" <> UCase(rsTemp("Id")) & "") And (UCase(rsTemp2("Id2")) & "" <> UCase(rsTemp("Id")) & "") Then
          ...
          End If
          Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "身份證字號不合!", rsTemp("Id") & "", strString1 & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
        End If
        
        If Trim(rsTemp2("DeclarantName")) & "" <> strDeclarantName Then
          '判斷客戶名稱
          Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "登記姓名不合!", strDeclarantName & "", rsTemp2("DeclarantName") & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
        End If
        
        If (rsTemp("InstDate") & "") <> Format(rsTemp2("InstDate") & "", "YYYYMMDD") Then
          Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "服務啟始日期不合!", rsTemp("InstDate") & "", rsTemp2("InstDate") & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
        End If
        
        If (rsTemp("PrDate") & "") <> Format(rsTemp2("PrDate") & "", "YYYYMMDD") Then
          Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "服務結束日期不合!", rsTemp("PrDate") & "", rsTemp2("PrDate") & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
        End If
...
'                    Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "此CP門號不存在!", strTel, "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
'              End If
'            If Trim(rsTemp("CustId") & "") <> rsTemp2("CustId") & "" Then
'               Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "客戶編號不合!", rsTemp("CustId") & "", rsTemp2("CustId") & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
'            End If
...
'            If rsTemp2("FaciSNo") & "" <> strTel Then
'              '判斷 FaciSNo
'              Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "客戶設備找不到!", strTel & "", rsTemp2("FaciSNo") & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
'            End If
'
'            If (rsTemp("InstDate") & "") <> Format(rsTemp2("InstDate") & "", "YYYYMMDD") Then
'              Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "服務啟始日期不合!", rsTemp("InstDate") & "", rsTemp2("InstDate") & "", rsTemp2("SNo") & "", rsTemp2("InstDate") & "", rsTemp2("PrDate") & "", rsTemp("RecNo") & "")
...
'                '判斷 TFNServiceID
'                Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "EBT合約編號不符!", rsTemp("EBTContNo") & "", rsTemp2("FaciSNo") & "", rsTemp("RecNo") & "")
'              End If
'            End If
        Else
            Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "EBT合約編號不符!", rsTemp("EBTContNo") & "", "", "", "", "", rsTemp("RecNo") & "")
...
''              If Trim(rsTemp("EBTContNo") & "") <> rsTemp2("EBTContNo") & "" Then
''                '判斷 TFNServiceID
''                Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "EBT合約編號不符!", rsTemp("EBTContNo") & "", rsTemp2("EBTContNo") & "", rsTemp("RecNo") & "")
''              End If
''            End If
'          lngIndex = lngBodyErrorCount
'          If rsTemp2.EOF Then
...
'                Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "客戶編號不合!", rsTemp("CustId") & "", rsTemp2("CustId") & "", rsTemp("RecNo") & "")
'              Else
'                Call LogError22(lngBodyErrorCount, rsTemp("TFNServiceID") & "", rsTemp("CustId") & "", strTel & "", "此CP門號不存在!", strTel, "", rsTemp("RecNo") & "")
'              End If
'            End If
'            Set rsTemp2 = Nothing
'          Else
...
Private Sub ImportData3()
    ...
    If optDetailMonthly.Value And blnGoOn Then
        ...
        If blnGoOn2 Then
            ...
            If optDetailMonthly.Value And blnGoOn Then
                ...
                If Not rsTemp.EOF Then
                    blnGoOn2 = MsgBox("出帳年月 " & Trim(txtbillYM) & "：該月份CDR資料已匯入，是否清空該月份的CDR資料，再匯入？", vbYesNo, "警告") = 6
                    '#3703 測試不OK,如果選擇否 則最後不出出現任何警告訊息 By Kin 2008/04/23
  ...
  'EBTContNo
    ...
    If blnGoOn2 And blnGoOn Then
        ...
        '客編,發話方區域號碼,發話方電話號碼,發話起始時間,受話方區域號碼,受話方電話號碼
        gcnGi.Execute strString1
      ...
      '若匯入失敗，則將已匯入 SO168/SO169 的資料取消
        ...
        For lngIndex = 1 To lngLastLine - 1
            ...
            If Not IsNumeric(vDataArray2(16)) Then
                Call LogError1(lngErrorCount, "客編：" & vDataArray2(0) & "　APBT用戶代號：" & vDataArray2(16) & "，格式錯誤", "", lngIndex + 1, vDataArray(lngIndex))
                blnGoOn = False
            End If
...
'                             "And (hour_type='" & vDataArray2(15) & "')"
                ...
                '#3703 測試不OK 原本串Duration,將此欄位拿掉,改串Calling_Area_Code By Kin 2008/04/23
                strString1 = "Select * From " & StrTableName & " " & _
                             "Where (Account_Id='" & vDataArray2(1) & "') " & _
                             "And (Called_Nbr='" & vDataArray2(8) & "') " & _
                             "And (Start_Time=To_Date('" & vDataArray2(10) & "', 'YYYY/MM/DD HH24:MI:SS')) " & _
                ...
                Else
                    ...
                    '***********************************************************************************************************************************************************************************
                    '#3703 測試不OK,錯誤訊息沒有客編 By Kin 2008/04/24
                    strCustId = ""
                    If Not GetRS(rsSO004, "SELECT Distinct(Custid) FROM " & GetOwner & "SO004 WHERE TFNSERVICEID= '" & vDataArray2(16) & "'", gcnGi, adUseClient, adOpenKeyset, adLockOptimistic) Then Exit Sub
                    If Not rsSO004.EOF Then strCustId = rsSO004("CustId") & ""
                    '**********************************************************************************************************************************************************************************
                    Call LogError1(lngErrorCount, "客編：" & strCustId & "　要匯入的資料已存在於資料庫中", "", lngIndex + 1, vDataArray(lngIndex))
                    'rsSo168.Close
                End If
    ...
    gcnGi.CommitTrans
    '***************************************************************************************************
    '#3703 判斷是否有選擇清除再重新匯入,如果選擇否則不出任何警告 By Kin 2008/04/23
...
End Sub
'#3195將要存在SO19J0B.Ini的資訊先建立一個Collect集合
